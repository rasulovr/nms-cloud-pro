...
function Recipes({ t }) {
  ...
  const [ingredientForm, setIngredientForm] = useState({ name: '', unit: 'g', purchase_price: '', purchase_unit_quantity: '1000' })
  ...
  function unitLabel(unit) { return BASE_UNITS.find(u => u.value === unit)?.label || unit || '—' }
  function shortUnit(unit) { return unit || '—' }
  ...
  function recipeLineCost(row) {
    const usageQty = parseNum(row.quantity)
    const waste = parseNum(row.waste_percent)
    return ingredientUnitCost(row.ingredients) * usageQty * (1 + waste / 100)
  }
  ...
  async function addIngredient() {
    ...
    setIngredientForm({ name: '', unit: 'g', purchase_price: '', purchase_unit_quantity: '1000' })
    await loadBase()
    setMessage(t('saved'))
  }
  ...
  async function addRecipeItem() {
    ...
    const { error } = await supabase.from('recipe_items').insert({ menu_item_id: selectedMenuId, ingredient_id: ingredient.id, quantity: 0, waste_percent: 0 })
    if (error) return setMessage(error.message)
  ...
  function fillCappuccinoExample() {
    setMessage('Пример: молоко = 2.20 AZN / 1000 ml, кофе = 38 AZN / 1000 g. В техкарте: кофе 18 g, молоко 200 ml.')
  }
  ...
  return (
    <section>
      ...
      <section className="grid">
        ...
        <div className="card span-2">
          <div className="card-head"><div><h3>Блюда / позиции меню</h3><p className="hint">Категория выбирается из списка.</p></div></div>
          ...
          <div className="table-wrap"><table><thead><tr><th>Блюдо</th><th>Категория</th><th>Цена</th><th>Цель food cost</th><th></th></tr></thead><tbody>
        ...
        <div className="card span-2">
          <div className="card-head"><div><h3>Ингредиенты / закупочные цены</h3><p className="hint">Например: кофе — 38 AZN / 1000 g; молоко — 2.20 AZN / 1000 ml.</p></div></div>
          <div className="form-grid compact">
            <label><span>Ингредиент</span><input value={ingredientForm.name} onChange={e => setIngredientForm({...ingredientForm, name: e.target.value})} placeholder="Молоко / Кофе / Сыр" /></label>
            <label><span>Ед. использования</span><select value={ingredientForm.unit} onChange={e => setIngredientForm({...ingredientForm, unit: e.target.value})}>{BASE_UNITS.map(u => <option key={u.value} value={u.value}>{u.label}</option>)}</select></label>
            <label><span>Цена закупки, AZN</span><input inputMode="decimal" value={ingredientForm.purchase_price} onChange={e => setIngredientForm({...ingredientForm, purchase_price: e.target.value})} placeholder="2.20" /></label>
            <label><span>Размер закупки в ед. использования</span><input inputMode="decimal" value={ingredientForm.purchase_unit_quantity} onChange={e => setIngredientForm({...ingredientForm, purchase_unit_quantity: e.target.value})} placeholder="1000" /></label>
          </div>
          <p className="hint">Если молоко покупается пачкой 1 литр — ед. использования ml, размер закупки 1000. Если кофе покупается 1 кг — ед. использования g, размер закупки 1000.</p>
          <button className="small" onClick={addIngredient}>+ Добавить ингредиент</button><br /><br />
          <div className="table-wrap"><table><thead><tr><th>Ингредиент</th><th>Ед. использования</th><th>Цена закупки</th><th>Размер закупки</th><th>Цена за 1 ед.</th><th></th></tr></thead><tbody>
            {ingredients.map(item => <tr key={item.id}>
              ...
              <td><select value={item.unit || 'g'} onChange={e => updateIngredient(item.id, { unit: e.target.value })}>{BASE_UNITS.map(u => <option key={u.value} value={u.value}>{u.label}</option>)}</select></td>
              ...
              <td>{fmt(ingredientUnitCost(item))} / {shortUnit(item.unit)}</td>
        ...
        <div className="card span-2">
          <div className="card-head"><div><h3>Состав техкарты</h3><p className="hint">Выбранное блюдо: <strong>{selectedMenu?.name || 'не выбрано'}</strong>. Здесь вводится количество использования в блюде.</p></div><button className="small" onClick={addRecipeItem}>+ Ингредиент в техкарту</button></div>
          <div className="form-grid compact"><label><span>Выбрать блюдо</span><select value={selectedMenuId} onChange={e => setSelectedMenuId(e.target.value)}><option value="">Выберите блюдо</option>{menuItems.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div>
          <div className="table-wrap"><table><thead><tr><th>Ингредиент</th><th>Кол-во использования</th><th>Ед.</th><th>Потери %</th><th>Цена за 1 ед.</th><th>Сумма</th><th></th></tr></thead><tbody>
            {recipeItems.map(row => <tr key={row.id}>
              <td><select value={row.ingredient_id} onChange={e => updateRecipeItem(row.id, { ingredient_id: e.target.value })}>{ingredients.map(i => <option key={i.id} value={i.id}>{i.name}</option>)}</select></td>
              ...
              <td>{unitLabel(row.ingredients?.unit)}</td>
              ...
              <td>{fmt(ingredientUnitCost(row.ingredients))} / {shortUnit(row.ingredients?.unit)}</td>
              <td><strong>{fmt(recipeLineCost(row))}</strong></td>
              <td><button className="remove" onClick={() => deleteRecipeItem(row.id)}>×</button></td>
            </tr>)}
          </tbody></table></div>
          {recipeItems.length === 0 && <p className="hint">Пока нет ингредиентов в техкарте выбранного блюда.</p>}
          <p className="hint">Формула: цена закупки / размер закупки × количество использования × потери. Например, молоко 2.20 / 1000 × 200 = 0.44 AZN.</p>
