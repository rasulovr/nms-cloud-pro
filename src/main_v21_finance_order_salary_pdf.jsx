...
function Salaries({ t }) {
  ...
  function printSalaryEmployeesPdf() {
    ...
    const rowsHtml = salaryRows.map(r => {
      ...
      const cells = hidden
        ...
        : [
            ...
            fmt(r.deduction_amount),
            fmt(r.salary_net),
            fmt(r.payroll_payments),
            fmt(r.worked_days),
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>${title}</title><style>
      body{font-family:Arial,sans-serif;padding:24px;color:#17251d}
      ...
      th{background:#f4eddf}
      .summary{display:flex;gap:12px;margin:14px 0 18px}
      .box{border:1px solid #ddd;border-radius:12px;padding:10px 14px}
      ...
      <div class="summary">
        ...
        <div class="box">К выплате / баланс<b>${fmt(totals.balance)}</b></div>
        <div class="box">Выплачено<b>${fmt(totals.payments)}</b></div>
      </div>
      <table>
        <thead><tr><th>Филиал</th><th>Должность</th><th>Сотрудник</th><th>Начислено</th><th>К выплате</th><th>Авансы</th><th>Удержание</th><th>Остаток месяца</th><th>Выплачено</th><th>Дни</th><th>Долг нач.</th></tr></thead>
        <tbody>${rowsHtml || '<tr><td colspan="11">Нет сотрудников</td></tr>'}</tbody>
      </table>
    </body></html>`
