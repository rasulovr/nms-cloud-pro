...
function extractToken(value) {
  ...
  try {
    ...
    // QR scanner may return plain token
  }
...
export default function RMSLoyaltyPOSScan() {
  ...
  async function findClientByToken(e) {
    e?.preventDefault?.()
    setMessage('')
    setTokenRow(null)
    setClient(null)
    ...
    <div className="loyalty-scan-page">
      <section className="loyalty-scan-hero">
        <div>
          ...
          <p>Привяжите чек к клиенту и начислите cashback после оплаты наличными или картой.</p>
        </div>
      ...
      <section className="loyalty-scan-grid">
        <div className="loyalty-scan-card">
          <h2>1. QR-код клиента</h2>
          <p>Сканер обычно вставляет содержимое QR в поле ниже. Можно вставить JSON, URL или plain token.</p>

          <form onSubmit={findClientByToken} className="loyalty-scan-form">
            <label>
              QR / Token
              <textarea
                value={rawScan}
                onChange={(e) => setRawScan(e.target.value)}
                placeholder='Например: {"type":"rms_loyalty","token":"..."}'
              />
            </label>
        ...
        <div className="loyalty-scan-card">
          ...
          {client ? (
            ...
            <div className="loyalty-scan-empty">Клиент появится здесь после сканирования QR.</div>
          )}
        </div>
      </section>
    </div>
