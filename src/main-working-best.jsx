...
async function fetchRmsReferenceWorkspaceSnapshot() {
  const { data, error } = await supabase.rpc('rms_reference_workspace_snapshot')
  if (error) return { data: null, error }
  const normalized = data && typeof data === 'object' ? data : null
  if (!normalized) return { data: null, error: new Error('Empty RMS reference workspace') }
  return { data: normalized, error: null }
}
    ...
    <div className={`app theme-${theme || 'classic'}`}>
      <aside className="sidebar">
        ...
        <div className="userbar">
          ...
          <button className="ghost small" onClick={logout}>{t('logout')}</button>
        </div>
      </aside>
...
function useBranches() {
  ...
  useEffect(() => {
    ...
    async function loadBranches() {
      ...
      const { data: ref } = await fetchRmsReferenceWorkspaceSnapshot()
      if (ref?.branches?.length) {
        if (alive) setBranches(ref.branches)
        return
      }
...
function Recipes({ t }) {
  ...
  async function loadBase() {
    ...
    if (isInternal) {
      const { data: ref, error: refError } = await fetchRmsReferenceWorkspaceSnapshot()
      if (!refError && ref) {
        setProducts(ref.supplier_products || [])
        setMenuItems(ref.menu_items || [])
        setCosts(ref.latest_product_costs || [])
        setAllRecipeItems(ref.recipe_items || [])
        if (!selectedMenuId && ref.menu_items?.[0]) setSelectedMenuId(ref.menu_items[0].id)
        return
      }
    }

    const [{ data: prod, error: prodError }, { data: item, error: itemError }, { data: latest, error: latestError }, { data: recipes, error: recipeError }] = await Promise.all([
    ...
    if (prodError || itemError || latestError || recipeError) {
      const { data: ref } = await fetchRmsReferenceWorkspaceSnapshot()
      if (ref) {
        setProducts(ref.supplier_products || [])
        setMenuItems(ref.menu_items || [])
      ...
      setMessage(prodError?.message || itemError?.message || latestError?.message || recipeError?.message || 'Нет доступа к тех. картам')
      return
    }
  ...
  async function loadRecipeItems(menuId = selectedMenuId) {
    ...
    if (error) {
      const { data: ref } = await fetchRmsReferenceWorkspaceSnapshot()
      const rows = (ref?.recipe_items || []).filter(r => r.menu_item_id === menuId)
      setAllRecipeItems(ref?.recipe_items || [])
      setRecipeItems(rows)
...
function Settings({ session, t, theme, setTheme }) {
  ...
  async function deleteUser(user) {
    setMsg('')
    const login = normalizeInternalLogin(user?.login_name || user?.email || user?.id)
    const label = user?.login_name || user?.email || user?.full_name || user?.id

    if (!user?.id) return setMsg('Не удалось определить пользователя')
    if (session?.user?.id === user.id) return setMsg('Нельзя удалить текущего пользователя, под которым выполнен вход')
    if (!window.confirm(`Удалить пользователя ${label}?`)) return

    const internalUsers = getInternalUsers()
    const localLogin = internalUsers[login]
      ? login
      : Object.keys(internalUsers).find(k => internalUsers[k]?.id === user.id || normalizeInternalLogin(internalUsers[k]?.login) === login)
    ...
    if (localLogin) {
      ...
      if (active?.user?.id === userId || normalizeInternalLogin(active?.user?.login_name || active?.user?.email) === localLogin) {
        setInternalSessionStorage(null)
      }
    ...
    // Для старых/ошибочно созданных пользователей удаляем профиль и права из RMS.
    // Auth-запись Supabase из браузера удалить нельзя, но после удаления профиля пользователь исчезает из управления RMS.
    ...
    setMsg(`Профиль пользователя ${label} удалён из RMS`)
    window.dispatchEvent(new Event('rms-user-settings-updated'))
    await load()
  }
