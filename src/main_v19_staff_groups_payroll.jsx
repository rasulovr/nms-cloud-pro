...
function Revenue({ t }) {
  ...
  const [monthStats, setMonthStats] = useState({ cash: 0, bank: 0, wolt: 0, revenue: 0, expenses: 0 })
  ...
  async function loadMonthStats(activeBranchId = branchId, activeDate = date) {
    ...
    const [{ data: monthRows }, { data: expRows }] = await Promise.all([
    ...
    setMonthStats({ cash, bank, wolt, revenue: cash + bank + wolt, expenses })
  }
  ...
  const expectedClosingCash = parseNum(cashForm.opening_cash) + dailyCashRevenue - dailyExpenseTotal
...
function entityLabel(entity) {
  const labels = { revenue: 'Выручка', expense: 'Расход', cash: 'Касса' }
...
function fieldLabel(field) {
  const labels = { daily_revenue: 'Выручка за дату', daily_revenue_entry: 'Строка выручки', cash_register: 'Касса за день', expense: 'Расход', cash_amount: 'Наличные', bank_amount: 'Банк', wolt_amount: 'Wolt', comment: 'Комментарий', amount: 'Сумма', category_id: 'Статья', custom_category: 'Своя статья' }
