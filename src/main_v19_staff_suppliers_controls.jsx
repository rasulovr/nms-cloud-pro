...
const calcGrossSalary = (emp, workedDays) => calcDailyRate(emp, workedDays) * parseNum(workedDays)
...
function Salaries({ t }) {
  ...
  async function load() {
    ...
    setRows((emp || []).map(e => {
      ...
      return { ...salary, employees: { ...e, ...(salary.employees || {}) }, branches: e.branches || salary.branches, branch_id: e.branch_id, advance_amount: advanceTotal, salary_net: net }
    }))
  }

  async function updateEmployeeDetails(row, patch) {
    setMessage('')
    const payload = { ...patch }
  ...
  async function updateSalary(row, patch) {
    ...
    if ('worked_days' in payload && !('salary_gross' in payload)) payload.salary_gross = calcGrossSalary(row.employees, payload.worked_days)
    ...
    if ('salary_gross' in payload || 'deduction_amount' in payload || 'worked_days' in payload) payload.salary_net = gross - advance - deduction
  ...
  return <section>
    ...
    <section className="grid">
      ...
      <div className="card span-2">
        ...
        <div className="table-wrap"><table>
          <thead><tr><th>Филиал</th><th>Должность</th><th style={{minWidth:280}}>Сотрудник</th><th>Инфо</th><th>Расчёт</th><th>Мес. ставка</th><th>Дневная</th><th style={{minWidth:90}}>Дни</th><th>Начислено</th><th>Авансы</th><th>Удержание</th><th>Карта</th><th>Наличные</th><th>Остаток</th><th>Комментарий</th></tr></thead>
          <tbody>{rows.map(r => <React.Fragment key={r.id || r.employee_id}>
            <tr>
              <td>{r.branches?.name || '—'}</td><td>{r.employees?.position || '—'}</td><td style={{minWidth:280}}>{r.employees?.full_name}</td>
              <td><button className="ghost small" onClick={() => setEmployeeInfoId(employeeInfoId === r.employee_id ? '' : r.employee_id)}>i</button></td>
              <td><select value={r.employees?.salary_type || 'monthly'} onChange={e => updateEmployeeDetails(r, { salary_type: e.target.value })}><option value="monthly">Фикс</option><option value="daily">Ежедневный</option></select></td>
              <td><input inputMode="decimal" defaultValue={r.employees?.monthly_salary || 0} onBlur={e => updateEmployeeDetails(r, { monthly_salary: e.target.value })} /></td>
              <td><input inputMode="decimal" key={(r.employees?.salary_type || 'monthly') + '-' + (r.employees?.daily_rate || '') + '-' + (r.employees?.monthly_salary || '')} defaultValue={fmt(calcDailyRate(r.employees))} readOnly={r.employees?.salary_type !== 'daily'} onBlur={e => r.employees?.salary_type === 'daily' && updateEmployeeDetails(r, { daily_rate: e.target.value })} /></td>
            ...
            {employeeInfoId === r.employee_id && <tr><td colSpan="15"><div className="notice"><div className="form-grid compact">
              ...
              <label><span>Удостоверение</span><input defaultValue={r.employees?.document_info || ''} onBlur={e => updateEmployeeDetails(r, { document_info: e.target.value.trim() || null })} placeholder="серия, номер, срок" /></label>
              <label><span>Заявления</span><input defaultValue={r.employees?.statement_info || ''} onBlur={e => updateEmployeeDetails(r, { statement_info: e.target.value.trim() || null })} placeholder="отпуск, увольнение, прочее" /></label>
...
function Advances({ t }) {
  ...
  async function deleteAdvance(row) {
    setMessage('')
    if (!canEditAdvance(row)) return setMessage('Аванс можно удалить только в течение 24 часов после создания')
      ...
      <div className="card span-2">
        ...
        <div className="table-wrap"><table>
          ...
          <tbody>{advances.map(a => {
            const editable = canEditAdvance(a)
            return <tr key={a.id} className={editable ? '' : 'muted-row'}>
              ...
              <td><button className="remove" disabled={!editable} onClick={() => deleteAdvance(a)}>×</button></td>
            </tr>
