...
export default function RMSQRMenuAdmin() {
  ...
  async function loadAll() {
    setLoading(true)
    await Promise.all([loadProducts(), loadRatings(), loadAds(), loadBills(), loadInfo(), loadRecommendations()])
    setLoading(false)
  }
  ...
  async function loadRatings() {
    const { data } = await supabase.from('rms_qr_dish_ratings').select('*').order('created_at', { ascending: false }).limit(200)
    setRatings(data || [])
  }
  ...
  async function loadInfo() {
    const { data } = await supabase.from('rms_qr_info').select('*').eq('branch_id', 'BC1').maybeSingle()
  ...
  const ratingSummary = useMemo(() => {
    ...
    return Object.entries(grouped).map(([product_id, v]) => ({ product_id, product_name: v.product_name, avg: v.sum / v.count, count: v.count }))
  }, [ratings])
  ...
  return (
    <section>
      <section className="topbar">
        <div>
          <h2>QR Menu</h2>
          <p>Управление гостевым QR Menu: позиции, фото, рейтинги, реклама, рекомендации, счёт и информация для гостей.</p>
        </div>
        <button className="small primary" onClick={loadAll}>{loading ? 'Загрузка...' : 'Обновить'}</button>
      </section>

      <div className="settings-tabs">
        <button className={tab === 'menu' ? 'active' : ''} onClick={() => setTab('menu')}>Позиции и фото</button>
        ...
        {tab === 'menu' && (
          <div className="card span-2">
            ...
            <div className="table-wrap">
              <table>
                <thead><tr><th>Фото</th><th>Позиция</th><th>Категория</th><th>Цена</th><th>Image URL</th><th>Активна</th></tr></thead>
                <tbody>
                  {products.map(p => (
                    <tr key={p.id}>
                  ...
                  {!products.length && <tr><td colSpan="6" className="hint">Позиции не найдены.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
