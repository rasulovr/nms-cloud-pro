...
function Salaries({ t, view = 'employees' }) {
  ...
  function exportSalarySheetCsv() {
    ...
    const lines = [
      ...
      ['ИТОГО', '', '', fmt(salarySheetTotals.days), fmt(salarySheetTotals.base), fmt(salarySheetTotals.accrued), fmt(salarySheetTotals.advances), fmt(salarySheetTotals.toPay)].map(escapeCsv).join(';')
    ]
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>${title}</title>
      ...
      <div class="page"><div class="sheet">
        ...
        <table>
          ...
          <tfoot><tr><td colspan="3">Итого</td><td class="num">${fmt(salarySheetTotals.days)}</td><td class="num">${fmt(salarySheetTotals.base)}</td><td class="num">${fmt(salarySheetTotals.accrued)}</td><td class="num">${fmt(salarySheetTotals.advances)}</td><td class="num pay">${fmt(salarySheetTotals.toPay)}</td></tr></tfoot>
        </table>
      </div></div>
    </body></html>`
      ...
      <section className="grid">
        <div className="card span-2">
          ...
          <div className="grid mini-grid">
            <div className="metric"><span>Дни</span><strong>{fmt(salarySheetTotals.days)}</strong></div>
            ...
            <table>
              ...
              <tfoot>
                <tr>
                  ...
                  <td><strong>{fmt(salarySheetTotals.days)}</strong></td>
