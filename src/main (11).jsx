...
const THEMES = [
  ...
  { id: 'executive', name: 'Executive / Glass Pro' }
]
...
const normalizeInternalLogin = (value) => String(value || '').trim().toLowerCase().replace(/@(rms|nms)\.local\.az$/, '').replace(/@rms\.internal$/, '')
const readJsonStorage = (key, fallback) => { try { return JSON.parse(localStorage.getItem(key) || JSON.stringify(fallback)) } catch (_e) { return fallback } }
const writeJsonStorage = (key, value) => { try { localStorage.setItem(key, JSON.stringify(value)) } catch (_e) {} }
const getInternalUsers = () => readJsonStorage(RMS_INTERNAL_USERS_KEY, null) || readJsonStorage('rms_internal_users_v1', null) || readJsonStorage('rms_internal_users_v2', {}) || {}
const setInternalUsers = (users) => writeJsonStorage(RMS_INTERNAL_USERS_KEY, users || {})
const getInternalPermissions = () => readJsonStorage(RMS_INTERNAL_PERMISSIONS_KEY, null) || readJsonStorage('rms_internal_permissions_v1', null) || readJsonStorage('rms_internal_permissions_v2', {}) || {}
const setInternalPermissions = (perms) => writeJsonStorage(RMS_INTERNAL_PERMISSIONS_KEY, perms || {})
const getInternalSessionStorage = () => readJsonStorage(RMS_INTERNAL_SESSION_KEY, null) || readJsonStorage('rms_internal_session_v1', null)
const setInternalSessionStorage = (session) => { if (session) writeJsonStorage(RMS_INTERNAL_SESSION_KEY, session); else localStorage.removeItem(RMS_INTERNAL_SESSION_KEY) }
const RMS_DATA_CACHE_KEY = 'rms_data_cache_v1'
const getRmsDataCache = () => readJsonStorage(RMS_DATA_CACHE_KEY, {}) || {}
const setRmsDataCache = (patch) => writeJsonStorage(RMS_DATA_CACHE_KEY, { ...getRmsDataCache(), ...(patch || {}), updated_at: new Date().toISOString() })
const cacheSalarySnapshot = (snapshot) => setRmsDataCache({ salary: { ...(snapshot || {}), updated_at: new Date().toISOString() } })
const getCachedSalarySnapshot = () => getRmsDataCache()?.salary || null
const cacheAttendanceSnapshot = (snapshot) => setRmsDataCache({ attendance: { ...(snapshot || {}), updated_at: new Date().toISOString() } })
const getCachedAttendanceSnapshot = () => getRmsDataCache()?.attendance || null
...
function getRmsLocalUser() {
  ...
  const localUser = getInternalUsers()[login] || Object.values(getInternalUsers()).find(u => u?.id === sess?.user?.id)
  ...
  return (
    <div className={`app theme-${theme || 'classic'}`}>
      <aside className="sidebar">
        <div className="brand"><ProductLogo compact /><div><h1>{t('system_short_title')}</h1><p>{t('brand_subtitle')}</p></div></div>
        <nav className="nav-tabs">{visibleSections.map(s => <button key={s.id} className={`nav ${section === s.id ? 'active' : ''}`} onClick={() => setSection(s.id)}>{t(s.key)}</button>)}</nav>
        <div className="userbar">
          <span>{profile?.full_name || profile?.login_name || session.user.email}</span>
          <button className="ghost small" onClick={() => { if (session?.rms_internal) { setInternalSessionStorage(null); setSession(null); setProfile(null); setPermissions([]) } else { supabase.auth.signOut() } }}>{t('logout')}</button>
        </div>
      </aside>
...
function Login({ lang, setLang, t, onInternalLogin }) {
  ...
  async function signIn() {
    ...
    const localUser = getInternalUsers()[internalLogin]
    if (localUser) {
      if (localUser.is_active === false) return setError('Пользователь отключён')
      if (String(localUser.password || '') !== String(password || '')) return setError(t('login_error'))
      const nextSession = {
        rms_internal: true,
        user: {
          id: localUser.id,
          email: `${localUser.login}@rms.internal`,
          login_name: localUser.login,
          full_name: localUser.full_name || localUser.login
        }
      }
  ...
  return <div className="login-screen"><div className="login-card">
    ...
    <p>Admin входит через Supabase. Внутренние пользователи входят по login, созданному в настройках RMS.</p>
...
function ThemeStyles() {
  return <style>{`
    ...
    .theme-executive { --bg: #0b1220; --panel: rgba(15,23,42,.72); --ink: #e5eefc; --muted: #97a9c5; --line: rgba(148,163,184,.20); --accent: #60a5fa; --gold: #22d3ee; --danger: #f87171; --good: #34d399; }
    .theme-executive .sidebar { background: linear-gradient(180deg, rgba(2,6,23,.96), rgba(15,23,42,.98) 52%, rgba(30,41,59,.98)); box-shadow: 18px 0 40px rgba(2,6,23,.45); border-right: 1px solid rgba(148,163,184,.16); }
    .theme-executive .main { background: radial-gradient(circle at top left, rgba(96,165,250,.14), transparent 28%), radial-gradient(circle at top right, rgba(34,211,238,.12), transparent 24%), linear-gradient(180deg, #0b1220 0%, #0f172a 100%); color: var(--ink); }
    .theme-executive .card, .theme-executive .dash-kpi { background: linear-gradient(180deg, rgba(15,23,42,.78), rgba(15,23,42,.64)); border-color: rgba(148,163,184,.18); box-shadow: 0 22px 55px rgba(2,6,23,.34); backdrop-filter: blur(16px); }
    .theme-executive .topbar h2, .theme-executive h3, .theme-executive .big-number, .theme-executive .card-head h3, .theme-executive .brand h1 { color: #f8fbff; }
    .theme-executive .brand p, .theme-executive .userbar span, .theme-executive .hint, .theme-executive .dash-kpi span, .theme-executive .metric span { color: #9fb2d0; }
    .theme-executive .nav { background: rgba(255,255,255,.04); color: #d8e4f8; border: 1px solid rgba(148,163,184,.12); }
    .theme-executive .nav:hover { background: rgba(255,255,255,.08); }
    .theme-executive .nav.active { background: linear-gradient(135deg, rgba(96,165,250,.95), rgba(34,211,238,.92)); color: #06111f; box-shadow: 0 12px 28px rgba(96,165,250,.25); border-color: transparent; }
    .theme-executive input, .theme-executive select, .theme-executive textarea { background: rgba(15,23,42,.68); color: #e5eefc; border-color: rgba(148,163,184,.22); }
    .theme-executive input::placeholder, .theme-executive textarea::placeholder { color: #7e93b3; }
    .theme-executive .button, .theme-executive button.primary, .theme-executive .primary { background: linear-gradient(135deg, #60a5fa, #22d3ee); color: #06111f; border-color: transparent; }
    .theme-executive button.ghost, .theme-executive .ghost { background: rgba(255,255,255,.04); color: #dbe7f8; border-color: rgba(148,163,184,.18); }
    .theme-executive table th { background: rgba(255,255,255,.03); color: #b8c8e3; }
    .theme-executive table td { color: #ebf2ff; border-color: rgba(148,163,184,.12); }
    .theme-executive .dash-bar { background: linear-gradient(90deg, #60a5fa, #22d3ee); }
    .theme-executive .dash-bar.negative { background: linear-gradient(90deg, #fb7185, #ef4444); }
    .theme-executive .login-card { background: linear-gradient(180deg, rgba(15,23,42,.88), rgba(15,23,42,.72)); color: #eef5ff; border-color: rgba(148,163,184,.16); box-shadow: 0 30px 70px rgba(2,6,23,.45); }
    .theme-executive .logo, .theme-executive .login-logo { background: linear-gradient(135deg, rgba(96,165,250,.22), rgba(34,211,238,.16)); color: #dff7ff; box-shadow: 0 14px 34px rgba(34,211,238,.14); }
    .theme-executive .readonly-banner { background: rgba(59,130,246,.12); border-color: rgba(96,165,250,.30); color: #dbeafe; }
  ...
  async function load() {
    ...
    if (isInternal) {
      ...
      if (snap?.employees?.length) {
        ...
        cacheAttendanceSnapshot({ employees: empRows, attendance: attRows })
        const oldSalaryCache = getCachedSalarySnapshot() || {}
        cacheSalarySnapshot({ ...oldSalaryCache, employees: empRows })
        sourceLabel = 'rpc'
      }
    }
    ...
    if (!empRows.length) {
      ...
      if (empError || attError) {
        ...
        if (snap?.employees?.length) {
          ...
          const cachedAttendance = getCachedAttendanceSnapshot()
          const cachedSalary = getCachedSalarySnapshot()
          empRows = cachedAttendance?.employees?.length ? cachedAttendance.employees : (cachedSalary?.employees || [])
          attRows = cachedAttendance?.attendance || []
          if (!empRows.length) {
          ...
          sourceLabel = 'cache'
          setMessage(`Список сотрудников показан из локального RMS-кэша. Обновлено: ${(cachedAttendance?.updated_at || cachedSalary?.updated_at) ? formatDT(cachedAttendance?.updated_at || cachedSalary?.updated_at) : '—'}`)
        }
      } else {
        cacheAttendanceSnapshot({ employees: empRows, attendance: attRows })
        const oldSalaryCache = getCachedSalarySnapshot() || {}
    ...
    if (sourceLabel === 'rpc') setMessage('Список сотрудников загружен через защищённый RMS workspace.')
    const visibleEmployees = (empRows || []).filter(e => e.is_active !== false).filter(e => matchesStaffGroup(e, branchId)).filter(e => matchesPositionGroup(e, positionFilter))
  ...
  async function load() {
    ...
    if (isInternal) {
      ...
      if (snap?.employees?.length) {
        ...
        cacheSalarySnapshot({
          employees: empRows,
          salary_periods: allSalaryPeriods,
          salary_advances: snap.salary_advances || [],
          salary_payments: allSalaryPayments,
          dsmf_expenses: snap.dsmf_expenses || []
        })
        sourceLabel = 'rpc'
      }
    }
    ...
    if (!empRows.length) {
      ...
      if (empError || salError || advError || payError || previousDisplayPayError || dsmfError || prevSalError || prevPayError) {
        ...
        if (snap?.employees?.length) {
          ...
          const cached = getCachedSalarySnapshot()
          ...
          empRows = cached.employees || []
          salRows = (cached.salary_periods || []).filter(r => r.salary_month === monthDate)
          advRows = (cached.salary_advances || []).filter(r => String(r.advance_date || '') >= monthDate && String(r.advance_date || '') <= monthEnd && !r.is_cancelled)
          payRows = (cached.salary_payments || []).filter(r => r.salary_month === monthDate && !r.is_cancelled)
          previousDisplayPayRows = (cached.salary_payments || []).filter(r => r.salary_month === previousMonthForDisplay && !r.is_cancelled)
          dsmfExpenseRows = (cached.dsmf_expenses || []).filter(r => String(r.expense_date || '') >= monthDate && String(r.expense_date || '') <= monthEnd && !r.deleted_at)
          prevSalRows = (cached.salary_periods || []).filter(r => String(r.salary_month || '') < monthDate)
          prevPayRows = (cached.salary_payments || []).filter(r => String(r.salary_month || '') < monthDate && !r.is_cancelled)
          sourceLabel = 'cache'
          setMessage(`Данные сотрудников показаны из локального RMS-кэша. Обновлено: ${cached.updated_at ? formatDT(cached.updated_at) : '—'}`)
        }
      } else {
        cacheSalarySnapshot({
          employees: empRows,
          salary_periods: [...salRows, ...prevSalRows],
          salary_advances: advRows,
          salary_payments: [...payRows, ...previousDisplayPayRows, ...prevPayRows],
          dsmf_expenses: dsmfExpenseRows
        })
        sourceLabel = 'direct'
      }
    }

    if (sourceLabel === 'rpc') setMessage('Зарплаты и сотрудники загружены через защищённый RMS workspace.')
    ...
    const mappedRows = (empRows || []).filter(e => e.is_active !== false).filter(e => matchesStaffGroup(e, branchId)).filter(e => matchesPositionGroup(e, positionFilter)).map(e => {
...
function Settings({ session, t, theme, setTheme }) {
  ...
  async function load() {
    const [{ data: u }, { data: le }, { data: perms }, { data: br }, { data: cats }, { data: employeeCacheRows }] = await Promise.all([
      ...
      supabase.from('employees').select('*, branches(name)').order('branch_id').order('position').order('full_name')
    ])
    if (employeeCacheRows?.length) {
      const oldSalaryCache = getCachedSalarySnapshot() || {}
      cacheSalarySnapshot({ ...oldSalaryCache, employees: employeeCacheRows })
      const oldAttendanceCache = getCachedAttendanceSnapshot() || {}
      cacheAttendanceSnapshot({ ...oldAttendanceCache, employees: employeeCacheRows })
    }
    ...
    const allUsers = [...(u || []), ...internalUsers.filter(iu => !(u || []).some(su => su.id === iu.id || normalizeInternalLogin(su.login_name || su.email) === iu.login_name))]
    setUsers(allUsers)
    setLegalEntities(le || [])
  ...
  async function addUser() {
    ...
    internalUsers[login] = {
      ...
      hide_manager_salary: Boolean(existing?.hide_manager_salary),
      ui_theme: theme || 'classic'
    }
  ...
  async function changeUserPassword(userId, loginName) {
    ...
    const internalUsers = getInternalUsers()
    const localLogin = internalUsers[login] ? login : Object.keys(internalUsers).find(k => internalUsers[k]?.id === userId)
    if (localLogin) {
  ...
  async function updateUser(id, patch) {
    const internalUsers = getInternalUsers()
    const login = Object.keys(internalUsers).find(k => internalUsers[k]?.id === id)
    if (login) {
      internalUsers[login] = { ...internalUsers[login], ...patch }
      setInternalUsers(internalUsers)
      setMsg(t('saved'))
      await load()
      return
  ...
  async function deleteUser(user) {
    ...
    if (error) return setMsg(error.message)
    setMsg(`Пользователь ${login} удалён`)
    await load()
  }
