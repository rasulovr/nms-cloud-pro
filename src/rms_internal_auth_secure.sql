...
-- Tenant-scoped. No supplier purchase, purchase item, payment, balance or revenue row is modified.
      ...
      coalesce((
        ...
        from (
          ...
          left join public.supplier_products sp
            on sp.id = i.product_id
           and sp.organization_id = v_account.organization_id
          where i.purchase_id = p.id
            and i.organization_id = v_account.organization_id
        ) item_data
      ), '[]'::jsonb) as supplier_purchase_items
    from public.supplier_purchases p
    left join public.suppliers s
      on s.id = p.supplier_id
     and s.organization_id = v_account.organization_id
    left join public.legal_entities le
      on le.id = p.legal_entity_id
     and le.organization_id = v_account.organization_id
    left join public.branches b
      on b.id = p.branch_id
     and b.organization_id = v_account.organization_id
    where p.organization_id = v_account.organization_id
