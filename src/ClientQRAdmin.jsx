...
export default function ClientQRAdmin({ lang = localStorage.getItem('rms_lang') || localStorage.getItem('nms_lang') || 'ru', session = null, profile = null, access = 'read' }) {
  const organizationId = profile?.organization_id || session?.user?.user_metadata?.organization_id || ''
  ...
  return (
    <section className="qr-admin client-qr-admin" data-module="qr_menu" data-access={access} data-organization-id={organizationId}>
      ...
      <div className="settings-tabs qr-admin-tabs" aria-label="QR Admin sections">
        <div className="client-qr-admin-nav-head">
          <span>QR ADMIN</span>
          <small>Отдельный модуль RMS Pro</small>
        </div>
