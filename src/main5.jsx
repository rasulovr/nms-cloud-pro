    ...
    <section className="grid">
      ...
      <div className="card span-2">
        ...
        <div className="table-wrap"><table style={{ tableLayout: 'auto' }}>
          <thead><tr><th>Филиал</th><th>Должность</th><th style={{minWidth:220}}>Сотрудник</th><th style={{minWidth:130}}>Статус сотрудника</th><th style={{width:54}}>Инфо</th><th style={{minWidth:130}}>Расчёт</th><th style={{minWidth:110}}>Мес. ставка</th><th style={{minWidth:100}}>Дневная</th><th style={{minWidth:90}}>Дни</th><th style={{minWidth:120}}>Статус</th><th>Долг нач.</th><th>Начислено</th><th>Авансы</th><th>Удержание</th><th>Остаток месяца</th><th>Выплачено</th><th>Баланс</th><th>Комментарий</th></tr></thead>
          <tbody>{salaryGroups.map(group => {
            ...
            return <React.Fragment key={`group-${group.id}`}>
              <tr className="system-row"><td colSpan="18"><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12}}>
              ...
              {groupRows.map(r => {
                ...
                return <React.Fragment key={r.id || r.employee_id}>
                  <tr style={underworked ? { background: 'rgba(155,45,45,.08)' } : undefined}>
                    ...
                    <td>{r.employees?.employment_status === 'terminated' ? `Уволен ${r.employees?.terminated_at || ''}` : `Активный ${r.employees?.hired_at || ''}`}</td>
                    ...
                    <td><input style={{minWidth:90}} inputMode="decimal" defaultValue={r.employees?.monthly_salary || 0} onBlur={e => updateEmployeeDetails(r, { monthly_salary: e.target.value }, true)} /></td>
                    <td><input style={{minWidth:90}} inputMode="decimal" key={(r.employees?.salary_type || 'monthly') + '-' + (r.employees?.daily_rate || '') + '-' + (r.employees?.monthly_salary || '')} defaultValue={fmt(calcDailyRate(r.employees))} readOnly={r.employees?.salary_type !== 'daily'} onBlur={e => r.employees?.salary_type === 'daily' && updateEmployeeDetails(r, { daily_rate: e.target.value }, true)} /></td>
                    ...
                    <td>{underworked ? <span className="bad" title={`Отработано ${fmt(r.worked_days)} из ${DAILY_DIVISOR}`}>Недоработка</span> : <span className="hint">—</span>}</td>
                    <td><strong className={parseNum(r.opening_balance) < 0 ? 'bad' : ''}>{fmt(r.opening_balance)}</strong></td>
                    <td><input style={{minWidth:90}} defaultValue={r.salary_gross} onBlur={e => updateSalary(r, { salary_gross: e.target.value })} /></td>
                    <td><strong>{fmt(r.advance_amount)}</strong></td>
                    <td><input style={{minWidth:90}} defaultValue={r.deduction_amount} onBlur={e => updateSalary(r, { deduction_amount: e.target.value })} /></td>
                    ...
                    <td><input style={{minWidth:180}} defaultValue={r.comment || ''} onBlur={e => updateSalary(r, { comment: e.target.value })} /></td>
                  </tr>
                  {employeeInfoId === r.employee_id && <tr><td colSpan="18"><div className="notice">
          ...
          })}{!visibleSalaryRowsCount && <tr><td colSpan="18" className="hint">Нет активных сотрудников.</td></tr>}</tbody>
        </table></div>
      </div>
