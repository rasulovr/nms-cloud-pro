...
function Salaries({ t }) {
  ...
  const canEditPayrollHistory = !salaryPrivacyProfile?.role || ['admin', 'manager', 'accountant'].includes(String(salaryPrivacyProfile?.role || '').toLowerCase())
  ...
  async function load() {
    ...
    const allSalQ = supabase.from('salary_periods').select('*').order('salary_month', { ascending: false })
    const allAdvQ = supabase.from('salary_advances').select('*').or('is_cancelled.is.null,is_cancelled.eq.false').order('advance_date', { ascending: false })
    const allPayQ = supabase.from('salary_payments').select('*').or('is_cancelled.is.null,is_cancelled.eq.false').order('payment_date', { ascending: false })
    ...
    const [
      ...
      { data: allSal, error: allSalError },
      { data: allAdv, error: allAdvError },
      { data: allPays, error: allPaysError }
    ] = await Promise.all([empQ, salQ, advQ, payQ, previousPayDisplayQ, dsmfQ, prevSalQ, prevPayQ, allSalQ, allAdvQ, allPayQ])
    if (empError || salError || advError || payError || previousDisplayPayError || dsmfError || prevSalError || prevPayError || allSalError || allAdvError || allPaysError) {
      setMessage(empError?.message || salError?.message || advError?.message || payError?.message || previousDisplayPayError?.message || dsmfError?.message || prevSalError?.message || prevPayError?.message || allSalError?.message || allAdvError?.message || allPaysError?.message)
      return
    }
    ...
    const ensureHistoryRow = (employeeId, salaryMonth) => {
      if (!employeeId || !salaryMonth) return null
      const monthKey = String(salaryMonth).slice(0, 7) + '-01'
      if (!historyByEmployee[employeeId]) historyByEmployee[employeeId] = {}
      if (!historyByEmployee[employeeId][monthKey]) {
      ...
      return historyByEmployee[employeeId][monthKey]
    }

    ;(allSal || []).forEach(r => {
      const h = ensureHistoryRow(r.employee_id, r.salary_month)
      if (!h) return
      h.worked_days = parseNum(r.worked_days)
      h.salary_gross = parseNum(r.salary_gross)
      h.advance_amount = parseNum(r.advance_amount)
      h.deduction_amount = parseNum(r.deduction_amount)
      h.salary_net = parseNum(r.salary_net) || (h.salary_gross - h.advance_amount - h.deduction_amount)
    })
    ;(allAdv || []).forEach(a => {
      const h = ensureHistoryRow(a.employee_id, a.advance_date)
      if (!h) return
      h.advance_amount += parseNum(a.amount)
      h.salary_net = h.salary_gross - h.advance_amount - h.deduction_amount
    })
    ;(allPays || []).forEach(p => {
      const h = ensureHistoryRow(p.employee_id, p.salary_month || p.payment_date)
      if (!h) return
      const amount = parseNum(p.amount)
      if (p.method === 'cash') h.paid_cash += amount
      else h.paid_bank += amount
      h.paid_total += amount
      h.payments.push(p)
    })
    Object.values(historyByEmployee).forEach(months => {
      Object.values(months).forEach(h => {
        h.balance = h.salary_gross - h.advance_amount - h.deduction_amount - h.paid_total
      })
    })
    const normalizedHistory = {}
    Object.entries(historyByEmployee).forEach(([employeeId, months]) => {
      normalizedHistory[employeeId] = Object.values(months).sort((a, b) => String(b.salary_month).localeCompare(String(a.salary_month)))
    })

    setRows(mappedRows)
    setEmployeePayrollHistory(normalizedHistory)
    setSalaryPayments(allPays || [...(pays || []), ...(previousDisplayPays || [])])
    setDsmfRows(dsmf || [])
  ...
  function monthLabelShort(dateValue) {
    const parts = String(dateValue || '').slice(0, 7).split('-')
    const y = parts[0]
    const m = Number(parts[1])
    return `${I18N.ru.months[m - 1] || m} ${y}`
  }

  async function updatePayrollHistoryMonth(row, historyRow, patch) {
    if (!canEditPayrollHistory) return setMessage('Нет прав на редактирование истории выплат')
    const payload = {
      employee_id: row.employee_id,
      branch_id: row.branch_id || null,
      salary_month: historyRow.salary_month,
      worked_days: 'worked_days' in patch ? parseNum(patch.worked_days) : parseNum(historyRow.worked_days),
      salary_gross: 'salary_gross' in patch ? parseNum(patch.salary_gross) : parseNum(historyRow.salary_gross),
      advance_amount: 'advance_amount' in patch ? parseNum(patch.advance_amount) : parseNum(historyRow.advance_amount),
      deduction_amount: 'deduction_amount' in patch ? parseNum(patch.deduction_amount) : parseNum(historyRow.deduction_amount),
      salary_net: 0,
      comment: 'Редактирование из карточки сотрудника'
    }
    payload.salary_net = payload.salary_gross - payload.advance_amount - payload.deduction_amount
    const { error } = await supabase.from('salary_periods').upsert(payload, { onConflict: 'employee_id,salary_month' })
    if (error) return setMessage(error.message)
    await load()
    setMessage('История выплат обновлена')
  }

  async function updatePayrollPayment(payment, patch) {
    if (!canEditPayrollHistory) return setMessage('Нет прав на редактирование выплат')
    const payload = { ...patch }
    if ('amount' in payload) payload.amount = parseNum(payload.amount)
    const { error } = await supabase.from('salary_payments').update(payload).eq('id', payment.id)
    if (error) return setMessage(error.message)
    await load()
    setMessage('Выплата обновлена')
  }
                  ...
                  {groupRows.map(r => {
                    ...
                    return <React.Fragment key={r.id || r.employee_id}>
                      ...
                      {isInfoOpen && <tr><td colSpan="13"><div className="notice">
                        ...
                        <h3>История выплат по месяцам</h3>
                        <p className="hint">{canEditPayrollHistory ? 'Можно редактировать начисления, авансы, удержания и отдельные выплаты.' : 'Просмотр без редактирования: нет разрешения на изменение выплат.'}</p>
                        <div className="table-wrap"><table>
                          <thead><tr><th>Месяц</th><th>Дни</th><th>Начислено</th><th>Авансы</th><th>Удержание</th><th>Выплачено</th><th>Остаток</th></tr></thead>
                          <tbody>
                            {(employeePayrollHistory[r.employee_id] || []).map(h => <React.Fragment key={`payhist-${r.employee_id}-${h.salary_month}`}>
                              <tr>
                                <td><b>{monthLabelShort(h.salary_month)}</b></td>
                                <td>{canEditPayrollHistory ? <input style={{width:70}} inputMode="decimal" defaultValue={fmt(h.worked_days)} onBlur={e => updatePayrollHistoryMonth(r, h, { worked_days: e.target.value })} /> : fmt(h.worked_days)}</td>
                                <td>{canEditPayrollHistory ? <input style={{width:90}} inputMode="decimal" defaultValue={fmt(h.salary_gross)} onBlur={e => updatePayrollHistoryMonth(r, h, { salary_gross: e.target.value })} /> : fmt(h.salary_gross)}</td>
                                <td>{canEditPayrollHistory ? <input style={{width:90}} inputMode="decimal" defaultValue={fmt(h.advance_amount)} onBlur={e => updatePayrollHistoryMonth(r, h, { advance_amount: e.target.value })} /> : fmt(h.advance_amount)}</td>
                                <td>{canEditPayrollHistory ? <input style={{width:90}} inputMode="decimal" defaultValue={fmt(h.deduction_amount)} onBlur={e => updatePayrollHistoryMonth(r, h, { deduction_amount: e.target.value })} /> : fmt(h.deduction_amount)}</td>
                                <td><b>{fmt(h.paid_total)}</b></td>
                                <td className={h.balance > 0 ? 'bad' : h.balance < 0 ? 'good' : ''}><b>{fmt(h.balance)}</b></td>
                              </tr>
                              <tr>
                                <td></td>
                                <td colSpan="6">
                                  <div className="table-wrap"><table>
                                    <thead><tr><th>Дата выплаты</th><th>Способ</th><th>Сумма</th><th>Комментарий</th></tr></thead>
                                    <tbody>
                                      {h.payments.map(p => <tr key={p.id}>
                                        <td>{canEditPayrollHistory ? <input type="date" defaultValue={p.payment_date || ''} onBlur={e => updatePayrollPayment(p, { payment_date: e.target.value })} /> : (p.payment_date || '—')}</td>
                                        <td>{canEditPayrollHistory ? <select defaultValue={p.method || 'cash'} onChange={e => updatePayrollPayment(p, { method: e.target.value })}><option value="cash">Наличные</option><option value="bank">Банк</option></select> : (p.method === 'bank' ? 'Банк' : 'Наличные')}</td>
                                        <td>{canEditPayrollHistory ? <input style={{width:90}} inputMode="decimal" defaultValue={fmt(p.amount)} onBlur={e => updatePayrollPayment(p, { amount: e.target.value })} /> : fmt(p.amount)}</td>
                                        <td>{canEditPayrollHistory ? <input defaultValue={p.comment || ''} onBlur={e => updatePayrollPayment(p, { comment: e.target.value })} /> : (p.comment || '—')}</td>
                                      </tr>)}
                                      {!h.payments.length && <tr><td colSpan="4" className="hint">Выплат за этот месяц нет.</td></tr>}
                                    </tbody>
                                  </table></div>
                                </td>
                              </tr>
                            </React.Fragment>)}
                            {!(employeePayrollHistory[r.employee_id] || []).length && <tr><td colSpan="7" className="hint">История выплат пока пустая.</td></tr>}
                          </tbody>
                        </table></div>
