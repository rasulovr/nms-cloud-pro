...
function Revenue({ t }) {
  ...
  const [cashForm, setCashForm] = useState({ opening_cash: '', closing_cash: '', comment: '' })
  const [serviceForm, setServiceForm] = useState({ enabled: false, service_percent: '10', staff_cost_percent: '4' })
  const [serviceRow, setServiceRow] = useState(null)
  ...
  async function load() {
    ...
    setCashForm({ opening_cash: cash?.opening_cash ?? '', closing_cash: cash?.closing_cash ?? '', comment: cash?.comment ?? '' })
    const { data: svc } = await supabase.from('daily_service_charge').select('*').eq('branch_id', branchId).eq('charge_date', date).maybeSingle()
    setServiceRow(svc || null)
    setServiceForm({ enabled: Boolean(svc?.enabled), service_percent: svc?.service_percent ?? '10', staff_cost_percent: svc?.staff_cost_percent ?? '4' })
  ...
  async function saveCashRegister() {
    if (!branchId) return
    const user = await currentUserMeta()
    const payload = { branch_id: branchId, cash_date: date, opening_cash: parseNum(cashForm.opening_cash), closing_cash: parseNum(cashForm.closing_cash), comment: cashForm.comment || null, updated_by: user.user_id, created_by: cashRow?.created_by || user.user_id }
    const { data, error } = await supabase.from('daily_cash_register').upsert(payload, { onConflict: 'branch_id,cash_date' }).select('*').single()
    if (error) return setMessage(error.message)
    await writeLog({ entity_type: 'cash', record_id: data.id, action: cashRow ? 'update' : 'create', field_name: 'cash_register', old_value: cashRow ? `${cashRow.opening_cash}/${cashRow.closing_cash}` : null, new_value: `${data.opening_cash}/${data.closing_cash}` })
    setCashRow(data)
  ...
  async function saveServiceCharge() {
    ...
    const payload = {
      branch_id: branchId,
      charge_date: date,
      enabled: Boolean(serviceForm.enabled),
      service_percent: parseNum(serviceForm.service_percent),
      staff_cost_percent: parseNum(serviceForm.staff_cost_percent),
      created_by: serviceRow?.created_by || user.user_id,
      updated_by: user.user_id
    }
    const { data, error } = await supabase.from('daily_service_charge').upsert(payload, { onConflict: 'branch_id,charge_date' }).select('*').single()
    if (error) return setMessage(error.message)
    await writeLog({ entity_type: 'service_charge', record_id: data.id, action: serviceRow ? 'update' : 'create', field_name: 'service_charge', old_value: serviceRow ? `${serviceRow.enabled}/${serviceRow.service_percent}/${serviceRow.staff_cost_percent}` : null, new_value: `${data.enabled}/${data.service_percent}/${data.staff_cost_percent}` })
    setServiceRow(data)
    await Promise.all([loadMonthStats(branchId, date), loadLogs(branchId, date)])
    setMessage('Service charge сохранён и учтён в расходах персонала')
  }
  ...
  const dailyServiceChargeAmount = serviceForm.enabled && servicePercent > 0 ? dailyRevenueTotal * servicePercent / (100 + servicePercent) : 0
  const dailyServiceStaffCost = dailyServiceChargeAmount * staffCostPercent / 100
  const expectedClosingCash = parseNum(cashForm.opening_cash) + dailyCashRevenue + dailyInflowTotal - dailyExpenseTotal
  const cashDifference = parseNum(cashForm.closing_cash) - expectedClosingCash
    ...
    <section id="revenuePage">
      ...
      <section className="grid">
        ...
        <div className="card span-2">
          <div className="card-head"><div><h3>Service charge</h3><p className="hint">Включай только для филиалов, где есть сервисный сбор. Приходы не входят в выручку; service charge уже внутри выручки, а расход персонала считается отдельно.</p></div><button className="small primary" onClick={saveServiceCharge}>Сохранить</button></div>
          <div className="form-grid">
            <label className="checkbox-row"><input type="checkbox" checked={serviceForm.enabled} onChange={e => setServiceForm(f => ({ ...f, enabled: e.target.checked }))} /> Учитывать service charge</label>
            <MoneyInput label="Service charge % от счёта" value={serviceForm.service_percent} onChange={v => setServiceForm(f => ({ ...f, service_percent: v }))} />
            <MoneyInput label="% затрат персоналу от service charge" value={serviceForm.staff_cost_percent} onChange={v => setServiceForm(f => ({ ...f, staff_cost_percent: v }))} />
            <label><span>Сумма service charge внутри выручки</span><input value={fmt(dailyServiceChargeAmount)} readOnly /></label>
            <label><span>Расход персоналу</span><input value={fmt(dailyServiceStaffCost)} readOnly /></label>
          </div>
        </div>

        <div className="card span-2">
          <div className="card-head"><div><h3>{t('daily_expenses_title')}</h3><p className="hint">Изменения суммы, статьи и комментария фиксируются ниже. Отмена перечёркивает строку и исключает её из расчётов.</p></div><button className="small" onClick={addExpense}>{t('add_expense')}</button></div>
        ...
        <div className="card span-2">
          <div className="card-head"><div><h3>Приходы за выбранную дату</h3><p className="hint">Деньги извне: возврат подотчёта, пополнение кассы, личное внесение, прочие наличные поступления. Отмена перечёркивает строку и исключает её из кассы.</p></div><button className="small primary" onClick={addInflow}>+ Приход</button></div>
          <div className="form-grid">
        ...
        <div className="card span-2"><h3>Касса за день</h3><p className="hint">В расчёте кассы учитываются только наличные приходы и оплаченные наличные расходы. Wolt, банк и неоплаченные фактуры поставщиков не учитываются.</p>
          <div className="form-grid">
            <MoneyInput label="Касса на начало дня" value={cashForm.opening_cash} onChange={v => setCashForm(f => ({ ...f, opening_cash: v }))} />
            <label><span>Ожидаемая касса</span><input value={fmt(expectedClosingCash)} readOnly /></label>
            <MoneyInput label="Касса на конец дня" value={cashForm.closing_cash} onChange={v => setCashForm(f => ({ ...f, closing_cash: v }))} />
            <label><span>Разница</span><input className={Math.abs(cashDifference) > 0.009 ? 'bad' : 'good'} value={fmt(cashDifference)} readOnly /></label>
...
function Dashboard({ t }) {
  ...
  async function calcMonth(y, m) {
    ...
    const [{ data: revRows }, { data: expRows }, { data: salRows }, { data: svcRows }, supplierResult] = await Promise.all([
    ...
    if (!supplierRows.length || supplierResult?.error) {
      ...
      supplierRows = (fallback.data || []).map(r => ({ supplier_id: r.supplier_id, supplier_name: r.supplier_name, balance: r.balance }))
    }
    if (!supplierRows.length) {
      const [{ data: suppliersRaw }, { data: purchasesRaw }, { data: paymentsRaw }] = await Promise.all([
        supabase.from('suppliers').select('id,name').eq('is_active', true),
        supabase.from('supplier_purchases').select('supplier_id,total_amount').is('deleted_at', null),
        supabase.from('supplier_payments').select('supplier_id,amount')
      ])
      supplierRows = (suppliersRaw || []).map(s => {
        const purchases = (purchasesRaw || []).filter(p => p.supplier_id === s.id).reduce((sum, p) => sum + parseNum(p.total_amount), 0)
        const payments = (paymentsRaw || []).filter(p => p.supplier_id === s.id).reduce((sum, p) => sum + parseNum(p.amount), 0)
        return { supplier_id: s.id, supplier_name: s.name, balance: purchases - payments }
      })
    }
    ...
    const supplierDebtRows = (supplierRows || [])
      .map(r => ({ name: r.supplier_name || r.name || 'Поставщик', value: Math.max(0, parseNum(r.balance)) }))
      .filter(r => r.value > 0)
  ...
  const debtRows = data.supplierDebtRows?.length ? data.supplierDebtRows : [{ name: 'Нет долгов', value: 0 }]
...
function Attendance({ t }) {
  ...
  async function load() {
    ...
    setEmployees((emp || []).filter(e => matchesStaffGroup(e, branchId)).filter(e => matchesPositionGroup(e, positionFilter)))
    setAttendance(att || [])
  }
...
function Salaries({ t }) {
  ...
  }).filter(b => branchId === 'all' ? (b.gross || b.advances || b.net || b.employees) : b.id === branchId)
    ...
    <section className="grid">
      ...
      <div className="card span-2">
        ...
        <div className="table-wrap"><table>
          <thead><tr><th>Филиал / группа</th><th>Сотрудников</th><th>Начислено</th><th>Авансы</th><th>Удержания</th><th>Остаток</th></tr></thead>
          <tbody>{branchTotals.map(b => <tr key={b.id}><td>{b.name}</td><td><strong>{b.employees}</strong></td><td>{fmt(b.gross)}</td><td>{fmt(b.advances)}</td><td>{fmt(b.deductions)}</td><td><strong>{fmt(b.net)}</strong></td></tr>)}{!branchTotals.length && <tr><td colSpan="6" className="hint">Нет данных за выбранный период.</td></tr>}</tbody>
        </table></div>
      </div>
