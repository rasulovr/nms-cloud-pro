...
function getRmsLocalUser() {
  try {
    const keys = ['rms_current_user', 'rms_internal_user', 'rms_local_user', 'rms_session_user', 'rms_logged_user']
...
function Attendance({ t, mode = 'attendance' }) {
  ...
  const [salaryPrivacyProfile, setSalaryPrivacyProfile] = useState(() => getRmsLocalUser())
  ...
  async function loadSalaryPrivacyProfile() {
    ...
    if (localUser) {
      setSalaryPrivacyProfile(localUser)
      return
    }
  ...
  const hideManagerAttendance = Boolean(salaryPrivacyProfile?.hide_manager_salary)
  const attendanceVisibleEmployees = hideManagerAttendance ? employees.filter(emp => !isManagerStaff(emp)) : employees
  const attendanceGroups = staffGroupOptions(branches)
    .map(group => ({
      ...group,
      rows: attendanceVisibleEmployees.filter(emp => employeeGroupId(emp) === group.id)
    }))
    .filter(group => branchId === 'all' ? group.rows.length : group.id === branchId)
...
function Salaries({ t, view = 'employees' }) {
  ...
  async function load() {
    ...
    setDsmfRows(dsmf || [])
    const visibleMappedRows = shouldHideManagerSalaries ? mappedRows.filter(r => !isManagerRow(r)) : mappedRows
