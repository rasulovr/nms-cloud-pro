...
function Suppliers({ t }) {
  ...
  function selectProductForLine(index, value) {
    const selected = products.find(p => p.id === value) || products.find(p => productLabel(p) === value) || products.find(p => p.name === value)
    if (selected) {
      updateLine(index, { product_id: selected.id, product_query: productLabel(selected), category: selected.category || 'Прочее' })
    } else {
      updateLine(index, { product_id: '', product_query: value })
    }
  }
  ...
  async function ensureProduct(row) {
    ...
    const exact = products.find(p => productLabel(p) === typed) || products.find(p => p.name === typed)
    ...
    <section>
      ...
      <section className="grid">
        ...
        <div className="card span-2">
          ...
          <br />
          <div className="card-head"><div><h3>Товары в закупке</h3><p className="hint">В графе “Товар” выберите существующий товар через поиск. Если товара нет, сначала добавьте его в блоке “Товары”.</p></div><button className="small" onClick={() => setLineRows(rows => [...rows, { ...emptyLine }])}>+ Строка товара</button></div>
          <div className="table-wrap">
            <table>
                    ...
                    <tr key={idx}>
                      ...
                      <td style={{minWidth:260}}>
                        <input
                          ...
                          value={row.product_query || (products.find(p => p.id === row.product_id) ? productLabel(products.find(p => p.id === row.product_id)) : '')}
