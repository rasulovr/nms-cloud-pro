...
function Suppliers({ t }) {
  ...
  function updateLine(index, patch) {
    setLineRows(rows => rows.map((row, i) => {
      ...
      if (Object.prototype.hasOwnProperty.call(patch, 'category') && !Object.prototype.hasOwnProperty.call(patch, 'product_id')) {
        next.product_id = ''
      }
      return next
    }))
  ...
  function selectProductForLine(index, productId) {
    ...
    updateLine(index, {
      ...
      category: selected?.category || lineRows[index]?.category || PRODUCT_CATEGORIES[0]
    })
  }
    ...
    <section>
      ...
      <section className="grid">
        ...
        <div className="card span-2">
          ...
          <br />
          <div className="card-head"><div><h3>Товары в закупке</h3><p className="hint">В графе “Товар” выберите существующий товар из списка. Для быстрого поиска откройте список и начните печатать название товара. Если товара нет, сначала добавьте его в блоке “Товары”.</p></div><button className="small" onClick={() => setLineRows(rows => [...rows, { ...emptyLine }])}>+ Строка товара</button></div>
          <div className="table-wrap">
            <table>
                  ...
                  return (
                    <tr key={idx}>
                      ...
                      <td style={{minWidth:300}}>
                        <select value={row.product_id || ''} onChange={e => selectProductForLine(idx, e.target.value)}>
                          <option value="">Выберите товар...</option>
                          {productOptionsForRow(row).map(p => (
