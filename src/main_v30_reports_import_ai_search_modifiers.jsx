...
function Reports({ t }) {
  ...
  const departmentTotals = useMemo(() => {
    const base = { Бар: { quantity: 0, revenue: 0, cost: 0, profit: 0 }, Кухня: { quantity: 0, revenue: 0, cost: 0, profit: 0 }, Смешанный: { quantity: 0, revenue: 0, cost: 0, profit: 0 } }
    rows.forEach(r => {
      const dept = base[r.department] ? r.department : 'Смешанный'
  ...
  async function importParsedReports(parsedReports) {
    const cleaned = parsedReports.map(r => {
      const rows = (r.rows || []).filter(row => !isAikoModifierRow(row.name, row.source_category))
  ...
  async function syncMenuItemsFromSalesRows(salesRows) {
    ...
    ;(salesRows || []).forEach(row => {
      if (isAikoModifierRow(row.name, row.source_category)) return
  ...
  const renderAiTable = (title, list, sortLabel = 'Выручка') => {
    ...
    return <div className="card span-2" style={{overflow:'hidden'}}>
      <div className="card-head"><div><h3>{title}</h3><p className="hint">Компактный TOP-блок без внутреннего скролла. На экране максимум {limit} позиций.</p></div>{(title.includes('Топ продаж') || title.includes('аутсайдер')) && <label className="compact-select"><span className="hint">Показать TOP</span><select value={title.includes('Топ продаж') ? aiTopLimit : aiOutsiderLimit} onChange={e => title.includes('Топ продаж') ? setAiTopLimit(Number(e.target.value)) : setAiOutsiderLimit(Number(e.target.value))}>{limitOptions.map(n => <option key={n} value={n}>{n}</option>)}</select></label>}</div>
      <table style={{width:'100%', tableLayout:'fixed'}}><thead><tr><th>Позиция</th><th style={{width:90}}>Тип</th><th style={{width:80}}>Кол-во</th><th style={{width:110}}>{sortLabel}</th><th style={{width:90}}>Food cost</th><th style={{width:90}}>Маржа</th></tr></thead><tbody>
        {visible.map((r, idx) => <tr key={`${title}-${idx}-${r.name}`}><td><b>{r.name}</b><br /><span className="hint">{r.branch_name || '—'}</span></td><td><span className={r.department === 'Бар' ? 'pill good' : r.department === 'Кухня' ? 'pill warn' : 'pill'}>{r.department || '—'}</span></td><td>{fmt(r.quantity)}</td><td>{fmt(sortLabel === 'Маржа %' ? r.margin : r.revenue)}</td><td>{r.revenue ? pct((r.cost / r.revenue) * 100) : '0.0%'}</td><td className={r.margin < 20 ? 'bad' : r.margin > 65 ? 'good' : ''}><b>{pct(r.margin)}</b></td></tr>)}
        {!visible.length && <tr><td colSpan="6" className="hint">Нет данных</td></tr>}
      </tbody></table>
      {list.length > limit && <button className="ghost small" style={{marginTop:10}} onClick={() => setExpandedAiTables(v => ({...v, [title]: !expanded}))}>{expanded ? 'Свернуть' : `Показать все (${list.length})`}</button>}
    </div>
  }
  ...
  const aiSearchResult = useMemo(() => {
    ...
    let resultRows = aggregateSalesRows(baseReports)
  ...
  }, [salesAiQuery, reports, branchFilter, departmentFilter, monthFilter])

  const SalesTableBlock = <div className="card span-2">
    <div className="card-head"><div><h3>Общая таблица продаж</h3><p className="hint">Фильтр, сводка и таблица продаж собраны в одном блоке. Агрегировано по позиции, филиалу и типу отчёта.</p></div></div>
    <div className="form-grid compact">
      ...
      <label><span>Тип</span><select value={departmentFilter} onChange={e => { setDepartmentFilter(e.target.value); setExpandedSalesRows(false) }}><option value="all">Бар + Кухня</option><option value="Бар">Бар</option><option value="Кухня">Кухня</option><option value="Смешанный">Смешанный</option></select></label>
    </div>
    <div className="mini-grid">
    ...
    <div className="table-wrap" style={{marginTop:12}}><table><thead><tr><th>Раздел</th><th>Кол-во</th><th>Выручка</th><th>Себестоимость</th><th>Прибыль</th><th>Food cost</th></tr></thead><tbody>{['Бар', 'Кухня', 'Смешанный'].map(renderDepartmentSummaryRow)}</tbody></table></div>
    <div className="table-wrap" style={{marginTop:12}}><table><thead><tr><th>Позиция</th><th>Филиал</th><th>Тип</th><th>Категория</th><th>{sortHeader('quantity', 'Кол-во')}</th><th>{sortHeader('revenue', 'Выручка')}</th><th>{sortHeader('cost', 'Себестоимость')}</th><th>{sortHeader('profit', 'Прибыль')}</th><th>{sortHeader('margin', 'Маржа')}</th></tr></thead><tbody>
      {visibleSalesRows.map((r, idx) => <tr key={`${r.name}-${idx}`}><td><b>{r.name}</b></td><td>{r.branch_name || '—'}</td><td><span className={r.department === 'Бар' ? 'pill good' : r.department === 'Кухня' ? 'pill warn' : 'pill'}>{r.department || '—'}</span></td><td>{r.menu_category || r.source_category || '—'}</td><td>{fmt(r.quantity)}</td><td>{fmt(r.revenue)}</td><td>{fmt(r.cost)}</td><td className={r.profit >= 0 ? 'good' : 'bad'}>{fmt(r.profit)}</td><td>{pct(r.margin)}</td></tr>)}
  ...
  const AiSearchBlock = <div className="card span-2">
    ...
    {aiSearchResult && <>
      ...
      <div className="table-wrap" style={{marginTop:12}}><table><thead><tr><th>Позиция</th><th>Филиал</th><th>Тип</th><th>Кол-во</th><th>Выручка</th><th>Прибыль</th></tr></thead><tbody>
        {aiSearchResult.rows.slice(0, 20).map((r, idx) => <tr key={`ai-search-${idx}`}><td><b>{r.name}</b></td><td>{r.branch_name || '—'}</td><td>{r.department || '—'}</td><td>{fmt(r.quantity)}</td><td>{fmt(r.revenue)}</td><td className={r.profit >= 0 ? 'good' : 'bad'}>{fmt(r.profit)}</td></tr>)}
        {!aiSearchResult.rows.length && <tr><td colSpan="6" className="hint">По запросу ничего не найдено.</td></tr>}
      </tbody></table></div>
    </>}
  </div>
