...
function parseAikoSalesText(text, meta = {}) {
  ...
  for (const line of lines) {
    ...
    const avgPrice = parseAikoNumber(match[4])
    const cost = parseAikoNumber(match[5])
    rows.push({
...
function autoMergeSalesNameKey(name) {
  ...
  const cleaned = normalizeSalesSearchText(withoutParentheses)
  const tokens = cleaned
    .split(' ')
    .map(canonicalizeSalesToken)
    .filter(t => t && t.length >= 2 && !SALES_AUTO_MERGE_STOP_WORDS.has(t))
  if (!tokens.length) return normalizeSalesKey(name)
...
function autoMergeAiSalesRows(rows, includeBranch = false) {
  ...
  ;(rows || []).forEach(row => {
    ...
    const deptPart = row.department || ''
    const key = `${mergeNameKey}|${branchPart}|${deptPart}`
    const existing = map.get(key) || {
      ...row,
      name: normalizeSalesName(row.name),
    ...
    const currentName = normalizeSalesName(row.name)
    ...
    // Более чистое название показываем как основное: без скобок, короче и без лишних брендов.
    const candidate = normalizeSalesName(String(currentName).replace(/\([^)]*\)/g, ' '))
    const currentScore = String(existing.name || '').length
    const candidateScore = candidate.length || 999
    if (candidate && candidateScore < currentScore) existing.name = candidate
...
function Reports({ t }) {
  ...
  useEffect(() => {
    ...
    const normalizedReports = reports.map(report => {
      const nextRows = (report.rows || []).map(row => {
        const normalized = normalizeAikoSalesRow(row, report.department || '')
        if (normalized.name !== row.name || normalized.source_category !== row.source_category || normalized.menu_category !== row.menu_category) changed = true
        return normalized
      })
      return changed ? { ...report, rows: nextRows } : report
    })
  ...
  const parseAiSalesQuery = (query) => {
    ...
    const stop = new Set(['покажи','продажи','продажу','продаж','за','этот','текущий','месяц','год','и','по','все','всех','товары','товаров','товар','блюда','блюдо','напитки','напиток','сколько','выручка','выручку','количество','кол-во','штук','штуки','this','month','year','sales','show','sold','quantity','revenue'])
    const tokens = q.split(/[^a-zа-яçə0-9]+/i).map(x => normalizeSalesSearchText(x)).filter(x => x.length >= 2 && !stop.has(x) && !/^20\d{2}$/.test(x))
    const searchTokens = expandSalesSearchTokens(tokens)
    return { tokens, searchTokens, mode, targetMonth, targetYear }
  }
  ...
  const aiSearchResult = useMemo(() => {
    ...
    if (parsed.searchTokens.length) {
      resultRows = resultRows.filter(r => {
        const hay = normalizeSalesSearchText(`${r.name || ''} ${(r.original_names || []).join(' ')} ${r.menu_category || ''} ${r.source_category || ''}`)
        return parsed.searchTokens.some(token => hay.includes(token))
      })
    }
  ...
  const AiSearchBlock = <div className="card span-2">
    ...
    {aiSearchResult && <>
      ...
      <div className="table-wrap" style={{marginTop:12}}><table><thead><tr><th>Позиция</th>{!isNetworkSalesView && <th>Филиал</th>}<th>Тип</th><th>Кол-во</th><th>Выручка</th><th>Прибыль</th></tr></thead><tbody>
        {aiSearchResult.rows.slice(0, 20).map((r, idx) => <tr key={`ai-search-${idx}`}><td><b>{r.name}</b>{r.auto_merged && <><br /><span className="hint">автоматически объединено: {r.auto_merged_names.slice(0, 4).join(', ')}{r.auto_merged_names.length > 4 ? '…' : ''}</span></>}</td>{!isNetworkSalesView && <td>{r.branch_name || '—'}</td>}<td>{r.department === 'Кухня' ? 'Кухня' : r.department === 'Бар' ? 'Бар' : '—'}</td><td>{fmt(r.quantity)}</td><td>{fmt(r.revenue)}</td><td className={r.profit >= 0 ? 'good' : 'bad'}>{fmt(r.profit)}</td></tr>)}
        {aiSearchResult.rows.length > 0 && <tr><td colSpan={isNetworkSalesView ? 2 : 3}><b>Итого найдено</b></td><td><b>{fmt(aiSearchResult.totals.quantity)}</b></td><td><b>{fmt(aiSearchResult.totals.revenue)}</b></td><td className={aiSearchResult.totals.profit >= 0 ? 'good' : 'bad'}><b>{fmt(aiSearchResult.totals.profit)}</b></td></tr>}
