...
function Recipes({ t }) {
  ...
  async function loadBase() {
    ...
    if (isInternal) {
      ...
      if (!refError && ref) {
        const refProducts = (ref.supplier_products || []).map(p => ({ ...p, category: normalizeProductType(p.category) }))
        const refMenuItems = ref.menu_items || []
        setProducts(refProducts)
        setMenuItems(refMenuItems)
        setCosts(ref.latest_product_costs || [])
        setAllRecipeItems(ref.recipe_items || [])
        if (!selectedMenuId && refMenuItems?.[0]) setSelectedMenuId(refMenuItems[0].id)
        return
      }
    }
