...
function DebtsPayments({ t }) {
  ...
  return <section>
    ...
    <section className="grid">
      ...
      {activeSupplierId && <div className="card span-2 supplier-transactions-panel"><div className="card-head"><div><h3>Транзакции: {activeSupplier?.name}</h3><p className="hint">{activeLegalEntityId ? `Только по VOEN: ${legalEntities.find(le => le.id === activeLegalEntityId)?.name || '—'}` : 'Поступления и оплаты показаны отдельно, чтобы не смешивать операции.'}</p></div><button className="ghost small" onClick={() => { setActiveSupplierId(''); setActiveLegalEntityId('') }}>Закрыть</button></div>
