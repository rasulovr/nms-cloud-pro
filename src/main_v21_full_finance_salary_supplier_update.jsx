        ...
        <div className="card recipe-sheet" style={{ gridColumn: '1 / -1' }}>
          <div className="card-head recipe-sheet-head">
            ...
            <div className="row-actions">
              {!recipeEditMode && recipeItems.length > 0 && <button className="ghost small" onClick={printTechCard}>Печать</button>}
              {!recipeEditMode && recipeItems.length > 0 && <button className="ghost small" onClick={exportTechCardPdf}>PDF</button>}
...
function Salaries({ t }) {
  ...
  const [dsmfForm, setDsmfForm] = useState({ employee_id: '', amount: '', category: 'DSMF', comment: '' })
  const DAILY_DIVISOR = 26
  ...
  async function addDsmfExpense() {
    setMessage('')
    if (!dsmfForm.employee_id) return setMessage('Выберите сотрудника')
    const emp = rows.find(r => r.employee_id === dsmfForm.employee_id)?.employees
    const row = rows.find(r => r.employee_id === dsmfForm.employee_id)
    const amount = parseNum(dsmfForm.amount)
    if (!amount) return setMessage('Введите сумму DSMF')
    const { error } = await supabase.from('daily_expenses').insert({
      branch_id: row?.branch_id || emp?.branch_id || null,
      expense_date: monthDate,
      amount,
      custom_category: 'DSMF',
      comment: `DSMF · ${emp?.full_name || ''} · ${dsmfForm.category || ''}${dsmfForm.comment ? ' · ' + dsmfForm.comment : ''}`
    })
    if (error) return setMessage(error.message)
    setDsmfForm({ employee_id: '', amount: '', category: 'DSMF', comment: '' })
    setMessage('DSMF расход добавлен')
    load()
  }
  ...
  return <section>
    ...
    <section className="grid">
      ...
      <div className="card span-2">
        <div className="card-head"><div><h3>DSMF / расходы по сотрудникам</h3><p className="hint">Дополнительные статьи расходов по сотруднику для точного расчёта расходов филиала и налогов.</p></div><button className="small primary" onClick={addDsmfExpense}>+ Добавить DSMF</button></div>
        <div className="form-grid compact">
          <label><span>Сотрудник</span><select value={dsmfForm.employee_id} onChange={e => setDsmfForm({...dsmfForm, employee_id: e.target.value})}><option value="">Выберите сотрудника</option>{rows.map(r => <option key={r.employee_id} value={r.employee_id}>{employeeGroupName(r)} · {r.employees?.full_name}</option>)}</select></label>
          <label><span>Статья</span><select value={dsmfForm.category} onChange={e => setDsmfForm({...dsmfForm, category: e.target.value})}><option value="DSMF">DSMF</option><option value="Соц. выплаты">Соц. выплаты</option><option value="Налог">Налог</option><option value="Прочее">Прочее</option></select></label>
          <MoneyInput label="Сумма" value={dsmfForm.amount} onChange={v => setDsmfForm({...dsmfForm, amount: v})} />
          <label><span>Комментарий</span><input value={dsmfForm.comment} onChange={e => setDsmfForm({...dsmfForm, comment: e.target.value})} /></label>
        </div>
        <div className="table-wrap"><table><thead><tr><th>Дата</th><th>Филиал</th><th>Сумма</th><th>Комментарий</th></tr></thead><tbody>{dsmfRows.map(r => <tr key={r.id}><td>{r.expense_date}</td><td>{r.branches?.name || '—'}</td><td><b>{fmt(r.amount)}</b></td><td>{r.comment || '—'}</td></tr>)}{!dsmfRows.length && <tr><td colSpan="4" className="hint">—</td></tr>}</tbody></table></div>
      </div>
  ...
  return <section>
    ...
    <section className="grid">
      ...
      <div className="card span-2 supplier-transactions-panel">
        <div className="card-head"><div><h3>Баланс поставщика</h3><p className="hint">Профессиональная карточка взаиморасчётов: приход/долг, оплата и остаток.</p></div><button className="small primary" onClick={printSupplierLedger}>PDF/Print</button></div>
        <div className="form-grid compact"><label><span>Поставщик</span><select value={ledgerSupplierId} onChange={e => setLedgerSupplierId(e.target.value)}><option value="all">Все поставщики</option>{suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></label></div>
        <div className="table-wrap"><table><thead><tr><th>Дата</th><th>Поставщик</th><th>Операция</th><th>Приход / долг</th><th>Оплата</th><th>Остаток</th></tr></thead><tbody>{supplierLedgerRows.map(r => <tr key={r.id}><td>{r.transaction_date}</td><td>{r.suppliers?.name || '—'}</td><td>{r.comment}</td><td>{fmt(r.debit)}</td><td>{fmt(r.credit)}</td><td className={r.balance > 0 ? 'bad' : 'good'}><b>{fmt(r.balance)}</b></td></tr>)}{!supplierLedgerRows.length && <tr><td colSpan="6" className="hint">—</td></tr>}</tbody></table></div>
      </div>
