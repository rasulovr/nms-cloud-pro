    ...
    <section id="revenuePage">
      ...
      <section className="grid">
        ...
        <div className="card span-2"><h3>Касса за день</h3><p className="hint">Только наличные: касса на начало + наличная выручка + приходы − расходы. Сумма конца дня автоматически переходит на начало следующего дня.</p>
          <div className="form-grid">
            <label><span>Касса на начало дня</span><input value={fmt(cashForm.opening_cash)} readOnly title="Автоматически берётся из кассы конца предыдущего дня" /></label>
...
function RevenueEntryRow({ row, serviceEnabled=false, servicePercent=10, staffCostPercent=4, onSave, onCancel }) {
  ...
  async function saveChanges() {
    if (!editable) return
    await onSave({ cash_amount: parseNum(cash), bank_amount: parseNum(bank), wolt_amount: parseNum(wolt), comment })
    setEditing(false)
  }
  ...
  return <>
    ...
    {editing && <tr><td colSpan="8"><div className="notice">
...
function Finance({ t, lang }) {
  ...
  async function load() {
    ...
    let empQuery = supabase.from('employees').select('id, branch_id, position').eq('is_active', true)
    ...
    const [{ data: rows }, { data: purchaseRows }, { data: employeeRows }] = await Promise.all([expQuery, purQuery, empQuery])
    ...
    if (branchId === ALL_BRANCHES) {
      ...
      setAiRows(combinedRows.length ? combinedRows : [{
        ...
        fact: `Выручка ${fmt(current.revenue)} AZN · расходы ${fmt(current.expenses + current.salary + current.serviceCost + current.tax)} AZN`,
            ...
            <div key={group.id} className="supplier-entity-group">
              ...
              <div className="table-wrap">
                <table style={{ fontSize: 11, tableLayout: 'auto' }}>
                  <thead><tr><th style={{ minWidth: 95 }}>Должность</th><th style={{ minWidth: 230 }}>И.Ф.О.</th><th style={{ minWidth: 80 }}>Мес. ставка</th><th style={{ minWidth: 75 }}>День</th>{days.map(d => <th key={d} style={{ width: 30, minWidth: 30, textAlign: 'center' }}>{d}</th>)}<th style={{ minWidth: 48 }}>Дни</th><th style={{ minWidth: 80 }}>ЗП</th></tr></thead>
                  <tbody>
                    {group.rows.map(emp => {
                      const worked = workedDays(emp.id)
                      const salary = calcSalary(emp, worked)
                    ...
                    {!group.rows.length && <tr><td colSpan={days.length + 6} className="hint">Нет сотрудников в этом филиале.</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
...
function Salaries({ t }) {
  ...
  const [dsmfForm, setDsmfForm] = useState({ employee_id: '', official_salary: '', comment: '' })
  ...
  async function load() {
    ...
    ] = await Promise.all([empQ, salQ, advQ, payQ, dsmfQ, prevSalQ, prevPayQ])
    if (empError || salError || advError || payError || dsmfError || prevSalError || prevPayError) {
      setMessage(empError?.message || salError?.message || advError?.message || payError?.message || dsmfError?.message || prevSalError?.message || prevPayError?.message)
      return
    }
  ...
  function selectedDsmfEmployeeRow() {
    return rows.find(r => r.employee_id === dsmfForm.employee_id) || null
  }

  function dsmfCalcForSalary(salaryValue = dsmfForm.official_salary) {
    const base = parseNum(salaryValue)
  ...
  function saveDsmfRates() {
    ...
    setMessage('Проценты DSMF сохранены')
  }

  function selectDsmfEmployee(employeeId) {
    const row = rows.find(r => r.employee_id === employeeId)
    setDsmfForm(f => ({ ...f, employee_id: employeeId, official_salary: row ? String(parseNum(row.salary_gross) || '') : '' }))
  }

  async function saveDsmfExpense() {
    setMessage('')
    const row = selectedDsmfEmployeeRow()
    if (!row) return setMessage('Выберите сотрудника')
    const calc = dsmfCalcForSalary()
    if (!calc.base) return setMessage('Введите официальную зарплату')
    if (!calc.total) return setMessage('Сумма DSMF равна 0. Проверьте проценты')
    const { error } = await supabase.from('daily_expenses').insert({
      branch_id: row.branch_id || null,
      expense_date: monthDate,
      amount: calc.total,
      custom_category: 'DSMF',
      comment: `DSMF · ${row.employees?.full_name || ''} · оф. зарплата ${fmt(calc.base)} · ${dsmfForm.comment || ''}`
    })
    if (error) return setMessage(error.message)
    setDsmfForm({ employee_id: '', official_salary: '', comment: '' })
    setMessage('DSMF рассчитан и добавлен отдельной статьёй расходов')
    load()
  }
  ...
  return <section>
    ...
    <section className="grid">
      ...
      <div className="card span-2">
        <div className="card-head"><div><h3>DSMF / налоги по сотрудникам</h3><p className="hint">Расчёт идёт от официальной зарплаты. Проценты можно менять при изменении законодательства.</p></div><button className="small primary" onClick={saveDsmfExpense}>Сохранить DSMF в расходы</button></div>
        <div className="form-grid compact">
          <label><span>Сотрудник</span><select value={dsmfForm.employee_id} onChange={e => selectDsmfEmployee(e.target.value)}><option value="">Выберите сотрудника</option>{rows.map(r => <option key={r.employee_id} value={r.employee_id}>{employeeGroupName(r)} · {r.employees?.full_name}</option>)}</select></label>
          <MoneyInput label="Официальная зарплата" value={dsmfForm.official_salary} onChange={v => setDsmfForm({...dsmfForm, official_salary: v})} />
          ...
          <label><span>Комментарий</span><input value={dsmfForm.comment} onChange={e => setDsmfForm({...dsmfForm, comment: e.target.value})} /></label>
        </div>
        <div className="action-row" style={{marginBottom:12}}><button className="ghost small" onClick={saveDsmfRates}>Сохранить проценты</button><span className="hint">Итого DSMF: <b>{fmt(dsmfCalcForSalary().total)}</b> AZN</span></div>
        <div className="table-wrap"><table><thead><tr><th>База</th><th>DSMF сотр.</th><th>DSMF работ.</th><th>Безработица</th><th>Медстрах.</th><th>Налог</th><th>Итого</th></tr></thead><tbody><tr><td>{fmt(dsmfCalcForSalary().base)}</td><td>{fmt(dsmfCalcForSalary().employeeDsmf)}</td><td>{fmt(dsmfCalcForSalary().employerDsmf)}</td><td>{fmt(dsmfCalcForSalary().unemployment)}</td><td>{fmt(dsmfCalcForSalary().medical)}</td><td>{fmt(dsmfCalcForSalary().incomeTax)}</td><td><b>{fmt(dsmfCalcForSalary().total)}</b></td></tr></tbody></table></div>
        <div className="table-wrap" style={{marginTop:12}}><table><thead><tr><th>Дата</th><th>Филиал</th><th>Сумма</th><th>Комментарий</th></tr></thead><tbody>{dsmfRows.map(r => <tr key={r.id}><td>{r.expense_date}</td><td>{r.branches?.name || '—'}</td><td><b>{fmt(r.amount)}</b></td><td>{r.comment || '—'}</td></tr>)}{!dsmfRows.length && <tr><td colSpan="4" className="hint">—</td></tr>}</tbody></table></div>
      </div>

      <div className="card span-2">
        <h3>Зарплаты сотрудников</h3>
        <div className="table-wrap"><table style={{ tableLayout: 'auto' }}>
          <thead><tr><th style={{width:90}}>Филиал</th><th style={{width:110}}>Должность</th><th style={{minWidth:150, width:'auto'}}>Сотрудник</th><th>Начислено</th><th style={{textAlign:'center'}}>К выплате</th><th>Авансы</th><th>Удержание</th><th>Остаток месяца</th><th>Выплачено</th><th style={{minWidth:130}}>Расчёт</th><th>Мес. ставка</th><th>Дневная</th><th>Дни</th><th>Долг нач.</th><th style={{minWidth:150, textAlign:'center'}}>Статус сотрудника</th><th style={{width:54}}>Инфо</th></tr></thead>
          <tbody>{salaryGroups.map(group => {
            ...
            return <React.Fragment key={`group-${group.id}`}>
              <tr className="system-row"><td colSpan="16"><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12}}>
                <strong>{group.name} · сотрудников: {group.rows.length}</strong>
                {group.rows.length > 1 && <button className="ghost small" onClick={() => setExpandedSalaryGroups(s => ({...s, [group.id]: !expanded}))}>{expanded ? 'Свернуть' : 'Показать всех'}</button>}
              </div></td></tr>
              {groupRows.map(r => {
                ...
                return <React.Fragment key={r.id || r.employee_id}>
                  <tr style={underworked ? { background: 'rgba(155,45,45,.08)' } : undefined}>
                    <td>{employeeGroupName(r.employees)}</td>
                    <td>{r.employees?.position || '—'}</td>
                    <td style={{minWidth:150, maxWidth:260, whiteSpace:'normal'}}>{r.employees?.full_name}</td>
                    <td>{moneyCell(r, r.salary_gross)}</td>
                    <td style={{textAlign:'center'}}>{strongMoneyCell(r, r.final_balance, parseNum(r.final_balance) >= 0 ? 'good' : 'bad')}</td>
                    ...
                    <td><select style={{minWidth:120}} value={r.employees?.salary_type || 'monthly'} onChange={e => updateEmployeeDetails(r, { salary_type: e.target.value }, true)}><option value="monthly">Фикс</option><option value="daily">Ежедневный</option></select></td>
                    <td>{moneyCell(r, r.employees?.monthly_salary)}</td>
                    <td>{moneyCell(r, calcDailyRate(r.employees))}</td>
                    <td>{fmt(r.worked_days)}</td>
                    <td>{strongMoneyCell(r, r.opening_balance, parseNum(r.opening_balance) < 0 ? 'bad' : '')}</td>
                    <td style={{textAlign:'center'}}><span className={r.employees?.employment_status === 'terminated' ? 'bad' : 'good'}>{r.employees?.employment_status === 'terminated' ? `Уволен ${r.employees?.terminated_at || ''}` : `Активный ${r.employees?.hired_at || ''}`}</span></td>
                  ...
                  {employeeInfoId === r.employee_id && <tr><td colSpan="16"><div className="notice">
                    ...
                    <div className="form-grid compact">
                      ...
                      <label><span>Статус</span><select defaultValue={r.employees?.employment_status || 'active'} onChange={e => updateEmployeeDetails(r, { employment_status: e.target.value, is_active: e.target.value !== 'terminated' })}><option value="active">Активный</option><option value="terminated">Уволен</option></select></label>
                      ...
                      <label><span>Дата увольнения</span><input type="date" defaultValue={r.employees?.terminated_at || ''} onBlur={e => updateEmployeeDetails(r, { terminated_at: e.target.value || null })} /></label>
                    </div>
                    <br />
                    <div className="form-grid compact">
          ...
          })}{!visibleSalaryRowsCount && <tr><td colSpan="16" className="hint">Нет активных сотрудников.</td></tr>}</tbody>
        </table></div>
      </div>
      ...
      <div className="card span-2">
        ...
        <p className="hint">Уволенные сотрудники остаются в архиве и не участвуют в активных списках.</p>
        <div className="table-wrap"><table>
          <thead><tr><th>Филиал / группа</th><th>Должность</th><th>Сотрудник</th><th>Дата приёма</th><th>Дата увольнения</th></tr></thead>
          <tbody>{archiveRows.map(r => <tr key={`archive-${r.employee_id}`}><td>{employeeGroupName(r.employees)}</td><td>{r.employees?.position || '—'}</td><td>{r.employees?.full_name}</td><td>{r.employees?.hired_at || '—'}</td><td>{r.employees?.terminated_at || '—'}</td></tr>)}{!archiveRows.length && <tr><td colSpan="5" className="hint">Архив пуст.</td></tr>}</tbody>
        </table></div>
      </div>
...
function DebtsPayments({ t }) {
  ...
  function printSupplierLedger() {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Supplier Ledger</title><style>body{font-family:Arial;padding:24px;color:#17251d}h2{margin:0 0 6px}.muted{color:#777;margin-bottom:18px}.summary{display:flex;gap:12px;margin:16px 0}.box{border:1px solid #ddd;border-radius:12px;padding:10px 14px}.box b{display:block;font-size:18px}table{width:100%;border-collapse:collapse}th,td{border-bottom:1px solid #ddd;padding:8px;text-align:left}th{background:#f4eddf}</style></head><body><h2>Баланс поставщика: ${supplierName}</h2><div class="muted">${filters}</div><div class="summary"><div class="box">Приход / долг<b>${fmt(ledgerTotals.debit)}</b></div><div class="box">Оплата<b>${fmt(ledgerTotals.credit)}</b></div><div class="box">Остаток<b>${fmt(ledgerTotals.balance)}</b></div></div><table><thead><tr><th>Дата</th><th>Поставщик</th><th>E-qaimə</th><th>Операция</th><th>Приход / долг</th><th>Оплата</th><th>Остаток</th></tr></thead><tbody>${rowsHtml}</tbody></table></body></html>`
  ...
  return <section>
    ...
    <section className="grid">
      ...
      <div className="card span-2 supplier-transactions-panel">
        ...
        <div className="form-grid compact">
          <label><span>Поиск поставщика</span><input value={ledgerSearch} onChange={e => setLedgerSearch(e.target.value)} placeholder="Название или VOEN" /></label>
          <label><span>Поставщик</span><select value={ledgerSupplierId} onChange={e => setLedgerSupplierId(e.target.value)}><option value="all">Все найденные / все поставщики</option>{searchedSuppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></label>
          <label><span>Поиск E-qaimə / фактуры</span><input value={invoiceSearch} onChange={e => setInvoiceSearch(e.target.value)} placeholder="Номер фактуры" /></label>
        </div>
        <div className="mini-grid">
          ...
          <div className="metric"><span>Остаток</span><strong className={ledgerTotals.balance > 0 ? 'bad' : 'good'}>{fmt(ledgerTotals.balance)}</strong></div>
        </div>
        <div className="table-wrap"><table><thead><tr><th>Дата</th><th>Поставщик</th><th>E-qaimə</th><th>Операция</th><th>Приход / долг</th><th>Оплата</th><th>Остаток</th></tr></thead><tbody>{supplierLedgerRows.map(r => <tr key={r.id}><td>{r.transaction_date}</td><td>{r.suppliers?.name || '—'}</td><td>{r.invoice || '—'}</td><td>{r.comment}</td><td>{fmt(r.debit)}</td><td>{fmt(r.credit)}</td><td className={r.balance > 0 ? 'bad' : 'good'}><b>{fmt(r.balance)}</b></td></tr>)}{!supplierLedgerRows.length && <tr><td colSpan="7" className="hint">Операции не найдены.</td></tr>}</tbody></table></div>
      </div>
