...
function Revenue({ t }) {
  ...
  async function load() {
    ...
    setCategories(cats || [])
    await loadMonthStats(branchId, date)
  }
  ...
  async function addExpense() {
    ...
    const { error } = await supabase.from('daily_expenses').insert({
      ...
      custom_category: category ? null : t('new_expense'),
      amount: 0
    })
    if (!error) load()
  }
  ...
  async function cancelExpense(id) {
    ...
    const payload = { deleted_at: new Date().toISOString(), deleted_by: userData?.user?.id || null }
    setExpenses(prev => prev.map(e => e.id === id ? { ...e, ...payload } : e))
    await supabase.from('daily_expenses').update(payload).eq('id', id)
  }
  ...
  return (
    <section id="revenuePage">
      ...
      <section className="grid">
        ...
        </div><div className="form-grid compact" style={{marginTop:12}}><label><span>Итого за дату</span><input value={fmt(dailyRevenueTotal)} readOnly /></label></div></div>

        <div className="card span-2">
          <div className="card-head"><div><h3>{t('daily_expenses_title')}</h3><p className="hint">Сумма и комментарий сохраняются после выхода из поля, поэтому ввод не тормозит.</p></div><button className="small" onClick={addExpense}>{t('add_expense')}</button></div>
...
function ExpenseRow({ expense, categories, onSave, onCancel }) {
  ...
  return (
    <tr className={cancelled ? 'cancelled-row' : ''}>
      ...
      <td>{cancelled ? <span className="hint">Отменено</span> : <button className="remove" onClick={onCancel}>×</button>}</td>
    </tr>
  )
}
