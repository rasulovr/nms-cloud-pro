...
export default function RMSLoyaltyPOSScan() {
  ...
  async function startCamera() {
    ...
    try {
      ...
      setCameraError(`Не удалось открыть камеру: ${err?.message || err}. Проверьте, что сайт открыт через https и разрешён доступ к камере.`)
    }
  }
  ...
  function beginDetectLoop() {
    ...
    scanTimerRef.current = setInterval(async () => {
      try {
        ...
        if (!video || video.readyState < 2 || !window.jsQR) return
    ...
    try {
      ...
      img.onload = async () => {
        try {
          ...
          if (!code?.data) {
            setCameraError('QR-код на фото не распознан. Попробуйте сфотографировать ближе и ровнее.')
            return
          }

          setRawScan(code.data)
          await findClientByToken(null, code.data)
        } catch (err) {
          setCameraError(`Не удалось распознать QR с фото: ${err?.message || err}`)
        }
      }

      img.onerror = () => {
        URL.revokeObjectURL(imageUrl)
        setCameraError('Не удалось открыть выбранное фото.')
      }

      img.src = imageUrl
    } catch (err) {
      setCameraError(`Не удалось загрузить QR-сканер: ${err?.message || err}`)
    }
  }
    ...
    <div className="loyalty-scan-page">
      <section className="loyalty-scan-hero">
        <div>
          ...
          <p>Официант открывает этот раздел на телефоне и сканирует QR-код гостя из QR Menu через камеру или фото-режим.</p>
        </div>
      ...
      <section className="loyalty-scan-grid">
        <div className="loyalty-scan-card">
          ...
          <p>Откройте этот экран на телефоне официанта/планшете. Для iPhone используйте “Сканировать через фото/камеру iPhone”.</p>
          ...
          <div className="loyalty-camera-actions">
            ...
            />
            <button className="loyalty-scan-primary" onClick={openPhotoScanner}>Сканировать через фото/камеру iPhone</button>
            {!cameraOpen ? (
              <button className="loyalty-scan-secondary" onClick={startCamera}>Live camera scan</button>
            ) : (
