...
export default function RMSLoyaltyPOSScan() {
  ...
  const maxRedeem = client ? Math.min(Number(client.bonus_balance || 0), Number((orderTotal * 0.30).toFixed(2))) : 0
  const redeemAmount = Math.min(parseNum(form.redeem_amount), maxRedeem)
  const netPaidAmount = Math.max(0, orderTotal - redeemAmount)
  ...
  async function attachAndPay() {
    ...
    if (orderTotal <= 0) {
      setMessage('Укажите сумму чека.')
      return
    }

    if (parseNum(form.redeem_amount) > maxRedeem) {
      setMessage(`Превышен лимит списания. Можно списать максимум ${fmt(maxRedeem)}.`)
      return
    }

    const nextBalance = Math.max(0, Number(client.bonus_balance || 0) - redeemAmount + cashback)
    const nextSpent = Number(client.total_spent || 0) + netPaidAmount
    ...
    if (redeemAmount > 0) {
      const { error: redeemError } = await supabase.from('rms_loyalty_transactions').insert({
        ...
        comment: `Списание бонусов через POS scan · ${form.payment_method}`,
      })

      if (redeemError) {
        setLoading(false)
        setMessage(redeemError.message)
        return
      }
    }
    ...
    setMessage(`Готово. Списано: ${fmt(redeemAmount)}. Оплачено: ${fmt(netPaidAmount)}. Cashback: ${fmt(cashback)}. Новый баланс: ${fmt(updatedClient.bonus_balance)}.`)
    setForm({ ...form, pos_check_id: '', order_total: '', redeem_amount: '', note: '' })
  }
      ...
      <section className="loyalty-scan-grid">
        <div className="loyalty-scan-card">
          ...
          <div className="loyalty-camera-actions">
            ...
            />
            {!cameraOpen ? (
              <button className="loyalty-scan-secondary" onClick={startCamera}>Запасной live scan</button>
            ) : (
        ...
        <div className="loyalty-scan-card">
          ...
          <div className="loyalty-scan-form">
            ...
            <label>Сумма чека<input value={form.order_total} onChange={(e) => setForm({ ...form, order_total: e.target.value })} placeholder="Например: 45.00" /></label>
            <label>Списать бонусы<input value={form.redeem_amount} onChange={(e) => setForm({ ...form, redeem_amount: e.target.value })} placeholder={`Максимум: ${fmt(maxRedeem)}`} /></label>
          ...
          {client ? (
            <div className="loyalty-scan-client">
              ...
              <div className="loyalty-scan-cashback">
                <span>Оплата с Loyalty</span>
                <b>{fmt(netPaidAmount)}</b>
                <small>Чек {fmt(orderTotal)} · списание {fmt(redeemAmount)} · максимум {fmt(maxRedeem)}</small>
              </div>

              <div className="loyalty-scan-cashback">
                <span>Будет начислено</span>
                <b>{fmt(cashback)}</b>
                <small>{CASHBACK_PERCENT}% от фактически оплаченной суммы</small>
              </div>

              <button className="loyalty-scan-primary" onClick={attachAndPay} disabled={loading}>
                {loading ? 'Сохранение…' : 'Подтвердить оплату и обновить баланс'}
              </button>
