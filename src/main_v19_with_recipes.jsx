...
function Recipes({ t }) {
  ...
  const [menuForm, setMenuForm] = useState({ name: '', category: '', sale_price: '', target_food_cost_percent: '30' })
  ...
  function ingredientUnitCost(ingredient) {
    ...
    const qty = parseNum(ingredient?.purchase_unit_quantity) || 1
    return price / qty
  }

  function recipeLineCost(row) {
    const baseQty = parseNum(row.quantity)
    const waste = parseNum(row.waste_percent)
    return ingredientUnitCost(row.ingredients) * baseQty * (1 + waste / 100)
  }
  ...
  async function addMenuItem() {
    ...
    if (error) return setMessage(error.message)
    setMenuForm({ name: '', category: '', sale_price: '', target_food_cost_percent: '30' })
    await loadBase()
  ...
  return (
    <section>
      <section className="topbar">
        <div>
          <h2>{t('recipes_tab')}</h2>
          <p>Техкарты блюд, ингредиенты, себестоимость и food cost.</p>
        </div>
      </section>
      ...
      <section className="grid">
        <div className="card">
          <h3>Себестоимость</h3>
          <div className="big-number">{fmt(recipeCost)}</div>
          <p className="hint">Себестоимость выбранного блюда</p>
        </div>
        <div className="card">
          <h3>Цена продажи</h3>
          <div className="big-number">{fmt(salePrice)}</div>
          <p className="hint">Цена в меню</p>
        </div>
        <div className="card">
          <h3>Food cost</h3>
          <div className={`big-number ${foodCostPercent <= parseNum(selectedMenu?.target_food_cost_percent || 30) ? 'good' : 'bad'}`}>{pct(foodCostPercent)}</div>
          <p className="hint">Цель: {fmt(selectedMenu?.target_food_cost_percent || 30)}%</p>
        </div>
        <div className="card">
          <h3>Валовая прибыль</h3>
          <div className={`big-number ${grossProfit >= 0 ? 'good' : 'bad'}`}>{fmt(grossProfit)}</div>
          <p className="hint">Цена − себестоимость</p>
        </div>
        ...
        <div className="card span-2">
          <div className="card-head">
            <div>
              ...
              <p className="hint">Добавь блюдо, цену продажи и целевой food cost.</p>
            </div>
          </div>
          <div className="form-grid compact">
            ...
            <label><span>Категория</span><input value={menuForm.category} onChange={e => setMenuForm({...menuForm, category: e.target.value})} placeholder="Завтраки / Основные / Напитки" /></label>
          ...
          <button className="small" onClick={addMenuItem}>+ Добавить блюдо</button>
          <br /><br />
              ...
              <tbody>
                {menuItems.map(item => (
                  <tr key={item.id}>
                    ...
                    <td><input defaultValue={item.category || ''} onBlur={e => updateMenuItem(item.id, { category: e.target.value || null })} /></td>
        ...
        <div className="card span-2">
          <div className="card-head">
            <div>
              <h3>Ингредиенты / закупочные цены</h3>
              <p className="hint">Пример: сыр — 12 AZN за 1000 g, кофе — 38 AZN за 1000 g, молоко — 2.20 AZN за 1000 ml.</p>
            </div>
          </div>
          <div className="form-grid compact">
            <label><span>Ингредиент</span><input value={ingredientForm.name} onChange={e => setIngredientForm({...ingredientForm, name: e.target.value})} /></label>
            <label><span>Единица</span><input value={ingredientForm.unit} onChange={e => setIngredientForm({...ingredientForm, unit: e.target.value})} placeholder="g / ml / pcs" /></label>
            <label><span>Цена закупки</span><input inputMode="decimal" value={ingredientForm.purchase_price} onChange={e => setIngredientForm({...ingredientForm, purchase_price: e.target.value})} /></label>
            <label><span>Кол-во в закупке</span><input inputMode="decimal" value={ingredientForm.purchase_unit_quantity} onChange={e => setIngredientForm({...ingredientForm, purchase_unit_quantity: e.target.value})} /></label>
          </div>
          <button className="small" onClick={addIngredient}>+ Добавить ингредиент</button>
          <br /><br />
          <div className="table-wrap">
            <table>
              <thead><tr><th>Ингредиент</th><th>Ед.</th><th>Цена закупки</th><th>Кол-во</th><th>Цена за ед.</th><th></th></tr></thead>
              <tbody>
                {ingredients.map(item => (
                  <tr key={item.id}>
                    <td><input defaultValue={item.name} onBlur={e => updateIngredient(item.id, { name: e.target.value.trim() })} /></td>
                    <td><input defaultValue={item.unit} onBlur={e => updateIngredient(item.id, { unit: e.target.value.trim() || 'g' })} /></td>
                    ...
                    <td>{fmt(ingredientUnitCost(item))}</td>
        ...
        <div className="card span-2">
          <div className="card-head">
            <div>
              ...
              <p className="hint">Выбранное блюдо: <strong>{selectedMenu?.name || 'не выбрано'}</strong></p>
            </div>
          ...
          <div className="table-wrap">
            <table>
              <thead><tr><th>Ингредиент</th><th>Кол-во</th><th>Потери %</th><th>Ед.</th><th>Цена за ед.</th><th>Сумма</th><th></th></tr></thead>
              <tbody>
                {recipeItems.map(row => (
                  <tr key={row.id}>
                    <td>
                    ...
                    <td><input inputMode="decimal" defaultValue={row.quantity} onBlur={e => updateRecipeItem(row.id, { quantity: parseNum(e.target.value) })} /></td>
                    ...
                    <td>{row.ingredients?.unit}</td>
                    <td>{fmt(ingredientUnitCost(row.ingredients))}</td>
