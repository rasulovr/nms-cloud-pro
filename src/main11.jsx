...
function Attendance({ t }) {
  ...
  const [message, setMessage] = useState('')
  const [plannedDays, setPlannedDays] = useState('26')
  const [employeeForm, setEmployeeForm] = useState({ full_name: '', position: '', branch_id: '', salary_type: 'daily', monthly_salary: '', daily_rate: '' })
  ...
  function valueFor(employeeId, day) {
    ...
    return attendance.find(a => a.employee_id === employeeId && a.work_date === date)?.value || 0
  }
  ...
  function calcSalary(emp, worked) {
    if (emp.salary_type === 'monthly') {
      const baseDays = parseNum(plannedDays) || dim
      return baseDays ? (parseNum(emp.monthly_salary) / baseDays) * worked : 0
    }
    return parseNum(emp.daily_rate) * worked
  }
  ...
  async function addEmployee() {
    ...
    const { error } = await supabase.from('employees').insert({
      ...
      monthly_salary: parseNum(employeeForm.monthly_salary),
      daily_rate: parseNum(employeeForm.daily_rate)
    })
    if (error) return setMessage(error.message)
    setEmployeeForm(f => ({ ...f, full_name: '', position: '', monthly_salary: '', daily_rate: '' }))
    await load()
    setMessage(t('saved'))
  }
  ...
  async function syncSalaryForEmployee(emp) {
    ...
    const gross = calcSalary(emp, worked)
    const { error } = await supabase.from('salary_periods').upsert({
      ...
      salary_gross: gross,
      salary_net: gross,
      comment: `Авторасчёт из посещаемости: ${worked} дн.`
  ...
  async function syncAllSalaries() {
    ...
    setMessage('Зарплаты пересчитаны по посещаемости')
  }
  ...
  return (
    <section>
      <section className="topbar"><div><h2>{t('attendance_tab')}</h2><p>Ежедневная посещаемость сотрудников. На основании этой таблицы автоматически считается зарплата.</p></div></section>
      <section className="grid">
        <div className="card span-2">
          <div className="card-head"><div><h3>Период и филиал</h3><p className="hint">Формат как в месячной Excel-таблице: 1/0/0.5 по дням месяца.</p></div><button className="small" onClick={syncAllSalaries}>Пересчитать зарплаты</button></div>
          <div className="form-grid compact">
            <label><span>{t('year')}</span><select value={year} onChange={e => setYear(Number(e.target.value))}>{defaultYears().map(y => <option key={y} value={y}>{y}</option>)}</select></label>
            ...
            <label><span>Норма дней для месячной ставки</span><input inputMode="decimal" value={plannedDays} onChange={e => setPlannedDays(e.target.value)} /></label>
          </div>
          ...
          {message && <p className={`hint ${message === t('saved') || message.includes('пересчитаны') ? 'good' : 'bad'}`}>{message}</p>}
        </div>

        <div className="card span-2">
          <div className="card-head"><div><h3>Добавить сотрудника</h3><p className="hint">Сотрудник привязывается к филиалу, должности и ставке.</p></div></div>
          <div className="form-grid compact">
            <label><span>И.Ф.О.</span><input value={employeeForm.full_name} onChange={e => setEmployeeForm({...employeeForm, full_name: e.target.value})} /></label>
            ...
            <label><span>Тип ставки</span><select value={employeeForm.salary_type} onChange={e => setEmployeeForm({...employeeForm, salary_type: e.target.value})}><option value="daily">Дневная</option><option value="monthly">Месячная</option></select></label>
            ...
            <label><span>Дневная ставка</span><input inputMode="decimal" value={employeeForm.daily_rate} onChange={e => setEmployeeForm({...employeeForm, daily_rate: e.target.value})} /></label>
          </div>
          <button className="small" onClick={addEmployee}>+ Добавить сотрудника</button>
        </div>

        <div className="card span-2">
          <div className="card-head"><div><h3>Табель посещаемости</h3><p className="hint">Клик/ввод по ячейке: 1 — рабочий день, 0 — выходной, 0.5 — полдня.</p></div></div>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Филиал</th><th>Должность</th><th>И.Ф.О.</th>{days.map(d => <th key={d}>{d}</th>)}<th>Дни</th><th>Зарплата</th></tr></thead>
              <tbody>
                {employees.map(emp => {
                  const worked = workedDays(emp.id)
                  const salary = calcSalary(emp, worked)
                  return <tr key={emp.id}>
                    ...
                    <td><input defaultValue={emp.position || ''} onBlur={e => updateEmployee(emp.id, { position: e.target.value.trim() || null })} /></td>
                    <td><input defaultValue={emp.full_name} onBlur={e => updateEmployee(emp.id, { full_name: e.target.value.trim() })} /></td>
                    {days.map(d => <td key={d}><input style={{minWidth:46}} inputMode="decimal" defaultValue={valueFor(emp.id, d) || ''} onBlur={e => setAttendanceValue(emp, d, e.target.value)} /></td>)}
                    <td><strong>{fmt(worked)}</strong></td>
                    <td><strong>{fmt(salary)}</strong></td>
                  </tr>
                })}
                {!employees.length && <tr><td colSpan={days.length + 5} className="hint">Сначала добавьте сотрудников.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
...
function Salaries({ t }) {
  ...
  async function load() {
    let q = supabase.from('salary_periods').select('*, employees(full_name, position), branches(name)').eq('salary_month', monthStart(year, month)).order('branch_id')
  ...
  async function updateSalary(id, patch) {
    setMessage('')
    const payload = { ...patch }
    ;['worked_days','salary_gross','card_payment','cash_payment','advance_amount','deduction_amount','salary_net'].forEach(k => { if (k in payload) payload[k] = parseNum(payload[k]) })
    const { error } = await supabase.from('salary_periods').update(payload).eq('id', id)
    if (error) setMessage(error.message)
    else { await load(); setMessage(t('saved')) }
  }

  const total = rows.reduce((s, r) => s + parseNum(r.salary_net), 0)

  return <section>
    <section className="topbar"><div><h2>{t('salaries_tab')}</h2><p>Зарплаты формируются из посещаемости. Здесь можно проверить и вручную скорректировать выплаты.</p></div></section>
    <section className="grid">
      <div className="card span-2">
        <div className="form-grid compact">
          <label><span>{t('year')}</span><select value={year} onChange={e => setYear(Number(e.target.value))}>{defaultYears().map(y => <option key={y} value={y}>{y}</option>)}</select></label>
        ...
        <div className="metric"><span>Итого зарплата</span><strong>{fmt(total)}</strong></div>
        {message && <p className={`hint ${message === t('saved') ? 'good' : 'bad'}`}>{message}</p>}
      </div>
      <div className="card span-2">
        <div className="table-wrap"><table>
          <thead><tr><th>Филиал</th><th>Должность</th><th>Сотрудник</th><th>Дни</th><th>Начислено</th><th>Карта</th><th>Наличные</th><th>Аванс</th><th>Удержание</th><th>К выплате</th><th>Комментарий</th></tr></thead>
          <tbody>{rows.map(r => <tr key={r.id}>
            <td>{r.branches?.name || '—'}</td><td>{r.employees?.position || '—'}</td><td>{r.employees?.full_name}</td>
            <td><input defaultValue={r.worked_days} onBlur={e => updateSalary(r.id, { worked_days: e.target.value })} /></td>
            <td><input defaultValue={r.salary_gross} onBlur={e => updateSalary(r.id, { salary_gross: e.target.value })} /></td>
            <td><input defaultValue={r.card_payment} onBlur={e => updateSalary(r.id, { card_payment: e.target.value })} /></td>
            <td><input defaultValue={r.cash_payment} onBlur={e => updateSalary(r.id, { cash_payment: e.target.value })} /></td>
            <td><input defaultValue={r.advance_amount} onBlur={e => updateSalary(r.id, { advance_amount: e.target.value })} /></td>
            <td><input defaultValue={r.deduction_amount} onBlur={e => updateSalary(r.id, { deduction_amount: e.target.value })} /></td>
            <td><input defaultValue={r.salary_net} onBlur={e => updateSalary(r.id, { salary_net: e.target.value })} /></td>
            <td><input defaultValue={r.comment || ''} onBlur={e => updateSalary(r.id, { comment: e.target.value })} /></td>
          </tr>)}{!rows.length && <tr><td colSpan="11" className="hint">Нет расчётов за выбранный месяц. Заполните посещаемость и нажмите “Пересчитать зарплаты”.</td></tr>}</tbody>
        </table></div>
      </div>
    </section>
  </section>
