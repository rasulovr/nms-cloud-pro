...
function SalaryWorkspace({ t }) {
  ...
  return (
    <section>
      ...
      {salaryTab === 'sheet' && <Salaries t={t} view="sheet" />}
...
function Recipes({ t }) {
  ...
  useEffect(() => {
    const first = products.find(p => normalizeProductType(p.category) === selectedCategory)
  ...
  async function loadBase() {
    ...
    if (isInternal) {
      const { data: ws, error } = await fetchRmsRecipesWorkspace()
      if (!error && ws) {
      ...
      setMessage(error?.message || 'Нет доступа к тех. картам')
    }

    const [{ data: prod }, { data: item }, { data: latest }, { data: recipeRows }] = await Promise.all([
...
function Salaries({ t, view = 'employees' }) {
  ...
  function exportSalarySheetCsv() {
    const headers = ['Филиал', 'Сотрудник', 'Должность', 'Базовая зарплата', 'Начислено', 'Авансы', 'К выплате']
  ...
  function printSalarySheet() {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>${title}</title>
      <style>
        body{font-family:Arial,sans-serif;background:#f4f6f8;margin:0;padding:24px;color:#17212f}
        h1{margin:0 0 6px;font-size:24px}
        .muted{color:#667085;margin-bottom:18px}
        .sheet{background:#fff;border:1px solid #d7dee8;border-radius:18px;padding:22px}
        table{width:100%;border-collapse:collapse;font-size:13px}
        th{background:#eef2f7;color:#475569;text-align:left;padding:10px;border-bottom:1px solid #d7dee8}
        td{padding:10px;border-bottom:1px solid #e5e9ef}
        .num{text-align:right}
        ...
        @media print{body{background:white;padding:0}.actions{display:none}.sheet{border:0;border-radius:0}}
      </style></head><body>
      ...
      <div class="sheet">
        ...
        <table>
          <thead><tr><th>Филиал</th><th>Сотрудник</th><th>Должность</th><th class="num">Базовая зарплата</th><th class="num">Начислено</th><th class="num">Авансы</th><th class="num">К выплате</th></tr></thead>
          <tbody>${rowsHtml || '<tr><td colspan="7">Нет данных</td></tr>'}</tbody>
          <tfoot><tr><td colspan="3">Итого</td><td class="num">${fmt(salarySheetTotals.base)}</td><td class="num">${fmt(salarySheetTotals.accrued)}</td><td class="num">${fmt(salarySheetTotals.advances)}</td><td class="num pay">${fmt(salarySheetTotals.toPay)}</td></tr></tfoot>
        </table>
      </div>
    </body></html>`
    ...
    return <section className="salary-section salary-view-sheet">
      <section className="topbar">
        <div>
          ...
          <p>Компактная таблица по сотрудникам: филиал, должность, базовая зарплата, начислено, авансы и сумма к выплате.</p>
        </div>
          ...
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                ...
                {!salarySheetRows.length && <tr><td colSpan="7" className="hint">Нет данных за выбранный период.</td></tr>}
              </tbody>
              <tfoot>
                <tr>
