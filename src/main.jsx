...
function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: rev } = await supabase.from('monthly_branch_revenue').select('*').eq('month', currentMonth)
      const { data: exp } = await supabase.from('monthly_branch_expenses').select('*').eq('month', currentMonth)

      const revenue = (rev || []).reduce((s, r) => s + parseNum(r.total_revenue), 0)
      const expenses = (exp || []).reduce((s, r) => s + parseNum(r.total_expenses), 0)
  ...
  return (
    <>
      <div className="topbar">
        <div>
          <h2>Дашборд</h2>
          <p>Общая картина по сети за текущий месяц</p>
        </div>
      </div>
      <div className="grid">
        <MetricCard title="Выручка сети" value={stats.revenue} />
        <MetricCard title="Расходы сети" value={stats.expenses} />
        <MetricCard title="Прибыль до зарплат/налогов" value={stats.net} status={stats.net >= 0 ? 'good' : 'bad'} />
      </div>
    </>
  )
}
