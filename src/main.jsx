...
function Reports({ t }) {
  ...
  const renderAiTable = (title, list, sortLabel = 'Выручка') => {
    ...
    return <div className="card span-2" style={{overflow:'hidden'}}>
      ...
      {isLowMargin && showHiddenSalesItems && <div className="table-wrap" style={{marginBottom:12}}><table><thead><tr><th>Скрытая позиция</th><th></th></tr></thead><tbody>
        ...
        {!hiddenSalesKeys.length && <tr><td colSpan="2" className="hint">Скрытых позиций нет. Ненужные позиции скрываются из таблицы низкомаржинальных товаров.</td></tr>}
      </tbody></table></div>}
      <table style={{width:'100%', tableLayout:'fixed'}}><thead><tr><th>Позиция</th><th style={{width:90}}>Тип</th><th style={{width:110}}>Кол-во / цена</th><th style={{width:110}}>Выручка</th><th style={{width:90}}>Food cost</th><th style={{width:90}}>Маржа</th>{isLowMargin && <th style={{width:90}}></th>}</tr></thead><tbody>
        {visible.map((r, idx) => <tr key={`${title}-${idx}-${r.name}`}><td><b>{r.name}</b>{!isNetworkSalesView && <><br /><span className="hint">{r.branch_name || '—'}</span></>}</td><td><span className={r.department === 'Бар' ? 'pill good' : r.department === 'Кухня' ? 'pill warn' : 'pill'}>{r.department === 'Кухня' ? 'Кухня' : r.department === 'Бар' ? 'Бар' : '—'}</span></td><td>{fmt(r.quantity)}<br /><span className="hint">цена {fmt(r.avg_price)}</span></td><td>{fmt(r.revenue)}</td><td>{r.revenue ? pct((r.cost / r.revenue) * 100) : '0.0%'}</td><td className={r.margin < 20 ? 'bad' : r.margin > 65 ? 'good' : ''}><b>{pct(r.margin)}</b></td>{isLowMargin && <td><button className="ghost small" onClick={() => hideSalesItem(r.name)}>Скрыть</button></td>}</tr>)}
        {!visible.length && <tr><td colSpan={isLowMargin ? 7 : 6} className="hint">Нет данных</td></tr>}
      </tbody></table>
  ...
  const SalesTableBlock = <div className="card span-2">
    <div className="card-head"><div><h3>Общая таблица продаж</h3><p className="hint">В общем режиме продажи агрегируются по всей сети без разбивки по филиалам. Себестоимость берётся из техкарт, если позиция найдена в справочнике меню; иначе временно используется себестоимость из AIKO.</p></div></div>
    <div className="form-grid compact">
    ...
    <div className="table-wrap" style={{marginTop:12}}><table><thead><tr><th>Позиция</th>{!isNetworkSalesView && <th>Филиал</th>}<th>Тип</th><th>Категория</th><th>{sortHeader('quantity', 'Кол-во')}</th><th>{sortHeader('revenue', 'Выручка')}</th><th>{sortHeader('cost', 'Себестоимость')}</th><th>{sortHeader('profit', 'Прибыль')}</th><th>{sortHeader('margin', 'Маржа')}</th></tr></thead><tbody>
      ...
      {!rows.length && <tr><td colSpan={isNetworkSalesView ? 8 : 9} className="hint">Нет данных</td></tr>}
    </tbody></table></div>
