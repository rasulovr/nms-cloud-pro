...
async function fetchRmsStaffWorkspaceSnapshot(monthDate) {
  ...
  if (!normalized) return { data: null, error: new Error('Empty RMS staff workspace') }
...
function Login({ lang, setLang, t }) {
  ...
  async function signIn() {
    ...
    const internalUser = normalizedLogin
      ? (internalUsers[normalizedLogin] || Object.values(internalUsers).find(u => normalizeInternalLogin(u?.login || u?.email) === normalizedLogin || u?.id === normalizedLogin))
      : null

    if (internalUser) {
      if (internalUser.is_active === false) return setError('Пользователь отключён')
      if (String(internalUser.password || '') !== String(password || '')) return setError(t('login_error'))

      setInternalSessionStorage({
        rms_internal: true,
        access_token: `rms-internal-${internalUser.id}`,
        user: {
          id: internalUser.id,
          email: `${internalUser.login || normalizedLogin}@rms.internal`,
          login_name: internalUser.login || normalizedLogin
        }
      })
    ...
    const looksLikeInternalLogin = Boolean(normalizedLogin) && (!rawLogin.includes('@') || /@(rms|nms)\.local\.az$/i.test(rawLogin) || /@rms\.internal$/i.test(rawLogin))
    if (looksLikeInternalLogin) {
      return setError('Пользователь не найден или пароль неверный. Создайте пользователя заново в Настройки → Пользователи.')
    }

    const { error } = await supabase.auth.signInWithPassword({ email: rawLogin.toLowerCase(), password })
    if (!error) return
...
function Salaries({ t, view = 'employees' }) {
  ...
  async function load() {
    ...
    setSalaryPayments([...(pays || []), ...(previousDisplayPays || [])].filter(p => !shouldHideManagerSalaries || visibleEmployeeIds.has(p.employee_id)))
    setSalaryAdvanceRows((adv || []).filter(a => !shouldHideManagerSalaries || visibleEmployeeIds.has(a.employee_id)))
    setDsmfRows(dsmf || [])
