...
function Salaries({ t }) {
  ...
  async function addSalaryPayment() {
    ...
    const openingDebt = Math.max(0, parseNum(row.opening_balance))
    const closePreviousAmount = Math.min(amount, openingDebt)
    const advanceAmount = Math.max(0, amount - closePreviousAmount)

    let closedPrevious = 0
    let createdAdvance = 0

    if (closePreviousAmount > 0) {
      const { error: payError } = await supabase.from('salary_payments').insert({
        employee_id: employeeId,
        branch_id: row.branch_id || null,
        salary_month: previousMonthDate,
        payment_date: paymentDate,
        amount: closePreviousAmount,
        method,
        comment: `${commentBase} · закрытие остатка прошлого месяца`
      })
      if (payError) return setMessage(payError.message)
      closedPrevious = closePreviousAmount
    }
    ...
    if (advanceAmount > 0) {
      const { error: advError } = await supabase.from('salary_advances').insert({
        ...
        comment: openingDebt > 0
          ? `${commentBase} · превышение остатка переведено в аванс`
          : `${commentBase} · аванс текущего месяца`
      })
      if (advError) return setMessage(advError.message)
      createdAdvance = advanceAmount
    }
    ...
    const { error: salaryError } = await supabase.from('salary_periods').upsert({
      ...
      advance_amount: advance,
      deduction_amount: deduction,
      card_payment: parseNum(existing?.card_payment || row.card_payment),
      cash_payment: parseNum(existing?.cash_payment || row.cash_payment),
      comment: existing?.comment || 'Автообновление после выплаты / аванса'
    }, { onConflict: 'employee_id,salary_month' })
    if (salaryError) return setMessage(salaryError.message)
    ...
    const parts = []
    if (closedPrevious > 0) parts.push(`закрыт остаток прошлого месяца: ${fmt(closedPrevious)} AZN`)
    if (createdAdvance > 0) parts.push(`записано в аванс текущего месяца: ${fmt(createdAdvance)} AZN`)
    setMessage(parts.length ? parts.join(' · ') : 'Операция выполнена')
  }
    ...
    <section className="grid">
      ...
      <div className="card span-2">
        ...
        {selectedPaymentRow && <div className="mini-grid" style={{marginTop:12}}>
          ...
          <div className="metric"><span>Уже оплачено картой/банком</span><strong>{selectedPaymentCanSee ? fmt(selectedPaidCard) : maskedMoney}</strong></div>
                  ...
                  {groupRows.map(r => {
                    ...
                    return <React.Fragment key={r.id || r.employee_id}>
                      <tr style={underworked ? { background: 'rgba(240,232,216,.55)' } : undefined}>
                        ...
                        <td>{moneyCell(r, r.monthly_salary)}</td>
