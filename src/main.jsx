  ...
  async function signIn() {
    ...
    if (localUser && String(localUser.password || '') === String(password || '')) {
      const { data: prof } = await supabase
        ...
        .eq('id', localUser.id)
        .maybeSingle()
      if (prof?.is_active === false) return setError('Пользователь отключён администратором')
      const nextSession = {
        rms_internal: true,
        user: {
          id: localUser.id,
          ...
          full_name: prof?.full_name || localUser.full_name || internalLogin
        }
      }
...
function Settings({ session, t, theme, setTheme }) {
  ...
  async function addUser() {
    ...
    const existing = users.find(u => normalizeInternalLogin(u.login_name || u.email) === login)
    const userId = existing?.id || (crypto?.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random()}`)
    const basePayload = {
      id: userId,
      email: `${login}@rms.internal`,
      login_name: login,
      full_name: newUser.full_name || login,
    ...
    let profileRes = await supabase.from('user_profiles').upsert(basePayload, { onConflict: 'id' })
    if (profileRes.error && String(profileRes.error.message || '').includes('hide_manager_salary')) {
      const { hide_manager_salary, ...fallbackPayload } = basePayload
      profileRes = await supabase.from('user_profiles').upsert(fallbackPayload, { onConflict: 'id' })
    }
    if (profileRes.error && String(profileRes.error.message || '').includes('email')) {
      const { email, ...fallbackPayload } = basePayload
      profileRes = await supabase.from('user_profiles').upsert(fallbackPayload, { onConflict: 'id' })
    }
    if (profileRes.error) return setMsg(profileRes.error.message)

    const internalUsers = getInternalUsers()
    internalUsers[login] = { id: userId, login, password, full_name: newUser.full_name || login }
    setInternalUsers(internalUsers)

    const hasAnyPermissions = permissions.some(p => p.user_id === userId)
