      ...
      {tab === 'semis' && (
        <section className="grid semifinished-layout">
          ...
          <div className="card semi-composition-card">
            ...
            <p className="hint">Состав выбранного полуфабриката. Строки можно редактировать прямо здесь.</p>
                ...
                <tbody>
                  {selectedSemiItems.map(item => (
                    <tr key={item.id}>
                      ...
                      <td>{item.component_type === 'manual' ? <input defaultValue={item.item_name || ''} onBlur={e => updateSemiLine(item.id, { item_name: e.target.value })} /> : <b>{item.item_name}</b>}</td>
                      <td><input defaultValue={item.qty} onBlur={e => updateSemiLine(item.id, { qty: e.target.value })} /></td>
                      <td><select defaultValue={item.unit || 'g'} onChange={e => updateSemiLine(item.id, { unit: e.target.value })}><option value="g">g</option><option value="kg">kg</option><option value="ml">ml</option><option value="l">l</option><option value="pcs">pcs</option></select></td>
                      <td>{item.component_type === 'manual' ? <input defaultValue={item.manual_unit_cost || 0} onBlur={e => updateSemiLine(item.id, { manual_unit_cost: e.target.value })} /> : <span className="hint">из закупки</span>}</td>
                      <td><input defaultValue={item.waste_percent || 0} onBlur={e => updateSemiLine(item.id, { waste_percent: e.target.value })} /></td>
                      <td><b>{fmt(componentCost(item))} AZN</b></td>
