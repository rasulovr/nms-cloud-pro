...
function inferAikoDepartment(text, fileName = '') {
  ...
  if (value.includes('bar') || value.includes('бар')) return 'Бар'
  if (value.includes('kitchen') || value.includes('kuxna') || value.includes('кухн')) return 'Кухня'
  return 'Смешанный'
}
...
function guessAikoBranch(text, fileName, branches) {
  const source = `${text || ''} ${fileName || ''}`.toLowerCase()
  const clean = (v) => String(v || '').toLowerCase().replace(/[^a-zа-яёə0-9]+/g, ' ').trim()
  const sourceClean = clean(source)
  const direct = (branches || []).find(b => {
    const name = clean(b.name)
    if (!name) return false
    if (sourceClean.includes(name)) return true
...
function aggregateSalesRows(reports) {
  const map = new Map()
  ;(reports || []).forEach(report => {
    ;(report.rows || []).forEach(row => {
      const key = `${normalizeSalesKey(row.name)}|${report.branch_id || report.branch_name || ''}|${row.department || report.department || ''}`
      const existing = map.get(key) || {
        name: row.name,
        branch_id: report.branch_id || '',
        branch_name: report.branch_name || '',
        department: row.department || report.department || '',
...
function Reports({ t }) {
  ...
  const rows = useMemo(() => aggregateSalesRows(filteredReports), [filteredReports])
  ...
  const ai = useMemo(() => {
    ...
    return {
      topSales: [...withRevenue].sort((a, b) => parseNum(b.revenue) - parseNum(a.revenue)).slice(0, 15),
      topQty: [...withRevenue].sort((a, b) => parseNum(b.quantity) - parseNum(a.quantity)).slice(0, 15),
      outsiders: [...withRevenue].sort((a, b) => parseNum(a.quantity) - parseNum(b.quantity) || parseNum(a.revenue) - parseNum(b.revenue)).slice(0, 15),
      highMargin: [...withCost].sort((a, b) => parseNum(b.margin) - parseNum(a.margin) || parseNum(b.revenue) - parseNum(a.revenue)).slice(0, 15),
      lowMargin: [...withRevenue].sort((a, b) => parseNum(a.margin) - parseNum(b.margin)).slice(0, 15)
    }
  }, [rows])
  ...
  async function importParsedReports(parsedReports) {
    const valid = parsedReports.filter(r => (r.rows || []).length)
    if (!valid.length) return setMessage('Не удалось найти строки продаж. Проверьте PDF или вставьте текст отчёта AIKO вручную.')
    const next = [...valid, ...reports].slice(0, 80)
    saveReports(next)
    const allRows = valid.flatMap(r => r.rows || [])
    const created = await syncMenuItemsFromSalesRows(allRows)
    setMessage(`Импортировано отчётов: ${valid.length}. Строк продаж: ${allRows.length}. В техкарты добавлено новых позиций меню: ${created}.`)
  }
  ...
  async function syncMenuItemsFromSalesRows(salesRows) {
    ...
    ;(salesRows || []).forEach(row => {
      const key = normalizeSalesKey(row.name)
  ...
  async function handleFiles(filesLike) {
    ...
    try {
      ...
      for (const file of expandedFiles) {
        ...
        const guessedBranch = guessAikoBranch(text, file.name, branches)
        ...
        const report = parseAikoSalesText(text, {
          ...
          branch_name: selectedBranch?.name || guessedBranch?.name || ''
        })
        parsed.push(report)
      }
  ...
  async function importFromText() {
    ...
    setMessage('')
    try {
      const guessedBranch = guessAikoBranch(importText, 'manual text', branches)
      const selectedBranch = importBranchId !== 'auto' ? branches.find(b => String(b.id) === String(importBranchId)) : guessedBranch
  ...
  const renderAiTable = (title, list, sortLabel = 'Выручка') => <div className="card">
    <div className="card-head"><div><h3>{title}</h3><p className="hint">AI-анализ по выбранному фильтру.</p></div></div>
    <div className="table-wrap"><table><thead><tr><th>Позиция</th><th>Филиал</th><th>Тип</th><th>Кол-во</th><th>{sortLabel}</th><th>Food cost</th><th>Маржа</th></tr></thead><tbody>
      {list.map((r, idx) => <tr key={`${title}-${idx}-${r.name}`}><td><b>{r.name}</b></td><td>{r.branch_name || '—'}</td><td>{r.department || '—'}</td><td>{fmt(r.quantity)}</td><td>{fmt(sortLabel === 'Маржа %' ? r.margin : r.revenue)}</td><td>{r.revenue ? pct((r.cost / r.revenue) * 100) : '0.0%'}</td><td className={r.margin < 20 ? 'bad' : r.margin > 65 ? 'good' : ''}><b>{pct(r.margin)}</b></td></tr>)}
      {!list.length && <tr><td colSpan="7" className="hint">Нет данных</td></tr>}
    </tbody></table></div>
  </div>
  ...
  return <section>
    ...
    <div className="settings-tabs"><button className="active">Отчёт по продажам</button></div>
    <section className="grid">
      <div className="card span-2">
        <div className="card-head"><div><h3>Импорт отчёта продаж AIKO</h3><p className="hint">Поддерживаются PDF из AIKO, ZIP с PDF, TXT/CSV и ручная вставка текста. Позиции из отчётов автоматически создаются в “Тех. карты → Блюда / позиции меню”.</p></div></div>
        <div className="form-grid compact">
        ...
        <div className="action-row"><button className="small primary" disabled={busy} onClick={importFromText}>{busy ? 'Импорт...' : 'Импортировать текст'}</button><button className="ghost small" onClick={clearReports}>Очистить отчёты</button></div>
        {message && <p className={`hint ${message.includes('Импортировано') || message.includes('добавлено') || message.includes('очищ') || message.includes('удал') ? 'good' : 'bad'}`}>{message}</p>}
      </div>

      <div className="card span-2">
        <div className="card-head"><div><h3>Фильтр и сводка</h3><p className="hint">Можно смотреть общий отчёт по сети или отдельно по филиалу и кухне/бару.</p></div></div>
        <div className="form-grid compact">
          <label><span>Филиал</span><select value={branchFilter} onChange={e => setBranchFilter(e.target.value)}><option value="all">Все филиалы</option>{branchOptions.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select></label>
          <label><span>Тип</span><select value={departmentFilter} onChange={e => setDepartmentFilter(e.target.value)}><option value="all">Бар + Кухня</option><option value="Бар">Бар</option><option value="Кухня">Кухня</option><option value="Смешанный">Смешанный</option></select></label>
        </div>
        <div className="mini-grid">
          <div className="metric"><span>Отчётов</span><strong>{filteredReports.length}</strong></div>
          <div className="metric"><span>Позиции</span><strong>{rows.length}</strong></div>
          <div className="metric"><span>Выручка</span><strong>{fmt(totals.revenue)}</strong></div>
          <div className="metric"><span>Себестоимость</span><strong>{fmt(totals.cost)}</strong></div>
          <div className="metric"><span>Валовая прибыль</span><strong className={totals.profit >= 0 ? 'good' : 'bad'}>{fmt(totals.profit)}</strong></div>
          <div className="metric"><span>Food cost</span><strong>{totals.revenue ? pct((totals.cost / totals.revenue) * 100) : '0.0%'}</strong></div>
        </div>
      </div>

      <div className="card span-2">
        <div className="card-head"><div><h3>Импортированные отчёты</h3><p className="hint">Каждый PDF/файл сохраняется отдельным импортом.</p></div></div>
        <div className="table-wrap"><table><thead><tr><th>Файл</th><th>Период</th><th>Филиал</th><th>Тип</th><th>Строк</th><th>Выручка</th><th>Себестоимость</th><th></th></tr></thead><tbody>
          {reports.map(r => <tr key={r.id}><td><b>{r.source_file}</b><br /><span className="hint">{r.warehouse || '—'}</span></td><td>{r.period_start || '—'} — {r.period_end || '—'}</td><td>{r.branch_name || '—'}</td><td>{r.department}</td><td>{(r.rows || []).length}</td><td>{fmt(r.totals?.revenue)}</td><td>{fmt(r.totals?.cost)}</td><td><button className="remove" onClick={() => removeReport(r.id)}>×</button></td></tr>)}
          {!reports.length && <tr><td colSpan="8" className="hint">Загрузите отчёты AIKO для анализа.</td></tr>}
        </tbody></table></div>
      </div>

      <div className="card span-2">
        <div className="card-head"><div><h3>Общая таблица продаж</h3><p className="hint">Агрегировано по позиции, филиалу и типу отчёта.</p></div></div>
        <div className="table-wrap"><table><thead><tr><th>Позиция</th><th>Филиал</th><th>Тип</th><th>Категория</th><th>Кол-во</th><th>Выручка</th><th>Себестоимость</th><th>Прибыль</th><th>Маржа</th></tr></thead><tbody>
          {[...rows].sort((a,b)=>parseNum(b.revenue)-parseNum(a.revenue)).slice(0, 120).map((r, idx) => <tr key={`${r.name}-${idx}`}><td><b>{r.name}</b></td><td>{r.branch_name || '—'}</td><td>{r.department || '—'}</td><td>{r.menu_category || r.source_category || '—'}</td><td>{fmt(r.quantity)}</td><td>{fmt(r.revenue)}</td><td>{fmt(r.cost)}</td><td className={r.profit >= 0 ? 'good' : 'bad'}>{fmt(r.profit)}</td><td>{pct(r.margin)}</td></tr>)}
          {!rows.length && <tr><td colSpan="9" className="hint">Нет данных</td></tr>}
        </tbody></table></div>
      </div>
