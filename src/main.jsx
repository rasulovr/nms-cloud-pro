...
async function rmsCalculateNetworkForecastForMonth(y, m, scopeBranchId = 'all') {
  ...
  const [
    ...
    supabase.from('salary_periods').select('employee_id,branch_id,salary_month,salary_gross,salary_net,final_balance,payroll_payments').eq('salary_month', monthDate),
    supabase.from('salary_periods').select('employee_id,branch_id,salary_month,salary_gross,salary_net,final_balance,payroll_payments').gte('salary_month', historyStart).lt('salary_month', historyEnd),
  ...
  const buildArticleMap = (expenseRows = [], purchaseRows = []) => {
    const map = new Map()
  ...
  const currentArticleMap = buildArticleMap(
    ...
    (currentPurchaseRows || []).filter(filterByBranch)
  )
  const historyArticleMaps = prevMonths.map(pm => buildArticleMap(
    ...
    (historicalPurchaseRows || []).filter(r => r.purchase_date >= pm.start && r.purchase_date < pm.end).filter(filterByBranch)
  ))
  ...
  const sumSalaryPeriodRows = (rows = []) => (rows || []).filter(filterByBranch).reduce((sum, row) => sum + salaryPeriodAmount(row), 0)
  const salaryHistoryByMonth = prevMonths.map(pm => {
    ...
    const periodValue = sumSalaryPeriodRows((historicalSalaryPeriodRows || []).filter(r => r.salary_month >= pm.start && r.salary_month < pm.end))
    ...
    return monthlyValue || periodValue || specialValue
  })
  const payrollDetails = rmsFinancePayrollDetailsForScope(employeeRows || [], selectedBranchId, revenueShareMap)
  const fixedSalary = parseNum(payrollDetails.totalSalary)
  const currentMonthlySalary = sumMonthlySalaryRows(currentSalaryRows || [])
  const currentPeriodSalary = sumSalaryPeriodRows(currentSalaryPeriodRows || [])
  ...
  const salaryForecast = fixedSalary || currentMonthlySalary || currentPeriodSalary || currentExpenseSalary || salaryHistory
  const salaryForecastNote = fixedSalary > 0
    ? 'фиксированный фонд активных сотрудников из раздела “Зарплаты”'
    : currentMonthlySalary > 0
      ? 'из месячного расчёта зарплат за текущий месяц'
      : currentPeriodSalary > 0
        ? 'из зарплатных периодов за текущий месяц'
