...
function Salaries({ t, view = 'employees', isAdmin = false }) {
  ...
  function salaryRoleKey(position) {
    ...
    if (p.includes('повар')) return 'cook'
    if (p.includes('бариста') || p.includes('бармен') || p === 'бар' || p.includes('бар ')) return 'bar'
    if (p.includes('стюард') || p.includes('стьюард') || p.includes('стюарт') || p.includes('стьюарт') || p.includes('steward')) return 'steward'
    return 'other'
  }

  function salaryRoleLabel(position) {
    const key = salaryRoleKey(position)
    if (key === 'cook') return 'Повара'
