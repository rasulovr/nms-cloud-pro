...
async function rmsCalculateNetworkForecastForMonth(y, m, scopeBranchId = 'all') {
  ...
  const salaryForecast = currentPeriodSalary || activeEmployeeSalary || currentMonthlySalary || currentExpenseSalary || salaryHistory
  const salaryForecastNote = currentPeriodSalary > 0
    ? 'из “Зарплаты → Сотрудники → Сводка по филиалам”; менеджеры распределены по доле выручки и не суммируются дважды'
    : activeEmployeeSalary > 0
      ? 'из активных сотрудников; менеджеры распределены по доле выручки и не суммируются дважды'
