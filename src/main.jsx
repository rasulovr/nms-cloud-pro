...
function Settings({ session, t, theme, setTheme }) {
  ...
  function parseAttendanceImport(textValue = attendanceImportText) {
    ...
    const first = splitImportLine(lines[0]).map(x => normalizeImportValue(x))
    const hasCsvHeader = first.includes('branch') && first.includes('position') && first.includes('name') && first.includes('worked_days')
    ...
    if (hasCsvHeader) {
      ...
      const idxPosition = header.indexOf('position')
      const idxName = header.indexOf('name')
      const idxWorked = header.indexOf('worked_days')
      const dayIndexes = Array.from({ length: 31 }, (_, i) => header.indexOf(`day_${i + 1}`))
  ...
  async function importAttendanceToRms() {
    ...
    const report = { imported: 0, skipped: 0, attendanceRows: 0, salaryRows: 0, errors: [] }
    ...
    try {
      ...
      for (const row of rows) {
        ...
        let employee = (existingEmployees || []).find(e =>
          normalizeImportValue(e.full_name) === nameKey && ((e.branch_id || null) === (branch.branch_id || null))
        )

        if (!employee) {
          employee = (existingEmployees || []).find(e => normalizeImportValue(e.full_name) === nameKey)
        }

        if (!employee) {
          report.skipped += 1
          report.errors.push(`Строка ${row.line}: сотрудник не найден — ${row.name} / ${row.branch}`)
          continue
        }
      ...
      setMsg(`Импорт табеля завершён. Сотрудников: ${report.imported}, строк посещаемости: ${report.attendanceRows}, зарплатных периодов: ${report.salaryRows}, пропущено: ${report.skipped}.`)
    } catch (e) {
