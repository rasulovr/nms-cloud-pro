...
function Dashboard({ t }) {
  ...
  async function calcMonth(y, m) {
    ...
    const forecastAdjustments = { supplier: 0, dsmf: 0, salaryOverride: 0 }
    const branchRows = branches.map(b => {
      ...
      baseTotals.tax += tax
      forecastAdjustments.supplier += supplierExtra
      forecastAdjustments.dsmf += dsmfExtra
      forecastAdjustments.salaryOverride += Math.max(0, salaryOverride)
    ...
    const baseCostRate = baseTotals.revenue ? (baseTotals.expenses + baseTotals.salary + baseTotals.serviceCost) / baseTotals.revenue : 0
    const baseForecastProfit = forecastRevenue - (forecastRevenue * baseCostRate) - (forecastRevenue * TAX_RATE / 100)
    const forecastProfit = baseForecastProfit - forecastAdjustments.supplier - forecastAdjustments.dsmf - forecastAdjustments.salaryOverride
      ...
      {tab === 'semis' && (
        <section className="grid semifinished-layout">
          ...
          <div className="card semi-composition-card">
            ...
            <div className="table-wrap">
              <table>
                ...
                <tbody>
                  ...
                  {!selectedSemiItems.length && <tr><td colSpan="6" className="hint">Состав пока пустой. Добавь ингредиенты ниже.</td></tr>}
                </tbody>
              </table>
            </div>
            ...
            {semiEditModalOpen && (
              <div className="semi-selected-editor">
                <h4>Редактировать состав</h4>
                <p className="hint">Все ингредиенты редактируются здесь. Каждая строка имеет отдельные широкие поля.</p>

                <div className="semi-edit-cards">
                  {semiEditDraft.map(row => (
                    <div className="semi-edit-card" key={row.id}>
