      ...
      {tab === 'semis' && (
        <section className="grid semifinished-layout">
          ...
          <div className="card semi-composition-card">
            ...
            <p className="hint">Нажмите “Редактировать состав” — поля откроются прямо в этой таблице.</p>
              ...
              <table>
                ...
                <tbody>
                  {selectedSemiItems.map(item => {
                    const draft = semiEditDraft.find(row => String(row.id) === String(item.id))
                    return (
                      <tr
                        key={item.id}
                      ...
                      >
                        <td>
                          {semiEditModalOpen && draft && item.component_type === 'manual' ? (
                            <input className="semi-inline-input name" value={draft.item_name} onChange={e => updateSemiEditDraft(item.id, { item_name: e.target.value })} />
                          ) : (
                            <>
                              <div className="semi-main-value semi-name">{item.item_name}</div>
                              <div className="semi-sub-value">Цена: {item.component_type === 'manual' ? fmt(item.manual_unit_cost || 0) : 'закупка'} · Потери: {fmt(item.waste_percent || 0)}%</div>
                            </>
                          )}
                          {semiEditModalOpen && item.component_type !== 'manual' ? <b>{item.item_name}</b> : null}
                        </td>
                        <td>
                          {semiEditModalOpen && draft ? (
                            <input className="semi-inline-input qty" value={draft.qty} onChange={e => updateSemiEditDraft(item.id, { qty: e.target.value })} />
                          ) : (
                            <b>{fmt(item.qty)}</b>
                          )}
                        </td>
                        <td>
                          {semiEditModalOpen && draft ? (
                            <select className="semi-inline-input unit" value={draft.unit} onChange={e => updateSemiEditDraft(item.id, { unit: e.target.value })}>
                        ...
                        <td>
                          {semiEditModalOpen && draft && item.component_type === 'manual' ? (
                            <input className="semi-inline-input price" value={draft.manual_unit_cost} onChange={e => updateSemiEditDraft(item.id, { manual_unit_cost: e.target.value })} />
                          ) : (
                        ...
                        <td>
                          {semiEditModalOpen && draft ? (
                            <input className="semi-inline-input waste" value={draft.waste_percent} onChange={e => updateSemiEditDraft(item.id, { waste_percent: e.target.value })} />
                          ) : (
