...
function POSLite({ t }) {
  ...
  const tableButtons = useMemo(() => ([
    ...Array.from({ length: 12 }, (_, i) => ({ id: `T${i + 1}`, label: `Стол ${i + 1}` })),
    { id: 'TA', label: 'Take Away' },
    { id: 'DL', label: 'Доставка' },
    { id: 'BAR', label: 'Бар' },
    { id: 'VIP', label: 'VIP' }
  ]), [])
  ...
  const currentDraft = draftOrders[selectedTable] || { table_name: selectedTable, customer_name: '', payment_method: paymentMethod, items: [] }
  ...
  function setCurrentDraft(patchOrFn) {
    setDraftOrders(prev => {
      const current = prev[selectedTable] || { table_name: selectedTable, customer_name: '', payment_method: paymentMethod, items: [] }
  ...
  async function loadBase() {
    ...
    if (orderError && String(orderError.message || '').includes('pos_orders')) {
      setMessage('Для POS-lite нужно один раз выполнить SQL-файл rms_pos_lite_tables.sql в Supabase.')
      setOrders([])
    } else if (orderError) {
  ...
  function addMenuItemToOrder(item) {
    setCurrentDraft(current => {
      ...
      return { ...current, payment_method: current.payment_method || paymentMethod, items: nextItems }
    })
  }
  ...
  function clearCurrentTable() {
    setDraftOrders(prev => ({ ...prev, [selectedTable]: { table_name: selectedTable, customer_name: '', payment_method, items: [] } }))
  }
  ...
  async function closeOrder() {
    ...
    try {
      ...
      const revenuePayload = {
        ...
        comment: `POS-lite · ${items.length} поз. · ${fmt(totalQty)} шт.${selectedTable ? ` · ${selectedTable}` : ''}`,
      ...
      const { data: order, error: orderError } = await supabase.from('pos_orders').insert({
        branch_id: branchId,
        order_date: date,
        table_name: selectedTable || currentDraft.table_name || null,
      ...
      setDraftOrders(prev => ({ ...prev, [selectedTable]: { table_name: selectedTable, customer_name: '', payment_method: activePayment, items: [] } }))
      await loadBase()
      setMessage('Заказ закрыт. Продажа отправлена в RMS.')
    } catch (error) {
      setMessage(error?.message || 'Не удалось закрыть заказ')
    } finally {
      setLoading(false)
    }
  }
  ...
  function tableDraftSummary(tableId) {
    ...
    return { qty, total, count: draftItems.length }
  }
  ...
  return (
    <section>
      <section className="topbar">
        <div>
          <h2>POS / Продажи</h2>
          <p>Интерфейс в стиле POS-панели: выбор стола плитками и добавление блюд из меню ячейками.</p>
        </div>
      </section>
      ...
      <div className="grid">
        <div className="card span-2">
          <div className="card-head">
            <div><h3>Рабочая смена</h3><p className="hint">Выбери филиал, дату и стол. Затем добавляй позиции из меню справа.</p></div>
          </div>
          <div className="form-grid compact">
            ...
            <label><span>Поиск по меню</span><input value={search} onChange={e => setSearch(e.target.value)} placeholder="кофе, бургер, салат..." /></label>
          </div>

          <div style={{display:'flex', gap:8, flexWrap:'wrap', marginTop:12}}>
            <button className={menuTypeFilter === 'all' ? 'active' : ''} onClick={() => { setMenuTypeFilter('all'); setCategoryFilter('all') }}>Бар + Кухня</button>
            <button className={menuTypeFilter === 'Бар' ? 'active' : ''} onClick={() => { setMenuTypeFilter('Бар'); setCategoryFilter('all') }}>Бар</button>
            <button className={menuTypeFilter === 'Кухня' ? 'active' : ''} onClick={() => { setMenuTypeFilter('Кухня'); setCategoryFilter('all') }}>Кухня</button>
          </div>

          <div style={{display:'flex', gap:8, flexWrap:'wrap', marginTop:10}}>
            <button className={categoryFilter === 'all' ? 'active' : ''} onClick={() => setCategoryFilter('all')}>Все категории</button>
            {visibleCategories.map(cat => <button key={cat} className={categoryFilter === cat ? 'active' : ''} onClick={() => setCategoryFilter(cat)}>{cat}</button>)}
          </div>

          <div style={{marginTop:14}}>
            <h3 style={{marginBottom:10}}>Столы / заказы</h3>
            <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(120px, 1fr))', gap:10}}>
              {tableButtons.map(table => {
                const summary = tableDraftSummary(table.id)
                const active = selectedTable === table.id
                return <button key={table.id} onClick={() => setSelectedTable(table.id)} style={{border: active ? '2px solid #1f6f43' : '1px solid #d7dfd9', background: active ? '#eef7f0' : '#fff', borderRadius:14, padding:'12px 10px', textAlign:'left', cursor:'pointer', minHeight:88}}>
                  <div style={{fontWeight:800, marginBottom:8}}>{table.label}</div>
                  <div className="hint">позиций: {summary.count}</div>
                  <div className="hint">кол-во: {fmt(summary.qty)}</div>
        ...
        <div className="card">
          <div className="card-head">
            <div><h3>Текущий стол: {selectedTable}</h3><p className="hint">Позиции добавляются нажатием по ячейке меню.</p></div>
            <button className="ghost small" onClick={clearCurrentTable}>Очистить</button>
          </div>
          <div className="form-grid compact">
            <label><span>Гость / комментарий</span><input value={currentDraft.customer_name || ''} onChange={e => setCurrentDraft({ customer_name: e.target.value })} placeholder="Необязательно" /></label>
          ...
          <div className="table-wrap" style={{maxHeight:440, overflow:'auto', marginTop:12}}>
            <table>
              ...
              <tbody>
                {items.map((item, idx) => <tr key={`${item.menu_item_id}-${idx}`}>
                  ...
                  <td><input style={{width:70}} inputMode="decimal" value={item.qty} onChange={e => updateItem(idx, { qty: e.target.value })} /></td>
                  <td><input style={{width:86}} inputMode="decimal" value={item.price} onChange={e => updateItem(idx, { price: e.target.value })} /></td>
                ...
                {!items.length && <tr><td colSpan="6" className="hint">Заказ пуст. Выбери стол и нажми на позиции в меню.</td></tr>}
              </tbody>
            </table>
          </div>
          ...
          <button className="primary" disabled={loading || !items.length} onClick={closeOrder} style={{marginTop:12, width:'100%'}}>{loading ? 'Сохранение...' : 'Закрыть заказ и отправить в RMS'}</button>
        </div>

        <div className="card span-2">
          <h3>Меню</h3>
          <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(180px, 1fr))', gap:10}}>
            {filteredMenu.map(item => <button key={item.id} onClick={() => addMenuItemToOrder(item)} style={{border:'1px solid #d7dfd9', background:'#fff', borderRadius:14, padding:'12px 12px', textAlign:'left', cursor:'pointer', minHeight:96}}>
            ...
            {!filteredMenu.length && <div className="hint">Нет позиций в техкартах / меню для текущего фильтра.</div>}
          </div>
        </div>

        <div className="card span-2">
          <h3>Закрытые POS-заказы за выбранную дату</h3>
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                ...
                {!orders.length && <tr><td colSpan="6" className="hint">Заказов пока нет.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
