...
async function rmsCalculateNetworkForecastForMonth(y, m, scopeBranchId = 'all') {
  ...
  const rentForecast = configuredRent > 0 ? configuredRent : (currentExpenseGroups.rent > 0 ? currentExpenseGroups.rent : historicalAvg('rent'))

  const payrollDetails = rmsFinancePayrollDetailsForScope(employeeRows || [], selectedBranchId, revenueShareMap)
  const salaryForecast = parseNum(payrollDetails.totalSalary)
  ...
  const foodForecast = currentFoodAll > 0 ? currentFoodAll * dailyFactor : historicalAvg('food_all')
  const packagingForecast = currentPackagingAll > 0 ? currentPackagingAll : historicalAvg('packaging_all')
  const householdForecast = currentHouseholdAll > 0 ? currentHouseholdAll : historicalAvg('household_all')
  const otherSupplierForecast = currentSupplierTotals.other > 0 ? currentSupplierTotals.other : historicalAvg('other_supplier')
  const utilitiesForecast = currentExpenseGroups.utilities > 0 ? currentExpenseGroups.utilities : historicalAvg('utilities')
  const marketingForecast = currentExpenseGroups.marketing > 0 ? currentExpenseGroups.marketing : historicalAvg('marketing')
  const maintenanceForecast = currentExpenseGroups.maintenance > 0 ? currentExpenseGroups.maintenance : historicalAvg('maintenance')
  const otherForecast = currentExpenseGroups.other > 0 ? currentExpenseGroups.other : historicalAvg('other')
  ...
  const details = [
    ...
    { name: 'Food Cost / закупки и базар', amount: foodForecast, note: currentFoodAll > 0 ? 'текущий месяц × темп дней' : 'среднее прошлых месяцев' },
    { name: 'Зарплаты', amount: salaryForecast, note: 'полный месячный фонд' },
    { name: 'Аренда', amount: rentForecast, note: configuredRent > 0 ? 'из настроек филиалов' : currentExpenseGroups.rent > 0 ? 'из текущего месяца' : 'среднее прошлых месяцев' },
    ...
    { name: 'DSMF', amount: dsmfForecast, note: 'по официальному фонду зарплаты' },
    { name: 'Коммунальные', amount: utilitiesForecast, note: currentExpenseGroups.utilities > 0 ? 'из текущего месяца' : 'среднее прошлых месяцев' },
    { name: 'Take away / packaging', amount: packagingForecast, note: currentPackagingAll > 0 ? 'из текущего месяца' : 'среднее прошлых месяцев' },
    { name: 'Хозтовары', amount: householdForecast, note: currentHouseholdAll > 0 ? 'из текущего месяца' : 'среднее прошлых месяцев' },
    { name: 'Маркетинг', amount: marketingForecast, note: currentExpenseGroups.marketing > 0 ? 'из текущего месяца' : 'среднее прошлых месяцев' },
    { name: 'Ремонт / обслуживание', amount: maintenanceForecast, note: currentExpenseGroups.maintenance > 0 ? 'из текущего месяца' : 'среднее прошлых месяцев' },
    { name: 'Прочее', amount: otherForecast + otherSupplierForecast, note: 'текущий месяц или среднее прошлых месяцев' }
