...
async function rmsCalculateNetworkForecastForMonth(y, m, scopeBranchId = 'all') {
  ...
  const salarySummaryTotal = (directByBranch = new Map(), managersTotal = 0, selectedId = selectedBranchId) => {
    ...
    const managerShare = parseNum(revenueShareMap.get(selectedId))
  ...
  const salaryHistoryByMonth = prevMonths.map(pm => {
    const periodValue = salarySummaryFromPeriods((historicalSalaryPeriodRows || []).filter(r => r.salary_month >= pm.start && r.salary_month < pm.end), selectedBranchId)
  ...
  const activeEmployeeSalary = salarySummaryFromEmployees(employeeRows || [], selectedBranchId)
  const currentPeriodSalary = salarySummaryFromPeriods(currentSalaryPeriodRows || [], selectedBranchId)
  ...
  const salaryForecast = payrollSummarySalary || activeEmployeeSalary || currentMonthlySalary || currentPeriodSalary || currentExpenseSalary || salaryHistory
  const salaryForecastNote = payrollSummarySalary > 0
    ? `из “Зарплаты → Сотрудники → Сводка по филиалам”: зарплата филиала + доля менеджеров по выручке${payrollManagersSalary > 0 ? ` · менеджеры ${fmt(payrollManagersSalary)} AZN` : ''}; группа “Менеджеры” остаётся отдельно и не суммируется дважды`
    : activeEmployeeSalary > 0
      ? 'из активных сотрудников: зарплата филиала + доля менеджеров по выручке; менеджеры не суммируются дважды'
      : currentMonthlySalary > 0
        ? 'из месячного расчёта зарплат за текущий месяц'
        : currentPeriodSalary > 0
          ? 'из зарплатных периодов текущего месяца'
