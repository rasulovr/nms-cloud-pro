...
function ProductLogo({ compact = false, login = false }) {
  ...
  if (customLogo) {
    const wrapStyle = login
      ? { width: 220, height: 120, margin: '0 auto 14px', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }
      : compact
        ? { width: 50, height: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flex: '0 0 auto' }
        : { width: 120, height: 70, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }
...
function Finance({ t, lang }) {
  ...
  async function load() {
    ...
    if (branchId === ALL_BRANCHES) {
      const networkAiRows = buildAiForBranch({
      ...
      const singleRows = buildAiForBranch({
  ...
  function handleCustomLogoSelect(event) {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        ctx.drawImage(img, Math.round((maxW - drawW) / 2), Math.round((maxH - drawH) / 2), drawW, drawH)
        setCustomLogoPreview(canvas.toDataURL('image/png'))
        setMsg('Лого автоматически сжато под размер входного меню. Нажмите “Сохранить лого”.')
      }
      img.onerror = () => {
