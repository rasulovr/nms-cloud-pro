...
class RMSModuleErrorBoundary extends React.Component {
  constructor(props) {
    super(props)
  ...
  componentDidCatch(error, info) {
    console.error('RMS module error:', error, info)
  }
    ...
    if (this.state.error) {
      return (
        <section className="card">
          ...
          <p className="hint">Ошибка модуля: {this.state.error?.message || 'unknown error'}</p>
          <p className="hint">Проверьте, что файлы модуля и SQL-таблицы добавлены корректно.</p>
        </section>
      )
    }
  ...
  return (
    <div className={`app theme-${theme || 'classic'}`}>
      ...
      <main className={`main ${currentAccess === 'read' ? 'readonly-mode' : ''}`}>
        ...
        {canReadAccess(currentAccess) && section === 'loyalty' && (
          <RMSModuleErrorBoundary>
            <RMSLoyalty t={t} />
          </RMSModuleErrorBoundary>
        )}
