...
function DashboardStyles() {
  return <style>{`
    ...
    .supplier-risk-row { display: grid; grid-template-columns: minmax(120px, 1fr) 110px minmax(160px, 1.5fr); gap: 10px; align-items: center; padding: 9px 10px; border: 1px solid rgba(155,45,45,.18); border-radius: 12px; background: rgba(155,45,45,.05); }
    .supplier-risk-row span { text-align: right; font-weight: 900; color: var(--danger); }
    .supplier-risk-row em { font-style: normal; color: var(--muted); font-size: 12px; }
...
function Dashboard({ t }) {
  ...
  async function calcMonth(y, m) {
    ...
    const supplierDebtRows = (enrichedSupplierRows || [])
      .map(r => {
        ...
        const overdueInvoices = termDays > 0 && balance > 0
          ...
          : []
        const reason = [
          overLimit > 0 ? `лимит +${fmt(overLimit)}` : '',
  ...
  const debtRows = data.supplierDebtRows?.length ? data.supplierDebtRows : [{ name: 'Нет проблемных долгов', value: 0 }]
...
function Suppliers({ t }) {
  ...
  const emptyLine = { category: PRODUCT_CATEGORIES[0], product_id: '', product_query: '', base_unit: 'g', quantity: '1', unit: 'kg', unit_price: '' }
  ...
  function productLabel(product) {
    if (!product) return ''
    return `${product.category || 'Прочее'} · ${product.name}`
  }

  function productOptionsForRow(row) {
    const q = String(row.product_query || '').toLowerCase().trim()
    return products
      .filter(p => !row.category || (p.category || 'Прочее') === row.category)
      .filter(p => !q || productLabel(p).toLowerCase().includes(q) || String(p.name || '').toLowerCase().includes(q))
      .slice(0, 80)
  }

  function selectProductForLine(index, value) {
    const typed = String(value || '')
    const selected = products.find(p => p.id === typed)
      || products.find(p => productLabel(p).toLowerCase() === typed.toLowerCase())
      || products.find(p => String(p.name || '').toLowerCase() === typed.toLowerCase())

    if (selected) {
      updateLine(index, {
        product_id: selected.id,
        product_query: productLabel(selected),
        category: selected.category || 'Прочее'
      })
    } else {
      updateLine(index, { product_id: '', product_query: typed })
    }
  }

  async function ensureProduct(row) {
    if (row.product_id) return products.find(p => p.id === row.product_id)
    const typed = String(row.product_query || '').trim()
    const typedLower = typed.toLowerCase()
    const exact = products.find(p => productLabel(p).toLowerCase() === typedLower) || products.find(p => String(p.name || '').toLowerCase() === typedLower)
    if (exact) return exact
    throw new Error('Выберите товар из выпадающего списка. Если товара нет, сначала добавьте его в блоке “Товары”.')
  }
  ...
  async function addPurchase() {
    ...
    try {
      ...
      for (const row of lineRows) {
        const hasData = row.product_id || String(row.product_query || '').trim() || parseNum(row.quantity) || parseNum(row.unit_price)
        if (!hasData) continue
    ...
    <section>
      ...
      <section className="grid">
        ...
        <div className="card span-2">
          ...
          <br />
          <div className="card-head"><div><h3>Товары в закупке</h3><p className="hint">В графе “Товар” начните вводить название и выберите товар из выпадающего списка. Если товара нет, сначала добавьте его в блоке “Товары”.</p></div><button className="small" onClick={() => setLineRows(rows => [...rows, { ...emptyLine }])}>+ Строка товара</button></div>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Категория</th><th>Товар</th><th>Кол-во закупа</th><th>Ед. закупа</th><th>Цена за ед.</th><th>Сумма</th><th></th></tr></thead>
              <tbody>
                    ...
                    <tr key={idx}>
                      ...
                      <td style={{minWidth:300}}>
                        <input
                          ...
                          onBlur={e => selectProductForLine(idx, e.target.value)}
                          placeholder="Начните вводить товар..."
                        />
                        <datalist id={`purchase-products-${idx}`}>
                          {productOptionsForRow(row).map(p => <option key={p.id} value={productLabel(p)} />)}
                        </datalist>
                      </td>
