...
function Revenue({ t }) {
  ...
  async function addRevenueEntry() {
    ...
    if (getInternalSessionStorage()?.rms_internal) {
      ...
      setForm({ cash_amount: '', bank_amount: '', wolt_amount: '', comment: '' })
      await recalcExistingBazarExpenseForDate(date)
      await load()
      ...
      {tab === 'semis' && (
        <section className="grid semifinished-layout">
          ...
          <div className="card semi-composition-card">
            ...
            <div className="table-wrap">
              <table>
                <thead><tr><th>Компонент</th><th>Кол-во</th><th>Себес.</th></tr></thead>
                <tbody>
                  {selectedSemiItems.map(item => (
                    ...
                    >
                      <td>
                        ...
                        <div className="semi-sub-value">
                          Цена: {item.component_type === 'manual' ? fmt(item.manual_unit_cost || 0) : 'закупка'} · Потери: {fmt(item.waste_percent || 0)}%
                        </div>
                      </td>
...
function SemiFinishedInlineStyles() {
  return (
    <style>{`
      ...
      .semifinished-layout {
        grid-template-columns: minmax(250px, .8fr) minmax(280px, .8fr) minmax(430px, 1.3fr);
        align-items: start;
      }
      ...
      .semi-inline-editor .semi-modal-table {
      ...
      .semi-inline-editor .semi-modal-table input,
      .semi-inline-editor .semi-modal-table select {
        min-width: 110px !important;
        height: 42px;
        font-size: 14px;
      }
      .semi-inline-editor .semi-modal-table td:first-child input {
