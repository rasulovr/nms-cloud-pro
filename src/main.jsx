...
function Salaries({ t }) {
  ...
  function dsmfBaseForRow(row) {
    const officialDays = parseNum(dsmfOfficialDays) || DAILY_DIVISOR
  ...
  function dsmfLineForRow(row) {
    ...
    return {
      ...
      officialDays: parseNum(dsmfOfficialDays) || DAILY_DIVISOR
    }
  }
  ...
  function saveDsmfRates() {
    ...
    setMessage('Проценты и официальные рабочие дни DSMF сохранены')
  }

  async function saveAllDsmfExpenses() {
    setMessage('')
    if (!dsmfGroups.length || !dsmfTotal) return setMessage('Нет данных для DSMF')
    const payload = dsmfGroups
      ...
      .map(g => ({
        ...
        comment: `DSMF автоматически · ${g.name} · сотрудников ${g.rows.length} · оф. дней ${parseNum(dsmfOfficialDays) || DAILY_DIVISOR}`
      }))
    const { error } = await supabase.from('daily_expenses').insert(payload)
    if (error) return setMessage(error.message)
    setMessage(`DSMF автоматически добавлен в расходы: ${fmt(dsmfTotal)} AZN`)
    load()
  }
      ...
      <div className="card span-2">
        <div className="card-head">
          <div>
            ...
            <p className="hint">Система автоматически считает DSMF по сотрудникам, группирует по филиалам и добавляет итог филиала отдельной статьёй расходов.</p>
          </div>
          <button className="small primary" onClick={saveAllDsmfExpenses}>Сохранить DSMF всех филиалов в расходы</button>
        </div>
        ...
        <div className="action-row" style={{marginBottom:12}}>
          <button className="ghost small" onClick={saveDsmfRates}>Сохранить проценты и дни</button>
          <span className="hint">Итого DSMF за месяц: <b>{fmt(dsmfTotal)}</b> AZN</span>
        </div>

        {dsmfGroups.map(group => {
          const expanded = Boolean(expandedDsmfGroups[group.id])
          const shownRows = expanded ? group.rows : group.rows.slice(0, 2)
        ...
        <div className="table-wrap" style={{marginTop:12}}>
          <table><thead><tr><th>Дата</th><th>Филиал</th><th>Сумма</th><th>Комментарий</th></tr></thead><tbody>{dsmfRows.map(r => <tr key={r.id}><td>{r.expense_date}</td><td>{r.branches?.name || '—'}</td><td><b>{fmt(r.amount)}</b></td><td>{r.comment || '—'}</td></tr>)}{!dsmfRows.length && <tr><td colSpan="4" className="hint">DSMF ещё не сохранён в расходы за выбранный месяц.</td></tr>}</tbody></table>
        </div>
      </div>
