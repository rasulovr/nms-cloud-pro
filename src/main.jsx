...
const RMS_INTERNAL_USERS_KEY = 'rms_internal_users_v1'
const RMS_INTERNAL_PERMISSIONS_KEY = 'rms_internal_permissions_v1'
const RMS_INTERNAL_SESSION_KEY = 'rms_internal_session_v1'

const normalizeInternalLogin = (value) => String(value || '').trim().toLowerCase().replace(/@(rms|nms)\.local\.az$/, '')
const getInternalUsers = () => {
  try { return JSON.parse(localStorage.getItem(RMS_INTERNAL_USERS_KEY) || '{}') || {} } catch (_e) { return {} }
}
const setInternalUsers = (users) => {
  try { localStorage.setItem(RMS_INTERNAL_USERS_KEY, JSON.stringify(users || {})) } catch (_e) {}
}
const getInternalPermissions = () => {
  try { return JSON.parse(localStorage.getItem(RMS_INTERNAL_PERMISSIONS_KEY) || '{}') || {} } catch (_e) { return {} }
}
const setInternalPermissions = (permissions) => {
  try { localStorage.setItem(RMS_INTERNAL_PERMISSIONS_KEY, JSON.stringify(permissions || {})) } catch (_e) {}
}
...
const setInternalSessionStorage = (session) => {
  try {
    if (session) localStorage.setItem(RMS_INTERNAL_SESSION_KEY, JSON.stringify(session))
    else localStorage.removeItem(RMS_INTERNAL_SESSION_KEY)
  } catch (_e) {}
}
const getInternalSessionStorage = () => {
  try { return JSON.parse(localStorage.getItem(RMS_INTERNAL_SESSION_KEY) || 'null') } catch (_e) { return null }
}

const RMS_SHARED_CACHE_PREFIX = 'rms_shared_cache_v1_'
const setSharedCache = (key, value) => {
  try { localStorage.setItem(`${RMS_SHARED_CACHE_PREFIX}${key}`, JSON.stringify(value || [])) } catch (_e) {}
}
const getSharedCache = (key) => {
  try { return JSON.parse(localStorage.getItem(`${RMS_SHARED_CACHE_PREFIX}${key}`) || '[]') || [] } catch (_e) { return [] }
}
const currentInternalSession = () => getInternalSessionStorage()
const currentInternalProfile = () => {
  const sess = currentInternalSession()
  if (!sess?.rms_internal?.valueOf && !sess?.rms_internal) return null
  ...
  return {
    ...
    hide_manager_salary: Boolean(localUser.hide_manager_salary),
    ui_theme: localUser.ui_theme || 'classic',
    rms_internal: true
  }
}
const cacheEmployees = (rows) => { if (Array.isArray(rows) && rows.length) setSharedCache('employees', rows) }
const cachedEmployees = () => getSharedCache('employees')
const cacheSalaryDataset = (key, rows) => { if (Array.isArray(rows)) setSharedCache(key, rows) }
const cachedSalaryDataset = (key) => getSharedCache(key)
...
function App() {
  ...
  useEffect(() => {
    const storedInternalSession = getInternalSessionStorage()
    if (storedInternalSession?.rms_internal) {
      setSession(storedInternalSession)
      setLoading(false)
      return
    }
  ...
  useEffect(() => {
    async function loadProfile() {
      ...
      const loginName = normalizeInternalLogin(session.user.login_name || session.user.email)
      let prof = null
      let perms = []
      if (session?.rms_internal) {
        const internalUsers = getInternalUsers()
        const localUser = internalUsers[loginName] || Object.values(internalUsers).find(u => u?.id === userId)
        prof = {
          id: userId,
          login_name: loginName,
          email: `${loginName}@rms.internal`,
          full_name: localUser?.full_name || session.user.full_name || loginName,
          role: 'employee',
          is_active: localUser?.is_active !== false,
          hide_manager_salary: Boolean(localUser?.hide_manager_salary),
          ui_theme: localUser?.ui_theme || theme || 'classic'
        }
        perms = makeInternalPermissionRows(userId)
      } else {
        const { data: byId } = await supabase.from('user_profiles').select('*').eq('id', userId).maybeSingle()
        prof = byId
        if (!prof && loginName) {
          const { data: byLogin } = await supabase.from('user_profiles').select('*').eq('login_name', loginName).maybeSingle()
          prof = byLogin
        }
        if (prof?.id) {
          const { data: permRows } = await supabase.from('user_permissions').select('*').eq('user_id', prof.id)
          perms = permRows || []
        }
      }
      setProfile(prof)
  ...
  const canViewCurrentSection = canReadAccess(currentAccess)
  ...
  if (!session) return <Login lang={lang} setLang={setLang} t={t} onInternalLogin={(nextSession) => { setInternalSessionStorage(nextSession); setSession(nextSession) }} />
    ...
    <div className={`app theme-${theme || 'classic'}`}>
      <aside className="sidebar">
        ...
        <div className="userbar">
          ...
          <button className="ghost small" onClick={() => { setInternalSessionStorage(null); setSession(null); setProfile(null); setPermissions([]) }}>{t('logout')}</button>
        </div>
      </aside>
      ...
      <main className={`main ${currentAccess === 'read' ? 'readonly-mode' : ''}`}>
        ...
        {!canViewCurrentSection && <div className="card"><h3>{t('permission_denied')}</h3><p className="hint">Раздел скрыт для текущего пользователя.</p></div>}
        {canViewCurrentSection && section === 'dashboard' && <Dashboard t={t} />}
        {canViewCurrentSection && section === 'revenue' && <Revenue t={t} />}
        {canViewCurrentSection && section === 'finance' && <Finance t={t} lang={lang} />}
        {canViewCurrentSection && section === 'recipes' && <Recipes t={t} />}
        {canViewCurrentSection && section === 'salaries' && <SalaryWorkspace t={t} />}
        {canViewCurrentSection && section === 'suppliers' && <Suppliers t={t} />}
        {canViewCurrentSection && section === 'debts' && <DebtsPayments t={t} />}
        {canViewCurrentSection && section === 'settings' && <Settings session={session} t={t} theme={theme} setTheme={setTheme} />}
      </main>
    </div>
  )
}
...
function SalaryWorkspace({ t }) {
  ...
  return (
    <section>
      ...
      {salaryTab === 'employees' && <>
        <Salaries t={t} view="employees" />
        <Attendance t={t} mode="staff" />
      </>}
      {salaryTab === 'attendance' && <Attendance t={t} mode="attendance" />}
      {salaryTab === 'advances' && <Advances t={t} />}
      {salaryTab === 'dsmf' && <Salaries t={t} view="dsmf" />}
    </section>
  )
}
...
function Login({ lang, setLang, t, onInternalLogin }) {
  ...
  async function signIn() {
    ...
    const internalLogin = normalizeInternalLogin(rawLogin)
    const internalUsers = getInternalUsers()
    const localUser = internalUsers[internalLogin]

    if (localUser && String(localUser.password || '') === String(password || '')) {
      if (localUser?.is_active === false) return setError('Пользователь отключён администратором')
      const nextSession = {
        rms_internal: true,
        user: {
          id: localUser.id,
          email: `${internalLogin}@rms.internal`,
          login_name: internalLogin,
          full_name: localUser.full_name || internalLogin
        }
      }
      onInternalLogin?.(nextSession)
      return
    }

    const emails = normalizeLoginCandidates(rawLogin)
  ...
  return <div className="login-screen"><div className="login-card">
    ...
    <p>Вход администратора через Supabase Auth. Внутренние пользователи входят по login, созданному в настройках RMS.</p>
    ...
    <label><span>{t('password_label')}</span><input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password" onKeyDown={e => { if (e.key === 'Enter') signIn() }} /></label>
...
function useBranches() {
  ...
  useEffect(() => {
    supabase.from('branches').select('*').eq('is_active', true).order('name').then(({ data, error }) => {
      if (!error && Array.isArray(data) && data.length) {
        setSharedCache('branches', data)
        setBranches(data)
      } else {
        setBranches(getSharedCache('branches'))
      }
    })
  }, [])
  return branches
...
function Attendance({ t, mode = 'attendance' }) {
  const branches = useBranches()
  ...
  async function loadSalaryPrivacyProfile() {
    const internalProfile = currentInternalProfile()
    if (internalProfile) {
      setSalaryPrivacyProfile(internalProfile)
      return
    }
  ...
  useEffect(() => { load() }, [year, month, branchId, positionFilter, salaryPrivacyProfile?.hide_manager_salary])
  ...
  async function load() {
    ...
    const employeeSource = empError || !(emp || []).length ? cachedEmployees() : (emp || [])
    if (!empError && (emp || []).length) cacheEmployees(emp || [])
    if (!attError && Array.isArray(att)) cacheSalaryDataset(`attendance_${year}_${month}`, att || [])
    const attendanceSource = attError ? cachedSalaryDataset(`attendance_${year}_${month}`) : (att || [])
    const visibleEmployees = (employeeSource || [])
      .filter(e => !shouldHideManagerSalaries || !isManagerStaff(e))
...
function Salaries({ t, view = 'employees' }) {
  const branches = useBranches()
  ...
  async function loadSalaryPrivacyProfile() {
    ...
    if (profById) {
      setSalaryPrivacyProfile(profById)
      return
    }
  ...
  async function load() {
    ...
    if (!empError && Array.isArray(emp) && emp.length) cacheEmployees(emp)
    if (!salError && Array.isArray(sal)) cacheSalaryDataset(`salary_periods_${year}_${month}`, sal)
    if (!advError && Array.isArray(adv)) cacheSalaryDataset(`salary_advances_${year}_${month}`, adv)
    if (!payError && Array.isArray(pays)) cacheSalaryDataset(`salary_payments_${year}_${month}`, pays)
    if (!previousDisplayPayError && Array.isArray(previousDisplayPays)) cacheSalaryDataset(`salary_payments_prev_${year}_${month}`, previousDisplayPays)
    if (!dsmfError && Array.isArray(dsmf)) cacheSalaryDataset(`dsmf_${year}_${month}`, dsmf)
    if (!prevSalError && Array.isArray(prevSal)) cacheSalaryDataset(`prev_salary_periods_${year}_${month}`, prevSal)
    if (!prevPayError && Array.isArray(prevPays)) cacheSalaryDataset(`prev_salary_payments_${year}_${month}`, prevPays)

    const empRows = empError || !(emp || []).length ? cachedEmployees() : (emp || [])
    const salRows = salError ? cachedSalaryDataset(`salary_periods_${year}_${month}`) : (sal || [])
    const advRows = advError ? cachedSalaryDataset(`salary_advances_${year}_${month}`) : (adv || [])
    const payRows = payError ? cachedSalaryDataset(`salary_payments_${year}_${month}`) : (pays || [])
    const previousDisplayPayRows = previousDisplayPayError ? cachedSalaryDataset(`salary_payments_prev_${year}_${month}`) : (previousDisplayPays || [])
    const dsmfRowsSource = dsmfError ? cachedSalaryDataset(`dsmf_${year}_${month}`) : (dsmf || [])
    const prevSalRows = prevSalError ? cachedSalaryDataset(`prev_salary_periods_${year}_${month}`) : (prevSal || [])
    const prevPayRows = prevPayError ? cachedSalaryDataset(`prev_salary_payments_${year}_${month}`) : (prevPays || [])

    if (empError && !empRows.length) {
      setMessage(`Нет доступа к таблице сотрудников для внутреннего пользователя: ${empError.message}. Зайдите администратором один раз в раздел зарплат, чтобы обновить локальный кэш.`)
    } else if (empError || salError || advError || payError || previousDisplayPayError || dsmfError || prevSalError || prevPayError) {
      setMessage('Данные открыты из локального RMS-кэша. Для полного обновления зайдите администратором и откройте этот раздел.')
    }
    ...
    const mappedRows = (empRows || []).filter(e => matchesStaffGroup(e, branchId)).filter(e => matchesPositionGroup(e, positionFilter)).map(e => {
      ...
      // If salary_period does not exist for the month, show 0 accrued.
      // This prevents old employee monthly rates from appearing as operational salary after cleanup.
    ...
    const visibleMappedRows = shouldHideManagerSalaries
      ? mappedRows.filter(r => !isManagerStaff(r.employees))
      : mappedRows
    ...
    setDsmfRows(dsmfRowsSource || [])
    if (!paymentForm.employee_id && visibleMappedRows[0]) setPaymentForm(f => ({ ...f, employee_id: visibleMappedRows[0].employee_id }))
    if (shouldHideManagerSalaries && paymentForm.employee_id && !visibleEmployeeIds.has(paymentForm.employee_id) && visibleMappedRows[0]) {
      setPaymentForm(f => ({ ...f, employee_id: visibleMappedRows[0].employee_id }))
    }
  ...
  return <section className={`salary-section salary-view-${view}`}>
...
function Settings({ session, t, theme, setTheme }) {
  ...
  async function load() {
    ...
    const localUserRows = Object.values(localUsers).map(u => ({
      id: u.id,
      ...
      hide_manager_salary: Boolean(u.hide_manager_salary),
      ui_theme: u.ui_theme || theme || 'classic',
      rms_internal: true,
      created_at: u.created_at || null
    }))
    const dbUsers = (u || []).filter(row => !localUserRows.some(local => local.id === row.id || normalizeInternalLogin(local.login_name) === normalizeInternalLogin(row.login_name || row.email)))
    const localPerms = Object.entries(getInternalPermissions()).flatMap(([user_id, bySection]) => Object.entries(bySection || {}).map(([section, access]) => ({ user_id, section, access })))
    setUsers([...dbUsers, ...localUserRows])
    setLegalEntities(le || [])
    setPermissions([...(perms || []), ...localPerms])
    setBranches(br || [])
  ...
  async function addUser() {
    ...
    const internalUsers = getInternalUsers()
    const existingLocal = internalUsers[login]
    const existingDb = users.find(u => !u.rms_internal && normalizeInternalLogin(u.login_name || u.email) === login)
    if (existingDb) return setMsg('Такой login уже занят Supabase-пользователем. Используйте другой login или удалите старый профиль.')

    const userId = existingLocal?.id || (crypto?.randomUUID ? crypto.randomUUID() : `internal-${Date.now()}-${Math.random()}`)
    internalUsers[login] = {
      ...(existingLocal || {}),
      id: userId,
      login,
      password,
      full_name: newUser.full_name || existingLocal?.full_name || login,
      role: 'employee',
      is_active: existingLocal?.is_active !== false,
      hide_manager_salary: Boolean(existingLocal?.hide_manager_salary),
      ui_theme: existingLocal?.ui_theme || theme || 'classic',
      created_at: existingLocal?.created_at || new Date().toISOString()
    }
    setInternalUsers(internalUsers)

    const allLocalPerms = getInternalPermissions()
    if (!allLocalPerms[userId]) {
      allLocalPerms[userId] = Object.fromEntries(editableSections.map(sec => [sec.id, 'read']))
      setInternalPermissions(allLocalPerms)
    }
  ...
  async function changeUserPassword(userId, loginName) {
    ...
    if (!password || password.length < 6) return setMsg('Пароль должен быть минимум 6 символов')
    const login = normalizeInternalLogin(loginName || users.find(u => u.id === userId)?.login_name || users.find(u => u.id === userId)?.email)
    if (!login) return setMsg('Не найден login пользователя')
    const internalUsers = getInternalUsers()
    internalUsers[login] = {
      ...(internalUsers[login] || {}),
      id: userId,
      login,
      password,
      full_name: users.find(u => u.id === userId)?.full_name || login
    }
    setInternalUsers(internalUsers)
    setPasswordEdits(p => ({ ...p, [userId]: '' }))
    setMsg(`Пароль пользователя ${login} изменён`)
  }
  ...
  async function updateUser(id, patch) {
    ...
    if (row?.rms_internal) {
      const internalUsers = getInternalUsers()
      const login = normalizeInternalLogin(row.login_name || row.email)
      const current = internalUsers[login] || Object.values(internalUsers).find(u => u?.id === id)
      if (!current) return setMsg('Внутренний пользователь не найден')
      const nextUser = { ...current, ...patch }
      const oldKey = Object.keys(internalUsers).find(key => internalUsers[key]?.id === id) || login
      delete internalUsers[oldKey]
      internalUsers[normalizeInternalLogin(nextUser.login || login)] = nextUser
      setInternalUsers(internalUsers)
      setMsg(t('saved'))
      await load()
      return
  ...
  async function updatePermission(userId, section, access) {
    ...
    if (row?.rms_internal) {
      ...
      if (access === 'none') delete current[section]
      else current[section] = access
      all[userId] = current
      setInternalPermissions(all)
      try { window.dispatchEvent(new Event('rms-user-settings-updated')) } catch (_e) {}
      setMsg(t('saved'))
      await load()
      return
    }
    ...
    let error = null
    if (access === 'none') {
      const res = await supabase.from('user_permissions').delete().eq('user_id', userId).eq('section', section)
      error = res.error
    } else {
      const res = await supabase.from('user_permissions').upsert({ user_id: userId, section, access }, { onConflict: 'user_id,section' })
      error = res.error
    }
  ...
  async function deleteUser(user) {
    setMsg('')
    if (!user?.id) return setMsg('Пользователь не выбран')
    if (user.id === session?.user?.id) return setMsg('Нельзя удалить текущего пользователя, под которым вы вошли')
    const login = normalizeInternalLogin(user.login_name || user.email || user.full_name || user.id)
    const ok = window.confirm(`Удалить пользователя ${login}? Его права доступа, профиль и внутренний пароль будут удалены.`)
    if (!ok) return

    if (!user.rms_internal) {
      const { error: permError } = await supabase.from('user_permissions').delete().eq('user_id', user.id)
    ...
    const internalUsers = getInternalUsers()
    delete internalUsers[login]
    Object.keys(internalUsers).forEach(key => {
      if (internalUsers[key]?.id === user.id) delete internalUsers[key]
    })
    setInternalUsers(internalUsers)
    const allPerms = getInternalPermissions()
    delete allPerms[user.id]
    setInternalPermissions(allPerms)
    setPasswordEdits(p => { const next = { ...p }; delete next[user.id]; return next })
    try { window.dispatchEvent(new Event('rms-user-settings-updated')) } catch (_e) {}
    setMsg(`Пользователь ${login} удалён. Теперь его можно создать заново.`)
    await load()
  }
