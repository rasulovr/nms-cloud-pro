...
function Suppliers({ t }) {
  ...
  const [lineForm, setLineForm] = useState({ product_id: '', quantity: '1', unit: 'kg', unit_price: '' })
  ...
  useEffect(() => {
    ...
    if (!lineForm.product_id && products[0]) setLineForm(f => ({ ...f, product_id: products[0].id }))
  }, [suppliers, legalEntities, branches, products])

  async function load() {
    const [{ data: le }, { data: sup }, { data: prod }, { data: bal }, { data: pur }] = await Promise.all([
      ...
      supabase.from('supplier_purchases').select('*, suppliers(name), legal_entities(name,voen), branches(name), supplier_purchase_items(*, supplier_products(name,base_unit,category))').order('purchase_date', { ascending: false }).limit(30)
    ])
    setLegalEntities(le || [])
    setSuppliers(sup || [])
    setProducts(prod || [])
  ...
  async function addPurchase() {
    setMessage('')
    if (!purchaseForm.supplier_id || !purchaseForm.legal_entity_id) return setMessage('Выберите поставщика и VOEN')
    if (!lineForm.product_id) return setMessage('Выберите товар')
    const product = products.find(p => p.id === lineForm.product_id)
    const quantity = parseNum(lineForm.quantity)
    const unitPrice = parseNum(lineForm.unit_price)
    const total = quantity * unitPrice
    const baseQty = convertToBase(quantity, lineForm.unit, product?.base_unit)
    if (!quantity || !unitPrice || !baseQty) return setMessage('Введите количество и цену')
    ...
    await load()
    setLineForm({ ...lineForm, quantity: '1', unit_price: '' })
    setMessage(t('saved'))
  }
  ...
  async function addPayment(supplierId) {
    ...
    if (!amount) return
    const { error } = await supabase.from('supplier_payments').insert({ supplier_id: supplierId, payment_date: todayISO(), amount })
    if (error) setMessage(error.message)
    ...
    <section>
      ...
      <section className="grid">
        ...
        <div className="card span-2">
          ...
          <button className="small" onClick={addProduct}>+ Добавить товар</button>
        </div>

        <div className="card span-2">
          <div className="card-head"><div><h3>Новая закупка</h3><p className="hint">Пока одна строка товара на одну фактуру. Следующим шагом можно добавить несколько строк в одну фактуру.</p></div></div>
          <div className="form-grid compact">
            ...
            <label><span>Товар</span><select value={lineForm.product_id} onChange={e => setLineForm({...lineForm, product_id: e.target.value})}>{products.map(p => <option key={p.id} value={p.id}>{p.category} · {p.name}</option>)}</select></label>
            <label><span>Кол-во закупа</span><input inputMode="decimal" value={lineForm.quantity} onChange={e => setLineForm({...lineForm, quantity: e.target.value})} /></label>
            <label><span>Ед. закупа</span><select value={lineForm.unit} onChange={e => setLineForm({...lineForm, unit: e.target.value})}>{PURCHASE_UNITS.map(u => <option key={u.value} value={u.value}>{u.label}</option>)}</select></label>
            <label><span>Цена за ед. закупа</span><input inputMode="decimal" value={lineForm.unit_price} onChange={e => setLineForm({...lineForm, unit_price: e.target.value})} /></label>
          </div>
          <p className="hint">Пример: молоко — 1 l × 2.20 AZN. Для техкарты система получит 0.0022 AZN/ml. Кофе — 1 kg × 37 AZN = 0.037 AZN/g.</p>
        ...
        <div className="card span-2">
          <div className="card-head"><div><h3>Последние закупки</h3></div></div>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Дата</th><th>Фактура</th><th>Поставщик</th><th>Наш VOEN</th><th>Филиал</th><th>Товары</th><th>Сумма</th></tr></thead>
              <tbody>
                {purchases.map(p => (
                  <tr key={p.id}>
                    ...
                    <td>{(p.supplier_purchase_items || []).map(i => `${i.supplier_products?.name}: ${fmt(i.quantity)} ${i.unit}`).join(', ')}</td>
                    <td><strong>{fmt(p.total_amount)}</strong></td>
                  </tr>
                ))}
                {!purchases.length && <tr><td colSpan="7" className="hint">—</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
