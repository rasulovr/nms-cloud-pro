  ...
  return (
    <div className={`app theme-${theme || 'classic'}`}>
      ...
      <main className={`main ${currentAccess === 'read' ? 'readonly-mode' : ''}`}>
        ...
        {section === 'salaries' && <SalaryWorkspace t={t} />}
...
function Settings({ session, t, theme, setTheme }) {
  ...
  async function deleteUser(user) {
    ...
    const login = user.login_name || (user.email || '').split('@')[0] || user.full_name || user.id
    const ok = window.confirm(`Удалить пользователя ${login}? Его права доступа и профиль будут удалены. При необходимости его можно создать заново.`)
    if (!ok) return

    const tryRpcDelete = async (fn) => {
      const res = await supabase.rpc(fn, { p_user_id: user.id })
      if (!res.error) return { done: true, error: null }
      const msg = String(res.error.message || '')
      if (msg.includes(fn) || msg.toLowerCase().includes('could not find the function')) return { done: false, error: null }
      return { done: false, error: res.error }
    }

    const rpc1 = await tryRpcDelete('delete_rms_login_user')
    if (rpc1.error) return setMsg(rpc1.error.message)
    if (!rpc1.done) {
      const rpc2 = await tryRpcDelete('delete_nms_login_user')
      if (rpc2.error) return setMsg(rpc2.error.message)
    }
