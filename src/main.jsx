...
async function rmsCalculateNetworkForecastForMonth(y, m, scopeBranchId = 'all') {
  ...
  const [
    ...
    supabase.from('daily_expenses').select('branch_id,amount,custom_category,expense_categories(name)').gte('expense_date', monthDate).lt('expense_date', monthEnd).is('deleted_at', null),
    supabase.from('supplier_purchases').select('branch_id,total_amount,supplier_purchase_items(total_amount,supplier_products(name,category))').gte('purchase_date', monthDate).lt('purchase_date', monthEnd).is('deleted_at', null),
  ...
  const scopedRevRows = (revRows || []).filter(filterByBranch)
  const revenue = scopedRevRows.reduce((s, r) => s + parseNum(r.total_revenue), 0)
  ...
  const currentExpenseGroups = rmsForecastSumExpenseGroups((currentExpenseRows || []).filter(filterByBranch))
  const currentPurchasesScoped = (currentPurchaseRows || []).filter(filterByBranch)
  const currentSupplierTotals = rmsFinanceAllocatedSupplierTotals(currentPurchasesScoped, 1)
  const currentSupplierTotal = currentSupplierTotals.food + currentSupplierTotals.packaging + currentSupplierTotals.household + currentSupplierTotals.other

  const previousGroupedByMonth = prevMonths.map(pm => {
    const expenseRows = (historicalExpenseRows || []).filter(r => r.expense_date >= pm.start && r.expense_date < pm.end).filter(filterByBranch)
    const purchaseRows = (historicalPurchaseRows || []).filter(r => r.purchase_date >= pm.start && r.purchase_date < pm.end).filter(filterByBranch)
    const expenseGroups = rmsForecastSumExpenseGroups(expenseRows)
    const supplierTotals = rmsFinanceAllocatedSupplierTotals(purchaseRows, 1)
    return {
      ...expenseGroups,
      supplier: supplierTotals.food + supplierTotals.packaging + supplierTotals.household + supplierTotals.other,
      food_all: expenseGroups.food_market + supplierTotals.food,
      packaging_all: expenseGroups.packaging + supplierTotals.packaging,
      household_all: expenseGroups.household + supplierTotals.household,
      other_supplier: supplierTotals.other
    }
  })
  const historicalAvg = (key) => rmsForecastAverageNonZero(previousGroupedByMonth.map(row => row[key]))
  ...
  const rentHistory = historicalAvg('rent')
  const rentFallback = forecastRevenue * 0.08
  const rentForecast = configuredRent > 0 ? configuredRent : (currentExpenseGroups.rent > 0 ? currentExpenseGroups.rent : rentHistory > 0 ? rentHistory : rentFallback)

  const currentSpecialExpenses = rmsForecastSumSpecialExpenses((currentExpenseRows || []).filter(filterByBranch))
  const previousSpecialByMonth = prevMonths.map(pm => {
    const expenseRows = (historicalExpenseRows || []).filter(r => r.expense_date >= pm.start && r.expense_date < pm.end).filter(filterByBranch)
    return rmsForecastSumSpecialExpenses(expenseRows)
  })
  const historicalSpecialAvg = (key) => rmsForecastAverageNonZero(previousSpecialByMonth.map(row => row[key]))

  const payrollDetails = rmsFinancePayrollDetailsForScope(employeeRows || [], selectedBranchId, revenueShareMap)
  const salaryFromEmployees = parseNum(payrollDetails.totalSalary)
  const dsmfFromEmployees = parseNum(payrollDetails.totalDsmf)
  const salaryFromCurrentExpenses = parseNum(currentSpecialExpenses.salary)
  const salaryFromHistory = historicalSpecialAvg('salary')
  const salaryFallback = forecastRevenue * 0.22
  const salaryForecast = salaryFromEmployees > 0
    ? salaryFromEmployees
    : salaryFromCurrentExpenses > 0
      ? salaryFromCurrentExpenses
      : salaryFromHistory > 0
        ? salaryFromHistory
        : salaryFallback

  const dsmfFromCurrentExpenses = parseNum(currentSpecialExpenses.dsmf)
  const dsmfFromHistory = historicalSpecialAvg('dsmf')
  const dsmfFallback = salaryForecast * 0.08
  const dsmfForecast = dsmfFromEmployees > 0
    ? dsmfFromEmployees
    : dsmfFromCurrentExpenses > 0
      ? dsmfFromCurrentExpenses
      : dsmfFromHistory > 0
        ? dsmfFromHistory
        : dsmfFallback
  ...
  const monthlyVariableForecast = (currentValue, historyValue, fallbackPercent) => {
    const current = parseNum(currentValue)
    const history = parseNum(historyValue)
    const fallback = forecastRevenue * fallbackPercent / 100
    if (current > 0) return Math.max(current * dailyFactor, history, fallback)
    if (history > 0) return Math.max(history, fallback)
    return fallback
  }
  const monthlyFixedForecast = (currentValue, historyValue, fallbackPercent = 0) => {
    const current = parseNum(currentValue)
    const history = parseNum(historyValue)
    const fallback = forecastRevenue * fallbackPercent / 100
  ...
  const currentFoodAll = currentExpenseGroups.food_market + currentSupplierTotals.food
  const currentPackagingAll = currentExpenseGroups.packaging + currentSupplierTotals.packaging
  const currentHouseholdAll = currentExpenseGroups.household + currentSupplierTotals.household
  const foodForecast = monthlyVariableForecast(currentFoodAll, historicalAvg('food_all'), 35)
  const packagingForecast = monthlyVariableForecast(currentPackagingAll, historicalAvg('packaging_all'), 2)
  const householdForecast = monthlyFixedForecast(currentHouseholdAll, historicalAvg('household_all'), 1.5)
  const otherSupplierForecast = monthlyVariableForecast(currentSupplierTotals.other, historicalAvg('other_supplier'), 0)
  const utilitiesForecast = monthlyFixedForecast(currentExpenseGroups.utilities, historicalAvg('utilities'), 4)
  const marketingForecast = monthlyFixedForecast(currentExpenseGroups.marketing, historicalAvg('marketing'), 1)
  const maintenanceForecast = monthlyFixedForecast(currentExpenseGroups.maintenance, historicalAvg('maintenance'), 1)
  const otherForecast = monthlyFixedForecast(currentExpenseGroups.other, historicalAvg('other'), 2)

  const forecastExpenses = rentForecast + salaryForecast + dsmfForecast + serviceForecast + taxForecast + foodForecast + packagingForecast + householdForecast + otherSupplierForecast + utilitiesForecast + marketingForecast + maintenanceForecast + otherForecast
  ...
  const details = [
    ...
    { name: 'Food Cost / закупки и базар', amount: foodForecast, note: currentFoodAll > 0 ? 'макс: текущий темп / история / норматив 35%' : historicalAvg('food_all') > 0 ? 'макс: история / норматив 35%' : 'норматив 35% от прогноза выручки' },
    { name: 'Зарплаты', amount: salaryForecast, note: salaryFromEmployees > 0 ? 'из активных сотрудников' : salaryFromCurrentExpenses > 0 ? 'из текущего месяца' : salaryFromHistory > 0 ? 'среднее прошлых месяцев' : 'норматив 22% от прогноза выручки' },
    { name: 'Аренда', amount: rentForecast, note: configuredRent > 0 ? 'из настроек филиалов' : currentExpenseGroups.rent > 0 ? 'из текущего месяца' : rentHistory > 0 ? 'среднее прошлых месяцев' : 'норматив 8% от прогноза выручки' },
    ...
    { name: 'DSMF', amount: dsmfForecast, note: dsmfFromEmployees > 0 ? 'по официальному фонду зарплаты' : dsmfFromCurrentExpenses > 0 ? 'из текущего месяца' : dsmfFromHistory > 0 ? 'среднее прошлых месяцев' : 'норматив 8% от зарплат' },
    { name: 'Коммунальные', amount: utilitiesForecast, note: currentExpenseGroups.utilities > 0 ? 'из текущего месяца' : historicalAvg('utilities') > 0 ? 'среднее прошлых месяцев' : 'норматив 4% от прогноза выручки' },
    { name: 'Take away / packaging', amount: packagingForecast, note: currentPackagingAll > 0 ? 'макс: текущий темп / история / норматив 2%' : historicalAvg('packaging_all') > 0 ? 'среднее прошлых месяцев или норматив 2%' : 'норматив 2% от прогноза выручки' },
    { name: 'Хозтовары', amount: householdForecast, note: currentHouseholdAll > 0 ? 'из текущего месяца' : historicalAvg('household_all') > 0 ? 'среднее прошлых месяцев' : 'норматив 1.5% от прогноза выручки' },
    { name: 'Маркетинг', amount: marketingForecast, note: currentExpenseGroups.marketing > 0 ? 'из текущего месяца' : historicalAvg('marketing') > 0 ? 'среднее прошлых месяцев' : 'норматив 1% от прогноза выручки' },
    { name: 'Ремонт / обслуживание', amount: maintenanceForecast, note: currentExpenseGroups.maintenance > 0 ? 'из текущего месяца' : historicalAvg('maintenance') > 0 ? 'среднее прошлых месяцев' : 'норматив 1% от прогноза выручки' },
    { name: 'Прочее', amount: otherForecast + otherSupplierForecast, note: 'текущий месяц / история / норматив 2%' }
  ].filter(row => parseNum(row.amount) > 0 || row.type === 'revenue')
...
function Dashboard({ t }) {
  ...
  async function calcMonth(y, m) {
    ...
    const syncedForecast = await rmsCalculateNetworkForecastForMonth(y, m)
  ...
  return <section className={`salary-section salary-view-${view} attendance-card`}>
    ...
    <section className="grid">
      <div className="card span-2">
        ...
        {message && <p className={`hint ${message === t('saved') || message.includes('Файл') || message.includes('обнулены') ? 'good' : 'bad'}`}>{message}</p>}
      </div>
...
function Suppliers({ t }) {
  ...
  async function savePayment() {
    ...
    if (!paymentForm.legal_entity_id) return setMessage('Выберите наш VOEN / юрлицо для оплаты')
  ...
  return <section>
    ...
    <section className="grid">
      <div className="card span-2 supplier-opening-debt-card">
        ...
        {message && <p className={`hint ${message.includes('добавлен') || message === t('saved') ? 'good' : 'bad'}`}>{message}</p>}
      </div>
...
function DebtsPayments({ t }) {
  ...
  function supplierAlert(supplier, legalEntityId = '') {
    const balance = legalEntityId ? balanceForSupplierLegal(supplier.id, legalEntityId) : balanceForSupplier(supplier.id)
    const limit = parseNum(supplier.credit_limit)
    const termDays = parseNum(supplier.payment_term_days)
    const overLimit = limit > 0 && balance > limit ? balance - limit : 0
    const now = new Date()
