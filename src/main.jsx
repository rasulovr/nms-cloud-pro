  ...
  async function updateExpense(id, patch) {
    ...
    if (isBazarExpenseName(nextExpenseName)) {
      try {
        ...
        loadMonthStats(branchId, nextExpense.expense_date || date)
...
function Recipes({ t }) {
  ...
  async function editSelectedSemiLine() {
    const item = selectedSemiItems.find(row => String(row.id) === String(selectedSemiItemId))
    if (!item) {
      setMessage('Выберите ингредиент в таблице')
      return
    }
  ...
  return (
    <section>
      ...
      {tab === 'legacy' && (
        <section className="grid">
          <div className="card span-2">
            <h3>Текущая тех. карта блюда</h3>
            <p className="hint">Эта вкладка теперь учитывает новые компоненты: полуфабрикаты, ингредиенты из закупок и ручные ингредиенты.</p>
            ...
            {selectedMenu && <div className="metric-grid">
              ...
              <table>
                ...
                <tbody>
                  {selectedFinalItems.map(item => (
                    <tr key={item.id}>
                      <td>{item.component_type === 'semi' ? 'Полуфабрикат' : item.component_type === 'manual' ? 'Вручную' : 'Ингредиент'}</td>
                  ...
                  {!selectedFinalItems.length && <tr><td colSpan="5" className="hint">В новой техкарте блюда пока нет компонентов. Добавь их во вкладке “Создать блюдо”.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
          ...
          <div className="card span-2">
            <h3>Старые тех. карты</h3>
            <p className="hint">Ниже оставлен старый модуль для совместимости со старыми техкартами.</p>
            <RecipesLegacy t={t} />
          </div>
        </section>
      )}
      ...
      {tab === 'semis' && (
        <section className="grid semifinished-layout">
          ...
          <div className="card semi-composition-card">
            ...
            <p className="hint">Редактирование — только через кнопку “Изменить” в конце строки.</p>

            <div className="table-wrap">
              <table>
                <thead><tr><th>Компонент</th><th>Кол-во</th><th>Цена</th><th>Пот.</th><th>Себес.</th></tr></thead>
                <tbody>
                  {selectedSemiItems.map(item => (
                    <tr
                      key={item.id}
                      className={String(selectedSemiItemId) === String(item.id) ? 'semi-row selected' : 'semi-row'}
                      onClick={() => setSelectedSemiItemId(item.id)}
                    >
                      <td><div className="semi-main-value semi-name">{item.item_name}</div></td>
                      <td><b>{fmt(item.qty)} {item.unit || 'g'}</b></td>
                      <td>{item.component_type === 'manual' ? <span>{fmt(item.manual_unit_cost || 0)}</span> : <span className="hint">закупка</span>}</td>
          ...
          <div className="card semi-add-card span-2">
            <h3>Добавить ингредиенты в полуфабрикат</h3>
            <p className="hint">Добавление новой строки. Редактирование уже добавленных ингредиентов — в таблице состава справа.</p>
            <div className="form-grid compact">
              <label><span>Источник ингредиента</span><select value={semiLineForm.component_type} onChange={e => setSemiLineForm({ ...semiLineForm, component_type: e.target.value })}>
                <option value="product">Из списка закупок</option>
                <option value="manual">Вручную / без закупа</option>
              </select></label>

              {semiLineForm.component_type === 'product' ? (
                <label><span>Товар / ингредиент</span><select value={semiLineForm.product_id} onChange={e => setSemiLineForm({ ...semiLineForm, product_id: e.target.value })}>
                  <option value="">Выбрать</option>
                  {products.map(p => <option key={p.id} value={p.id}>{p.name} · {p.category}</option>)}
                </select></label>
              ) : (
                <>
                  <label><span>Название вручную</span><input value={semiLineForm.manual_name} onChange={e => setSemiLineForm({ ...semiLineForm, manual_name: e.target.value })} placeholder="Например: тёмный шоколад" /></label>
                  <label><span>Цена за 1 ед.</span><input value={semiLineForm.manual_unit_cost} onChange={e => setSemiLineForm({ ...semiLineForm, manual_unit_cost: e.target.value })} placeholder="AZN за 1 g/ml/шт" /></label>
                </>
              )}

              <label><span>Граммаж / кол-во</span><input value={semiLineForm.qty} onChange={e => setSemiLineForm({ ...semiLineForm, qty: e.target.value })} /></label>
              <label><span>Ед.</span><select value={semiLineForm.unit} onChange={e => setSemiLineForm({ ...semiLineForm, unit: e.target.value })}><option value="g">g</option><option value="kg">kg</option><option value="ml">ml</option><option value="l">l</option><option value="pcs">pcs</option></select></label>
              <label><span>Потери %</span><input value={semiLineForm.waste_percent} onChange={e => setSemiLineForm({ ...semiLineForm, waste_percent: e.target.value })} /></label>
            </div>
            <button className="small primary" onClick={addSemiLine}>+ Добавить компонент</button>
          </div>
        </section>
      )}
      ...
      {tab === 'final' && (
        <section className="grid">
          ...
          <div className="card">
            <h3>Пример подачи Брауни</h3>
            <p className="hint">В блюдо можно добавить: готовый полуфабрикат, ингредиент из закупок или ручной ингредиент без закупа.</p>
          </div>

          <div className="card span-2">
            <h3>Добавить состав блюда</h3>
            <div className="form-grid compact">
              <label><span>Что добавляем в блюдо</span><select value={finalLineForm.component_type} onChange={e => setFinalLineForm({ ...finalLineForm, component_type: e.target.value })}>
                <option value="semi">Готовый полуфабрикат</option>
                <option value="product">Ингредиент из закупок</option>
                <option value="manual">Вручную / без закупа</option>
              </select></label>

              {finalLineForm.component_type === 'semi' ? (
                <label><span>Полуфабрикат</span><select value={finalLineForm.semi_id} onChange={e => setFinalLineForm({ ...finalLineForm, semi_id: e.target.value })}>
                  <option value="">Выбрать</option>
                  {semis.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select></label>
              ) : finalLineForm.component_type === 'product' ? (
                <label><span>Ингредиент</span><select value={finalLineForm.product_id} onChange={e => setFinalLineForm({ ...finalLineForm, product_id: e.target.value })}>
                  <option value="">Выбрать</option>
                  {products.map(p => <option key={p.id} value={p.id}>{p.name} · {p.category}</option>)}
                </select></label>
              ) : (
                <>
                  <label><span>Название вручную</span><input value={finalLineForm.manual_name} onChange={e => setFinalLineForm({ ...finalLineForm, manual_name: e.target.value })} placeholder="Например: золотая стружка" /></label>
                  <label><span>Цена за 1 ед.</span><input value={finalLineForm.manual_unit_cost} onChange={e => setFinalLineForm({ ...finalLineForm, manual_unit_cost: e.target.value })} placeholder="AZN за 1 g/ml/шт" /></label>
                </>
              )}

              <label><span>Граммаж / кол-во</span><input value={finalLineForm.qty} onChange={e => setFinalLineForm({ ...finalLineForm, qty: e.target.value })} /></label>
              <label><span>Ед.</span><select value={finalLineForm.unit} onChange={e => setFinalLineForm({ ...finalLineForm, unit: e.target.value })}><option value="g">g</option><option value="kg">kg</option><option value="ml">ml</option><option value="l">l</option><option value="pcs">pcs</option></select></label>
              <label><span>Потери %</span><input value={finalLineForm.waste_percent} onChange={e => setFinalLineForm({ ...finalLineForm, waste_percent: e.target.value })} /></label>
            </div>
            <button className="small primary" onClick={addFinalLine}>+ Добавить в подачу</button>

            <div className="table-wrap">
              <table>
                <thead><tr><th>Тип</th><th>Компонент</th><th>Кол-во</th><th>Потери</th><th>Себестоимость</th><th></th></tr></thead>
                <tbody>
                  {selectedFinalItems.map(item => (
                    <tr key={item.id}>
                      <td>{item.component_type === 'semi' ? 'Полуфабрикат' : item.component_type === 'manual' ? 'Вручную' : 'Ингредиент'}</td>
                      ...
                      <td>{fmt(item.waste_percent)}%</td>
                      <td><b>{fmt(componentCost(item))} AZN</b></td>
                      <td><button className="small danger" onClick={() => deleteFinalLine(item.id)}>Удалить</button></td>
                    </tr>
                  ))}
                  {!selectedFinalItems.length && <tr><td colSpan="6" className="hint">Состав подачи пока пустой.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
...
function SemiFinishedInlineStyles() {
  return (
    <style>{`
      ...
      .semifinished-layout {
        grid-template-columns: minmax(250px, .8fr) minmax(280px, .8fr) minmax(560px, 1.6fr);
        align-items: start;
      }
      ...
      .semi-row { cursor: pointer; }
      .semi-row.selected td { background: #eef2f7; }
      .semi-table-actions { justify-content: flex-end; margin-top: 10px; }
      .semi-composition-card .table-wrap { overflow-x: visible !important; max-width: 100%; }
      .semi-composition-card table { width: 100%; min-width: 0 !important; table-layout: fixed; }
      .semi-composition-card th, .semi-composition-card td { padding: 7px 5px; font-size: 11px; line-height: 1.2; }
      .semi-composition-card th:nth-child(1), .semi-composition-card td:nth-child(1) { width: auto !important; }
      .semi-composition-card th:nth-child(2), .semi-composition-card td:nth-child(2) { width: 74px !important; }
      .semi-composition-card th:nth-child(3), .semi-composition-card td:nth-child(3) { width: 58px !important; }
      .semi-composition-card th:nth-child(4), .semi-composition-card td:nth-child(4) { width: 46px !important; }
      .semi-composition-card th:nth-child(5), .semi-composition-card td:nth-child(5) { width: 64px !important; }
