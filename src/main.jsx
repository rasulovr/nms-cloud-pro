...
function DebtsPayments({ t }) {
  ...
  async function load() {
    const [{ data: le }, { data: sup }, { data: bal }, { data: pur }, { data: pay }, { data: opening }, { data: logs }] = await Promise.all([
      supabase.from('legal_entities').select('*').eq('is_active', true).order('name'),
      supabase.from('suppliers').select('*').eq('is_active', true).order('name'),
      supabase.from('supplier_balances_v2').select('*').order('supplier_name'),
      ...
      supabase.from('supplier_opening_debts').select('*, suppliers(name,voen), legal_entities(name,voen)').eq('is_active', true).order('debt_date', { ascending: false }).order('created_at', { ascending: false }).limit(1000),
  ...
  function balanceForSupplier(id) {
    ...
    const openingTotal = openingDebts.filter(d => d.supplier_id === id).reduce((sum, d) => sum + parseNum(d.amount), 0)
  ...
  function balanceForSupplierLegal(supplierId, legalEntityId) {
    ...
    const openingTotal = openingDebts.filter(d => d.supplier_id === supplierId && d.legal_entity_id === legalEntityId).reduce((sum, d) => sum + parseNum(d.amount), 0)
  ...
  function startEditPurchaseTransaction(row) {
    setDetailPurchaseId('')
    setEditingOpeningDebtId('')
    setEditingPurchaseTransactionId(row.id)
    setPurchaseTransactionEditForm({
      purchase_date: row.purchase_date || todayISO(),
      invoice_number: row.invoice_number || '',
      branch_id: row.branch_id || '',
      total_amount: String(parseNum(row.total_amount)),
      comment: row.comment || ''
    })
  }

  async function savePurchaseTransactionEdit(row) {
    const totalAmount = parseNum(purchaseTransactionEditForm.total_amount)
    if (!totalAmount) return setMessage('Введите сумму поступления')
