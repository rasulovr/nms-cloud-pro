      ...
      {tab === 'semis' && (
        <section className="grid semifinished-layout">
          ...
          <div className="card semi-composition-card">
            ...
            <div className="table-wrap">
              <table>
                <thead><tr><th>Тип</th><th>Компонент</th><th>Кол-во</th><th>Цена</th><th>Пот.</th><th>Себес.</th><th></th></tr></thead>
                <tbody>
                  {selectedSemiItems.map(item => (
                    <tr key={item.id}>
                      <td>{item.component_type === 'semi' ? 'П/ф' : item.component_type === 'manual' ? 'Ручн.' : 'Зак.'}</td>
                      <td>
...
function SemiFinishedInlineStyles() {
  return (
    <style>{`
      ...
      .semifinished-layout {
        grid-template-columns: minmax(240px, .7fr) minmax(270px, .75fr) minmax(660px, 1.9fr);
        align-items: start;
      }
