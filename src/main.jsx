...
async function rmsCalculateNetworkForecastForMonth(y, m, scopeBranchId = 'all') {
  ...
  const salaryForecast = currentPeriodSalary || currentMonthlySalary || activeEmployeeSalary || currentExpenseSalary || salaryHistory || payrollSummarySalary
  const salaryForecastNote = currentPeriodSalary > 0
    ? `из “Зарплаты → Сотрудники → Сводка по филиалам”: используется колонка “Начислено”; менеджеры распределены между филиалами по доле выручки и не суммируются дважды${selectedBranchId !== 'all' && payrollManagersSalary > 0 ? ` · доля менеджеров ${fmt(payrollManagersSalary)} AZN` : ''}`
    : currentMonthlySalary > 0
      ? 'из месячного расчёта зарплат за текущий месяц'
      : activeEmployeeSalary > 0
        ? 'нет сводки за месяц — расчёт по активным сотрудникам: зарплата филиала + доля менеджеров по выручке; менеджеры не суммируются дважды'
