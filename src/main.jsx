...
function Suppliers({ t }) {
  ...
  function selectProductForLine(index, value) {
    ...
    if (selected) {
      updateLine(index, { product_id: selected.id, product_query: selected.name || productLabel(selected), category: selected.category || 'Прочее' })
    } else {
      updateLine(index, { product_id: '', product_query: '' })
    }
  }
                      ...
                      <td style={{minWidth:280}}>
                        <div style={{display:'grid', gap:6}}>
                          <input
                            ...
                            onChange={e => updateLine(idx, { product_query: e.target.value })}
                          ...
                          >
                            <option value="">Выберите товар</option>
                            {productOptionsForRow(row).map(p => <option key={p.id} value={p.id}>{productLabel(p)}</option>)}
                          </select>
                        </div>
                      </td>
