      ...
      {tab === 'semis' && (
        <section className="grid semifinished-layout">
          ...
          <div className="card semi-composition-card">
            ...
            <p className="hint">Нажмите “Редактировать состав”, выберите строку и редактируйте её в форме под таблицей.</p>
            ...
            {semiEditModalOpen && (() => {
              const draft = semiEditDraft.find(row => String(row.id) === String(selectedSemiItemId)) || semiEditDraft[0]
              if (!draft) return null
              return (
                <div className="semi-selected-editor">
                  <h4>Редактирование ингредиента</h4>
                  <p className="hint">Выберите строку в таблице выше. Поля редактируются здесь полностью и без сжатия.</p>
                  <div className="semi-selected-editor-grid">
                    <label><span>Ингредиент</span>{draft.component_type === 'manual' ? <input value={draft.item_name} onChange={e => updateSemiEditDraft(draft.id, { item_name: e.target.value })} /> : <input value={draft.item_name} readOnly />}</label>
                    <label><span>Кол-во</span><input value={draft.qty} onChange={e => updateSemiEditDraft(draft.id, { qty: e.target.value })} /></label>
                    <label><span>Ед.</span><select value={draft.unit} onChange={e => updateSemiEditDraft(draft.id, { unit: e.target.value })}><option value="g">g</option><option value="kg">kg</option><option value="ml">ml</option><option value="l">l</option><option value="pcs">pcs</option></select></label>
                    <label><span>Цена за 1 ед.</span>{draft.component_type === 'manual' ? <input value={draft.manual_unit_cost} onChange={e => updateSemiEditDraft(draft.id, { manual_unit_cost: e.target.value })} /> : <input value="из закупки" readOnly />}</label>
                    <label><span>Потери %</span><input value={draft.waste_percent} onChange={e => updateSemiEditDraft(draft.id, { waste_percent: e.target.value })} /></label>
                  </div>
                </div>
              )
            })()}
