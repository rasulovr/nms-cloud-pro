...
function Recipes({ t }) {
  ...
  function cancelSemiLineEdit() {
    setSelectedSemiItemId('')
    setSemiLineEditDraft(null)
  }

  function updateSemiLineEditDraft(patch) {
    setSemiLineEditDraft(prev => prev ? { ...prev, ...patch } : prev)
  }

  async function saveSemiLineEdit() {
    if (!semiLineEditDraft?.id) return

    const payload = {
      qty: parseNum(semiLineEditDraft.qty),
      unit: semiLineEditDraft.unit || 'g',
      waste_percent: parseNum(semiLineEditDraft.waste_percent)
    }

    if (semiLineEditDraft.component_type === 'manual') {
      payload.item_name = String(semiLineEditDraft.item_name || '').trim()
      payload.manual_unit_cost = parseNum(semiLineEditDraft.manual_unit_cost)
      if (!payload.item_name) return setMessage('Введите название ингредиента')
    }

    const { error } = await supabase.from('rms_semi_finished_items').update(payload).eq('id', semiLineEditDraft.id)
    if (error) return setMessage(error.message)
...
function SemiFinishedInlineStyles() {
  return (
    <style>{`
      ...
      .semifinished-layout {
        grid-template-columns: minmax(270px, .8fr) minmax(300px, .8fr) !important;
        align-items: start;
      }
      .semi-composition-card {
      ...
      .semi-composition-table th:first-child,
      .semi-composition-table td:first-child {
        width: 38% !important;
        min-width: 260px !important;
        white-space: normal !important;
      }
      .semi-composition-table th:nth-child(2),
      .semi-composition-table td:nth-child(2) { width: 110px !important; }
      .semi-composition-table th:nth-child(3),
      .semi-composition-table td:nth-child(3) { width: 90px !important; }
      .semi-composition-table th:nth-child(4),
      .semi-composition-table td:nth-child(4) { width: 120px !important; }
      .semi-composition-table th:nth-child(5),
      .semi-composition-table td:nth-child(5) { width: 90px !important; }
      .semi-composition-table th:nth-child(6),
      .semi-composition-table td:nth-child(6) { width: 100px !important; }
      .semi-composition-table th:nth-child(7),
      .semi-composition-table td:nth-child(7) { width: 190px !important; }
      .semi-row.editing td {
      ...
      .semi-component-input { min-width: 240px !important; }
      .semi-number-input { max-width: 96px !important; }
      .semi-unit-input { max-width: 88px !important; }
      .semi-price-input { max-width: 116px !important; }
      .semi-row-actions {
        display: flex;
        gap: 8px;
        align-items: center;
      ...
      @media (max-width: 980px) {
        .semifinished-layout { grid-template-columns: 1fr !important; }
        .semi-composition-table-wrap { overflow-x: auto !important; }
        .semi-composition-table { min-width: 900px !important; }
      }


      /* FINAL FIX: состав полуфабриката должен быть полноценной широкой таблицей, а не узкой колонкой */
      .semifinished-layout {
      ...
      .semi-component-input { min-width: 220px !important; }
      .semi-number-input { min-width: 82px !important; }
      .semi-unit-input { min-width: 68px !important; }
      .semi-price-input { min-width: 92px !important; }
      .semi-row-actions {
