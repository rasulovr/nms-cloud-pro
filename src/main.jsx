  ...
  return (
    <section>
      <section className="topbar">
        <div>
          <h2>Тех. карты</h2>
          <p>Ингредиенты → полуфабрикаты → финальные блюда. Себестоимость пересчитывается автоматически по последним закупочным ценам.</p>
        </div>
        <button className="small primary" onClick={loadSemiData}>{loading ? 'Загрузка...' : 'Обновить'}</button>
      </section>

      <div className="settings-tabs">
        <button className={tab === 'semis' ? 'active' : ''} onClick={() => setTab('semis')}>Полуфабрикаты</button>
        <button className={tab === 'final' ? 'active' : ''} onClick={() => setTab('final')}>Блюда / подача</button>
        <button className={tab === 'legacy' ? 'active' : ''} onClick={() => setTab('legacy')}>Текущие тех. карты</button>
      </div>

      {message ? <p className="hint">{message}</p> : null}

      {tab === 'legacy' && <RecipesLegacy t={t} />}

      {tab === 'semis' && (
        <section className="grid">
          <div className="card">
            <h3>Создать полуфабрикат</h3>
          ...
          <div className="card span-2">
            <h3>Состав полуфабриката</h3>
            <div className="form-grid compact">
              <label><span>Тип</span><select value={semiLineForm.component_type} onChange={e => setSemiLineForm({ ...semiLineForm, component_type: e.target.value })}>
              ...
              {semiLineForm.component_type === 'product' ? (
                <label><span>Ингредиент</span><select value={semiLineForm.product_id} onChange={e => setSemiLineForm({ ...semiLineForm, product_id: e.target.value })}>
                  <option value="">Выбрать</option>
                  {products.map(p => <option key={p.id} value={p.id}>{p.name} · {p.category}</option>)}
                </select></label>
              ) : semiLineForm.component_type === 'semi' ? (
                <label><span>Полуфабрикат</span><select value={semiLineForm.semi_id_ref} onChange={e => setSemiLineForm({ ...semiLineForm, semi_id_ref: e.target.value })}>
                  <option value="">Выбрать</option>
                  {semis.filter(s => String(s.id) !== String(selectedSemiId)).map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select></label>
              ) : (
                <>
                  ...
                  <label><span>Цена за 1 ед.</span><input value={semiLineForm.manual_unit_cost} onChange={e => setSemiLineForm({ ...semiLineForm, manual_unit_cost: e.target.value })} placeholder="AZN за 1 g/шт" /></label>
                </>
              )}

              <label><span>Кол-во</span><input value={semiLineForm.qty} onChange={e => setSemiLineForm({ ...semiLineForm, qty: e.target.value })} /></label>
      ...
      {tab === 'final' && (
        <section className="grid">
          ...
          <div className="card">
            ...
            <p className="hint">Добавь: Брауни масса 100g, Крем 10g, белый шоколад 6g, мороженое 55g, золотая стружка 1g.</p>
          </div>
            ...
            <div className="form-grid compact">
              ...
              {finalLineForm.component_type === 'semi' ? (
                ...
                <>
                  ...
                  <label><span>Цена за 1 ед.</span><input value={finalLineForm.manual_unit_cost} onChange={e => setFinalLineForm({ ...finalLineForm, manual_unit_cost: e.target.value })} placeholder="AZN за 1 g/шт" /></label>
                </>
              )}

              <label><span>Кол-во</span><input value={finalLineForm.qty} onChange={e => setFinalLineForm({ ...finalLineForm, qty: e.target.value })} /></label>
