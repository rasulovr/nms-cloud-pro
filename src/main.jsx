        ...
        {expenseDetail.name && <div className="card span-2">
          ...
          {!expenseDetail.loading && !expenseDetail.error && <>
            ...
            <div className="table-wrap" style={{marginTop: 12}}>
              <table>
                <thead><tr><th>Дата</th><th>Филиал</th><th>Статья</th><th>Сумма</th><th>Комментарий</th><th>Создано</th></tr></thead>
                <tbody>
                  {expenseDetail.rows.map(row => <tr key={row.id}><td>{row.expense_date}</td><td>{row.branchName}</td><td>{row.name}</td><td><b>{fmt(row.amountValue)}</b></td><td>{row.comment || '—'}</td><td>{row.created_at ? new Date(row.created_at).toLocaleString('ru-RU') : '—'}</td></tr>)}
                  {!expenseDetail.rows.length && <tr><td colSpan="6" className="hint">По этой статье за выбранный месяц прямых строк расхода не найдено. Если статья расчётная, она могла быть сформирована автоматически из зарплат, DSMF, налога или закупок поставщиков.</td></tr>}
                </tbody>
              </table>
            </div>
          </>}
