...
function POSLite({ t }) {
  ...
  function addMenuItemToOrder(item) {
    setItems(prev => {
      const existing = prev.find(x => String(x.menu_item_id) === String(item.id))
      if (existing) return prev.map(x => String(x.menu_item_id) === String(item.id) ? { ...x, qty: parseNum(x.qty) + 1 } : x)
      return [...prev, {
  ...
  function updateItem(index, patch) {
    setItems(prev => prev.map((item, i) => i === index ? { ...item, ...patch } : item).filter(item => parseNum(item.qty) > 0))
  }
  ...
  async function closeOrder() {
    ...
    try {
      ...
      const revenuePayload = {
        branch_id: branchId,
        revenue_date: date,
        cash_amount: paymentMethod === 'cash' ? subtotal : 0,
        bank_amount: paymentMethod === 'bank' ? subtotal : 0,
        wolt_amount: paymentMethod === 'wolt' ? subtotal : 0,
        comment: `POS-lite · ${items.length} поз. · ${totalQty} шт.${tableName ? ` · стол ${tableName}` : ''}`,
      ...
      setCustomerName('')
      await loadBase()
      setMessage('Заказ закрыт. Продажа отправлена в выручку RMS.')
    } catch (error) {
  ...
  return (
    <section>
      <section className="topbar">
        <div>
          <h2>POS / Продажи</h2>
          <p>POS-lite: закрытие заказов с автоматической передачей выручки в RMS. Фискальный чек пока не печатается.</p>
        </div>
      </section>
      ...
      <div className="grid">
        <div className="card">
          ...
          <div className="form-grid compact">
            ...
            <label><span>Стол / заказ</span><input value={tableName} onChange={e => setTableName(e.target.value)} placeholder="Напр. T12" /></label>
            <label><span>Гость / комментарий</span><input value={customerName} onChange={e => setCustomerName(e.target.value)} placeholder="Необязательно" /></label>
            <label><span>Оплата</span><select value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)}><option value="cash">Наличные</option><option value="bank">Банк</option><option value="wolt">Wolt / доставка</option></select></label>
            <label><span>Поиск позиции</span><input value={search} onChange={e => setSearch(e.target.value)} placeholder="кофе, burger, салат..." /></label>
          </div>

          <div className="table-wrap" style={{maxHeight:360, overflow:'auto', marginTop:12}}>
            <table>
              <thead><tr><th>Позиция</th><th>Категория</th><th>Цена</th><th></th></tr></thead>
              <tbody>
                {filteredMenu.map(item => <tr key={item.id}>
                  <td><b>{item.name}</b></td>
                  <td>{item.category || '—'}</td>
                  <td>{fmt(item.sale_price)}</td>
                  <td><button className="small" onClick={() => addMenuItemToOrder(item)}>+</button></td>
                </tr>)}
                {!filteredMenu.length && <tr><td colSpan="4" className="hint">Нет позиций в техкартах.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="card">
          <div className="card-head">
            <div><h3>Текущий заказ</h3><p className="hint">Позиции берутся из техкарт / меню RMS.</p></div>
            <button className="ghost small" onClick={() => setItems([])}>Очистить</button>
          </div>
            ...
            <table>
              ...
              <tbody>
                {items.map((item, idx) => <tr key={`${item.menu_item_id}-${idx}`}>
                  ...
                  <td><input style={{width:80}} inputMode="decimal" value={item.price} onChange={e => updateItem(idx, { price: e.target.value })} /></td>
                ...
                {!items.length && <tr><td colSpan="6" className="hint">Заказ пуст.</td></tr>}
              </tbody>
            </table>
          </div>
