  ...
  function printSalaryEmployeesPdf() {
    ...
    const renderRows = groupRows => groupRows.map(r => {
      ...
      return `<tr>
        ...
        <td>${hidden ? '***' : fmt(r.monthly_salary)}</td>
        <td>${hidden ? '***' : fmt(r.daily_rate)}</td>
    ...
    const groupHtml = salaryGroups.map(group => {
      ...
      return `<section class="group">
        ...
        <table>
          <thead><tr><th>Сотрудник</th><th>Должность</th><th>Начислено</th><th>К выплате</th><th>Авансы</th><th>Удержание</th><th>Остаток месяца</th><th>Выплачено</th><th>Мес. ставка</th><th>Дневная</th><th>Дни</th><th>Долг нач.</th><th>Статус</th></tr></thead>
          <tbody>${renderRows(group.rows) || '<tr><td colspan="13">Нет сотрудников</td></tr>'}</tbody>
        </table>
      ...
      <div className="card span-2">
        ...
        {salaryGroups.map(group => {
          ...
          return <div key={`salary-group-${group.id}`} className="supplier-entity-group">
            ...
            <div className="table-wrap">
              <table>
                <thead><tr><th>Сотрудник</th><th>Должность</th><th>Начислено</th><th>К выплате</th><th>Авансы</th><th>Удержание</th><th>Остаток месяца</th><th>Выплачено</th><th>Мес. ставка</th><th>Дневная</th><th>Дни</th><th>Долг нач.</th><th>Статус</th><th>Инфо</th></tr></thead>
                <tbody>
                  {groupRows.map(r => {
                    ...
                    return <React.Fragment key={r.id || r.employee_id}>
                      <tr style={underworked ? { background: 'rgba(155,45,45,.08)' } : undefined}>
                        <td><input style={{minWidth:160,padding:'6px 7px'}} defaultValue={r.employees?.full_name || ''} onBlur={e => updateEmployeeDetails(r, { full_name: e.target.value.trim() || r.employees?.full_name })} /></td>
                        <td><input style={{minWidth:110,padding:'6px 7px'}} defaultValue={r.employees?.position || ''} onBlur={e => updateEmployeeDetails(r, { position: e.target.value.trim() || null })} /></td>
                        ...
                        <td><input style={{minWidth:80,padding:'6px 7px'}} inputMode="decimal" defaultValue={r.employees?.monthly_salary || r.monthly_salary || ''} onBlur={e => updateEmployeeDetails(r, { monthly_salary: e.target.value }, true)} /></td>
                        <td>{moneyCell(r, r.daily_rate)}</td>
                        <td>{fmt(r.worked_days)}</td>
                      ...
                      {isInfoOpen && <tr><td colSpan="14"><div className="notice">
                        <div className="card-head">
                          ...
                          {r.employees?.employment_status === 'terminated' || r.employees?.is_active === false
                            ...
                            : <button className="danger small" onClick={() => updateEmployeeDetails(r, { employment_status: 'terminated', is_active: false, terminated_at: todayISO() })}>Уволить</button>}
                        </div>
                        <div className="form-grid compact">
                          ...
                          <label><span>Должность</span><input defaultValue={r.employees?.position || ''} onBlur={e => updateEmployeeDetails(r, { position: e.target.value.trim() || null })} /></label>
