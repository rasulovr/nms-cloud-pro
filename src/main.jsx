...
function Salaries({ t }) {
  ...
  const [paymentForm, setPaymentForm] = useState({ employee_id: '', payment_date: todayISO(), previous_amount: '', amount: '', method: 'cash', comment: '' })
  ...
  async function addSalaryPayment() {
    ...
    setPaymentForm(f => ({ ...f, previous_amount: '', amount: '', comment: '' }))
  ...
  const payrollPaymentsForSelectedDate = salaryPayments.filter(p => p.payment_date === paymentForm.payment_date)
  const payrollPaymentRowsForSelectedDate = rows
    .map(r => {
      ...
      return { row: r, payments, paidCash, paidBank, paidTotal: paidCash + paidBank, openingDebt: Math.max(0, parseNum(r.opening_balance)) }
    })
  ...
  return <section>
    ...
    <section className="grid">
      ...
      <div className="card span-2">
        ...
        <p className="hint">Например, зарплату за апрель можно выплатить 1 или 2 мая — она всё равно закроет апрель. Остаток прошлого месяца вводится отдельным полем и не попадает в авансы текущего месяца.</p>
        <div className="form-grid compact">
      ...
      <div className="card span-2">
        <div className="card-head">
          <div>
            ...
            <p className="hint">Список выплат по дате из формы выше, сгруппированный по филиалам. Показывает также остаток зарплаты за прошлый месяц, если он остался.</p>
          </div>
        </div>
        <div className="mini-grid">
          <div className="metric"><span>Дата</span><strong>{paymentForm.payment_date}</strong></div>
        ...
        {payrollPaymentGroupsForSelectedDate.map(group => {
          ...
          return <div key={`pay-day-${group.id}`} className="supplier-entity-group">
            <div className="supplier-entity-head">
              <b>{group.name}</b>
              <span>Выплачено: <strong>{fmt(group.totalPaid)}</strong> AZN · остаток прошлого месяца: <strong className={group.totalOpeningDebt > 0 ? 'bad' : ''}>{fmt(group.totalOpeningDebt)}</strong> AZN · сотрудников: {group.items.length}</span>
            </div>
            <div className="table-wrap">
              <table>
                <thead><tr><th>Сотрудник</th><th>Должность</th><th>Наличные</th><th>Банк</th><th>Всего оплачено</th><th>Остаток прошлого месяца</th><th>Остаток к выплате</th></tr></thead>
                <tbody>
                  {shown.map(item => {
                    const r = item.row
                    const hidden = !canSeeSalaryValue(r)
