...
function DebtsPayments({ t }) {
  ...
  async function load() {
    const [{ data: le }, { data: br }, { data: sup }, { data: bal }, { data: pur }, { data: pay }, { data: opening }, { data: logs }] = await Promise.all([
      supabase.from('legal_entities').select('*').eq('is_active', true).order('name'),
  ...
  async function savePurchaseTransactionEdit(row) {
    ...
    const totalAmount = (itemRows || []).reduce((s, i) => s + parseNum(i.total_amount), 0) || parseNum(row.total_amount)
