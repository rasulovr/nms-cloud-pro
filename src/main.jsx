...
function Recipes({ t }) {
  ...
  async function addSemiLine() {
    ...
    if (!isSemi && !semiLineForm.product_id) return setMessage('Выберите ингредиент')
    ...
    const { error } = await supabase.from('rms_semi_finished_items').insert({
      ...
      product_id: isSemi ? null : semiLineForm.product_id,
      semi_id_ref: isSemi ? semiLineForm.semi_id_ref : null,
      item_name: isSemi ? semi?.name : product?.name,
    ...
    setSemiLineForm({ ...semiLineForm, product_id: '', semi_id_ref: '', qty: '' })
    await loadSemiData()
  ...
  async function addFinalLine() {
    ...
    if (!isSemi && !finalLineForm.product_id) return setMessage('Выберите ингредиент')
    ...
    const { error } = await supabase.from('rms_final_recipe_components').insert({
      ...
      product_id: isSemi ? null : finalLineForm.product_id,
    ...
    setFinalLineForm({ ...finalLineForm, product_id: '', semi_id: '', qty: '' })
    await loadSemiData()
        ...
        <section className="grid">
          <div className="card">
            ...
            <div className="form-grid compact">
              ...
              <label className="span-2"><span>Комментарий</span><input value={semiForm.notes} onChange={e => setSemiForm({ ...semiForm, notes: e.target.value })} /></label>
            </div>
              ...
              <table>
                ...
                <tbody>
                  {selectedSemiItems.map(item => (
                    <tr key={item.id}>
                      <td>{item.component_type === 'semi' ? 'Полуфабрикат' : 'Ингредиент'}</td>
