...
function aggregateSalesRows(reports, options = {}) {
  ...
  ;(reports || []).forEach(report => {
    ;(report.rows || []).forEach(row => {
      const itemKey = normalizeSalesKey(row.name)
      if (!itemKey || hiddenKeys.has(itemKey)) return
      ...
      const unitCost = Number.isFinite(Number(recipeUnitCostRaw)) ? parseNum(recipeUnitCostRaw) : null
      const cost = unitCost !== null ? parseNum(row.quantity) * unitCost : parseNum(row.cost)
...
function Reports({ t }) {
  ...
  useEffect(() => { writeAikoHiddenSalesKeys(hiddenSalesKeys) }, [hiddenSalesKeys])
  ...
  useEffect(() => {
    ...
    async function loadCloudReports() {
      ...
      if (cancelled) return
      if (error) {
        setCloudSyncStatus('Локальное хранение: для общей истории создайте таблицу rms_sales_reports в Supabase или используйте постоянный production-домен.')
        return
      }
      if (data?.length) {
    ...
    async function loadRecipeCosts() {
      try {
        ...
        ;(menuItems || []).forEach(item => {
          ...
          if (recipeCost > 0) map[normalizeSalesKey(item.name)] = recipeCost
        })
  ...
  const rows = useMemo(() => aggregateSalesRows(filteredReports, { includeBranch: !isNetworkSalesView, hiddenKeys: hiddenSalesKeys, recipeCostMap }), [filteredReports, isNetworkSalesView, hiddenSalesKeys, recipeCostMap])
  ...
  async function importParsedReports(parsedReports) {
    const cleaned = parsedReports.map(r => {
      const rows = (r.rows || []).filter(row => !isAikoModifierRow(row.name, row.source_category) && !hiddenSalesKeys.includes(normalizeSalesKey(row.name)))
  ...
  function exportAikoReportsBackup() {
    const payload = { reports, hiddenSalesKeys, branchMap, exported_at: new Date().toISOString() }
  ...
  const aiSearchResult = useMemo(() => {
    ...
    let resultRows = aggregateSalesRows(baseReports, { includeBranch: branchFilter !== 'all', hiddenKeys: hiddenSalesKeys, recipeCostMap })
  ...
  }, [salesAiQuery, reports, branchFilter, departmentFilter, monthFilter, hiddenSalesKeys, recipeCostMap])

  const SalesTableBlock = <div className="card span-2">
    <div className="card-head"><div><h3>Общая таблица продаж</h3><p className="hint">В общем режиме продажи агрегируются по всей сети без разбивки по филиалам. Себестоимость берётся из техкарт, если позиция найдена в справочнике меню; иначе временно используется себестоимость из AIKO.</p></div><button className="ghost small" onClick={() => setShowHiddenSalesItems(v => !v)}>{showHiddenSalesItems ? 'Скрыть список' : `Скрытые позиции (${hiddenSalesKeys.length})`}</button></div>
    <div className="form-grid compact">
    ...
    {showHiddenSalesItems && <div className="table-wrap" style={{marginTop:12}}><table><thead><tr><th>Скрытая позиция</th><th></th></tr></thead><tbody>
      ...
      {!hiddenSalesKeys.length && <tr><td colSpan="2" className="hint">Скрытых позиций нет. Ненужные строки можно скрывать кнопкой в общей таблице.</td></tr>}
    </tbody></table></div>}
    ...
    <div className="table-wrap" style={{marginTop:12}}><table><thead><tr><th>Позиция</th>{!isNetworkSalesView && <th>Филиал</th>}<th>Тип</th><th>Категория</th><th>{sortHeader('quantity', 'Кол-во')}</th><th>{sortHeader('revenue', 'Выручка')}</th><th>{sortHeader('cost', 'Себестоимость')}</th><th>{sortHeader('profit', 'Прибыль')}</th><th>{sortHeader('margin', 'Маржа')}</th><th></th></tr></thead><tbody>
      {visibleSalesRows.map((r, idx) => <tr key={`${r.name}-${idx}`}><td><b>{r.name}</b></td>{!isNetworkSalesView && <td>{r.branch_name || '—'}</td>}<td><span className={r.department === 'Бар' ? 'pill good' : r.department === 'Кухня' ? 'pill warn' : 'pill'}>{r.department === 'Кухня' ? 'Кухня' : r.department === 'Бар' ? 'Бар' : '—'}</span></td><td>{r.menu_category || r.source_category || '—'}</td><td>{fmt(r.quantity)}</td><td>{fmt(r.revenue)}</td><td>{fmt(r.cost)}</td><td className={r.profit >= 0 ? 'good' : 'bad'}>{fmt(r.profit)}</td><td>{pct(r.margin)}</td><td><button className="ghost small" onClick={() => hideSalesItem(r.name)}>Скрыть</button></td></tr>)}
      {!rows.length && <tr><td colSpan={isNetworkSalesView ? 9 : 10} className="hint">Нет данных</td></tr>}
    </tbody></table></div>
