...
function Attendance({ t }) {
  ...
  async function syncSalaryForEmployee(emp) {
    ...
    const { data: existing } = await supabase.from('salary_periods').select('*').eq('employee_id', emp.id).eq('salary_month', monthDate).maybeSingle()
    const advance = parseNum(existing?.advance_amount)
...
function Salaries({ t }) {
  ...
  function emptySalary(emp) {
    return {
      id: null,
      employee_id: emp.id,
      branch_id: emp.branch_id,
      employees: { full_name: emp.full_name, position: emp.position, monthly_salary: emp.monthly_salary, daily_rate: parseNum(emp.monthly_salary) / DAILY_DIVISOR },
  ...
  async function load() {
    ...
    let salQ = supabase.from('salary_periods').select('*, employees(full_name, position, monthly_salary), branches(name)').eq('salary_month', monthStart(year, month)).order('branch_id')
    if (branchId !== 'all') salQ = salQ.eq('branch_id', branchId)

    const [{ data: emp, error: empError }, { data: sal, error: salError }] = await Promise.all([empQ, salQ])
    if (empError || salError) setMessage(empError?.message || salError?.message)

    const salaryByEmployee = new Map((sal || []).map(r => [r.employee_id, r]))
    const merged = (emp || []).map(e => salaryByEmployee.get(e.id) || emptySalary(e))
    setRows(merged)
  }
  ...
  async function updateSalary(row, patch) {
    ...
    const advance = 'advance_amount' in payload ? payload.advance_amount : parseNum(row.advance_amount)
    const deduction = 'deduction_amount' in payload ? payload.deduction_amount : parseNum(row.deduction_amount)
    if ('salary_gross' in payload || 'advance_amount' in payload || 'deduction_amount' in payload) payload.salary_net = gross - advance - deduction
    ...
    if (row.id) {
      ...
      ;({ error } = await supabase.from('salary_periods').upsert({
        ...
        salary_month: monthStart(year, month),
        ...payload
      }, { onConflict: 'employee_id,salary_month' }))
    }
    if (error) setMessage(error.message)
  ...
  return <section>
    <section className="topbar"><div><h2>{t('salaries_tab')}</h2><p>Все активные сотрудники отображаются здесь сразу после добавления. Табель автоматически формирует начисление.</p></div></section>
    <section className="grid">
      <div className="card span-2">
        <div className="form-grid compact">
          <label><span>{t('year')}</span><select value={year} onChange={e => setYear(Number(e.target.value))}>{defaultYears().map(y => <option key={y} value={y}>{y}</option>)}</select></label>
        ...
        <div className="metric"><span>Итого к выплате</span><strong>{fmt(total)}</strong></div>
        {message && <p className={`hint ${message === t('saved') ? 'good' : 'bad'}`}>{message}</p>}
      </div>
      <div className="card span-2">
        <div className="table-wrap"><table>
          <thead><tr><th>Филиал</th><th>Должность</th><th style={{minWidth:220}}>Сотрудник</th><th>Мес. ставка</th><th>Дневная</th><th>Дни</th><th>Начислено</th><th>Карта</th><th>Наличные</th><th>Аванс</th><th>Удержание</th><th>К выплате</th><th>Комментарий</th></tr></thead>
          <tbody>{rows.map(r => <tr key={r.id || r.employee_id}>
            ...
            <td><input defaultValue={r.advance_amount} onBlur={e => updateSalary(r, { advance_amount: e.target.value })} /></td>
...
function Advances({ t }) {
  ...
  async function load() {
    ...
    if (empError || salError) setMessage(empError?.message || salError?.message)
    const salaryByEmployee = new Map((sal || []).map(r => [r.employee_id, r]))
    setRows((emp || []).map(e => salaryByEmployee.get(e.id) || emptySalary(e)))
  }

  async function updateAdvance(row, patch) {
    setMessage('')
    const payload = { ...patch }
    if ('advance_amount' in payload) payload.advance_amount = parseNum(payload.advance_amount)
    if ('deduction_amount' in payload) payload.deduction_amount = parseNum(payload.deduction_amount)
    const gross = parseNum(row.salary_gross)
    const advance = 'advance_amount' in payload ? payload.advance_amount : parseNum(row.advance_amount)
    const deduction = 'deduction_amount' in payload ? payload.deduction_amount : parseNum(row.deduction_amount)
    payload.salary_net = gross - advance - deduction
  ...
  const totalAdvance = rows.reduce((s, r) => s + parseNum(r.advance_amount), 0)

  return <section>
    <section className="topbar"><div><h2>{t('advances_tab')}</h2><p>Авансы привязаны к сотрудникам и автоматически попадают в зарплатный расчёт.</p></div></section>
    <section className="grid">
      <div className="card span-2">
        <div className="form-grid compact">
          <label><span>{t('year')}</span><select value={year} onChange={e => setYear(Number(e.target.value))}>{defaultYears().map(y => <option key={y} value={y}>{y}</option>)}</select></label>
        ...
        <div className="metric"><span>Итого авансы</span><strong>{fmt(totalAdvance)}</strong></div>
        {message && <p className={`hint ${message === t('saved') ? 'good' : 'bad'}`}>{message}</p>}
      </div>
      <div className="card span-2">
        <div className="table-wrap"><table>
          <thead><tr><th>Филиал</th><th>Должность</th><th style={{minWidth:220}}>Сотрудник</th><th>Мес. ставка</th><th>Дневная</th><th>Дни</th><th>Начислено</th><th>Аванс</th><th>Комментарий</th></tr></thead>
          <tbody>{rows.map(r => <tr key={r.id || r.employee_id}>
            <td>{r.branches?.name || '—'}</td><td>{r.employees?.position || '—'}</td><td>{r.employees?.full_name}</td>
            <td>{fmt(r.employees?.monthly_salary)}</td><td>{fmt(parseNum(r.employees?.monthly_salary) / DAILY_DIVISOR)}</td>
            <td>{fmt(r.worked_days)}</td><td>{fmt(r.salary_gross)}</td>
            <td><input defaultValue={r.advance_amount} onBlur={e => updateAdvance(r, { advance_amount: e.target.value })} /></td>
            <td><input defaultValue={r.comment || ''} onBlur={e => updateAdvance(r, { comment: e.target.value })} /></td>
          </tr>)}{!rows.length && <tr><td colSpan="9" className="hint">Нет активных сотрудников.</td></tr>}</tbody>
        </table></div>
      </div>
    </section>
  </section>
