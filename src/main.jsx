...
function normalizeLogin(login) {
  ...
  if (!value) return ''
  return value.includes('@') ? value : `${value}@nms.local`
}
...
function Settings({ session, t, theme, setTheme }) {
  ...
  async function addUser() {
    ...
    const technicalEmail = normalizeLogin(login)
    const { data, error } = await supabase.auth.signUp({ email: technicalEmail, password: newUser.password.trim() })
    if (error) return setMsg(error.message)
    const userId = data?.user?.id
    if (!userId) return setMsg('Пользователь создан в Auth, но ID не вернулся. Проверь Supabase Auth.')
    const { error: profileError } = await supabase.from('user_profiles').upsert({ id: userId, email: technicalEmail, login_name: login, full_name: newUser.full_name || login, role: 'readonly', is_active: true, ui_theme: theme })
  ...
  return (
    <section><section className="topbar"><div><h2>{t('settings_tab')}</h2><p>{t('settings_subtitle')}</p></div></section>
      <section className="grid">
        ...
        <div className="card span-2"><div className="card-head"><h3>Добавить пользователя</h3></div><p className="hint">Пользователь входит по login. Технический email создаётся автоматически и не показывается в интерфейсе.</p><div className="form-grid compact"><label><span>Login</span><input value={newUser.login} onChange={e => setNewUser({...newUser, login: e.target.value})} placeholder="nigar" /></label><label><span>Временный пароль</span><input type="password" value={newUser.password} onChange={e => setNewUser({...newUser, password: e.target.value})} /></label><label><span>Имя</span><input value={newUser.full_name} onChange={e => setNewUser({...newUser, full_name: e.target.value})} /></label></div><button className="small" onClick={addUser}>+ Добавить пользователя</button></div>
