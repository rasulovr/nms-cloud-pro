...
function Dashboard({ t }) {
  ...
  async function calcMonth(y, m) {
    ...
    // Важно: прогноз прибыли на Дашборде должен совпадать с блоком
    // “Финансы ресторанов → Прогноз месяца → Предполагаемая прибыль”.
    // Поэтому здесь используем ту же итоговую корректировку, что и в Finance.load()
    // для режима ALL_BRANCHES: supplier allocations + DSMF + salary override.
    const financeSupplierTotals = rmsFinanceAllocatedSupplierTotals(purchaseRowsForFinance || [], 1)
    const financeSupplierExpenseTotal = financeSupplierTotals.food + financeSupplierTotals.packaging + financeSupplierTotals.household + financeSupplierTotals.other
    const financePayrollDetails = rmsFinancePayrollDetailsForScope(employeeRows || [], 'all', revenueShareMap)
    const financeDsmfTotal = parseNum(financePayrollDetails.totalDsmf)
    const financeSalaryTotal = parseNum(financePayrollDetails.totalSalary) || parseNum(baseTotals.salary)
    const financeSalaryOverride = financeSalaryTotal - parseNum(baseTotals.salary)
    const forecastProfit = baseForecastProfit - financeSupplierExpenseTotal - Math.max(0, financeSalaryOverride) - financeDsmfTotal
