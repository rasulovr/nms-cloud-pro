...
function Reports({ t }) {
  ...
  const AiSearchBlock = <div className="card span-2">
    ...
    {aiSearchResult && <>
      ...
      <div className="table-wrap" style={{marginTop:12}}><table><thead><tr><th>Позиция</th>{!isNetworkSalesView && <th>Филиал</th>}<th>Тип</th><th>Кол-во</th><th>Цена</th><th>Выручка</th><th>Прибыль</th></tr></thead><tbody>
        {aiSearchResult.rows.slice(0, 20).map((r, idx) => <tr key={`ai-search-${idx}`}><td><b>{r.name}</b>{r.auto_merged && <><br /><span className="hint">автоматически объединено: {r.auto_merged_names.slice(0, 4).join(', ')}{r.auto_merged_names.length > 4 ? '…' : ''}</span></>}</td>{!isNetworkSalesView && <td>{r.branch_name || '—'}</td>}<td>{r.department === 'Кухня' ? 'Кухня' : r.department === 'Бар' ? 'Бар' : '—'}</td><td>{fmt(r.quantity)}</td><td>{fmt(r.avg_price)}</td><td>{fmt(r.revenue)}</td><td className={r.profit >= 0 ? 'good' : 'bad'}>{fmt(r.profit)}</td></tr>)}
        {aiSearchResult.rows.length > 0 && <tr><td colSpan={isNetworkSalesView ? 2 : 3}><b>Итого найдено</b></td><td><b>{fmt(aiSearchResult.totals.quantity)}</b></td><td><b>{fmt(aiSearchResult.totals.quantity ? aiSearchResult.totals.revenue / aiSearchResult.totals.quantity : 0)}</b></td><td><b>{fmt(aiSearchResult.totals.revenue)}</b></td><td className={aiSearchResult.totals.profit >= 0 ? 'good' : 'bad'}><b>{fmt(aiSearchResult.totals.profit)}</b></td></tr>}
        {!aiSearchResult.rows.length && <tr><td colSpan={isNetworkSalesView ? 6 : 7} className="hint">По запросу ничего не найдено.</td></tr>}
      </tbody></table></div>
    </>}
  </div>
