    ...
    <section id="revenuePage">
      ...
      <section className="grid">
        ...
        <div className="card span-2">
          <div className="card-head"><div><h3>{t('daily_expenses_title')}</h3><p className="hint">Изменения суммы, статьи и комментария фиксируются ниже. Отмена перечёркивает строку и исключает её из расчётов.</p></div><div className="actions-row"><button className="small" onClick={forceRecalcBazarForCurrentDate}>Перераспределить Базар</button><button className="small" onClick={addExpense}>+ Добавить</button></div></div>
...
function Recipes({ t }) {
  ...
  function openSemiIngredientsEditor(focusItemId = '') {
    ...
    const nextSelectedId = focusItemId || selectedSemiItemId || selectedSemiItems[0]?.id || ''
    if (nextSelectedId) setSelectedSemiItemId(nextSelectedId)
      ...
      {tab === 'semis' && (
        <section className="grid semifinished-layout">
          ...
          <div className="card semi-composition-card">
            ...
            <p className="hint">Нажмите “Редактировать состав” — все ингредиенты откроются ниже отдельными карточками.</p>
                ...
                <tbody>
                  {selectedSemiItems.map(item => (
                    ...
                    >
                      ...
                      <td><b>{fmt(item.qty)}</b></td>
                      <td><b>{item.unit || 'g'}</b></td>
                      <td><span>{item.component_type === 'manual' ? fmt(item.manual_unit_cost || 0) : 'закупка'}</span></td>
                      <td><span>{fmt(item.waste_percent || 0)}%</span></td>
                      ...
                      <td>
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation()
                            openSemiIngredientsEditor(item.id)
                          }}
                        >
                          Изменить
                        </button>
          ...
          {semiEditModalOpen && (
            <div className="card semi-selected-editor semi-editor-full-width">
              <div className="semi-editor-head">
                <div>
                  <h3>Редактировать состав полуфабриката</h3>
                  <p className="hint">Все ингредиенты открываются ниже в широком редакторе. Поля не переносятся в узкую колонку.</p>
                </div>
                <div className="semi-editor-actions">
                  <button className="small" onClick={() => setSemiEditModalOpen(false)}>Отмена</button>
                  <button className="small primary" onClick={saveSemiIngredientsEditor}>Сохранить изменения</button>
                </div>
              </div>

              <div className="semi-edit-cards semi-edit-cards-wide">
                {semiEditDraft.map(row => (
                  <div
                    className={String(selectedSemiItemId) === String(row.id) ? 'semi-edit-card active' : 'semi-edit-card'}
                    key={row.id}
                    onClick={() => setSelectedSemiItemId(row.id)}
                  >
                    <div className="semi-edit-card-title">{row.item_name || 'Ингредиент'}</div>

                    <div className="semi-edit-card-grid">
                      <label>
                        <span>Ингредиент</span>
                        {row.component_type === 'manual'
                          ? <input value={row.item_name} onChange={e => updateSemiEditDraft(row.id, { item_name: e.target.value })} />
                          : <input value={row.item_name} readOnly />}
                      </label>

                      <label>
                        <span>Кол-во</span>
                        <input value={row.qty} onChange={e => updateSemiEditDraft(row.id, { qty: e.target.value })} />
                      </label>

                      <label>
                        <span>Ед.</span>
                        <select value={row.unit} onChange={e => updateSemiEditDraft(row.id, { unit: e.target.value })}>
                      ...
                      <label>
                        ...
                        {row.component_type === 'manual'
                          ? <input value={row.manual_unit_cost} onChange={e => updateSemiEditDraft(row.id, { manual_unit_cost: e.target.value })} />
                          : <input value="из закупки" readOnly />}
                      </label>

                      <label>
                        <span>Потери %</span>
                        <input value={row.waste_percent} onChange={e => updateSemiEditDraft(row.id, { waste_percent: e.target.value })} />
                      </label>
                    </div>
                  </div>
                ))}
...
function SemiFinishedInlineStyles() {
  return (
    <style>{`
      ...
      .semi-selected-editor h4 {
        margin: 0 0 6px;
      }
      .semi-selected-editor-grid {
        display: grid;
        grid-template-columns: 2fr 1fr 1fr 1.2fr 1fr;
        gap: 12px;
        align-items: end;
      }
      .semi-selected-editor-grid label {
        display: flex;
        flex-direction: column;
        gap: 6px;
        min-width: 0;
      }
      .semi-selected-editor-grid input,
      .semi-selected-editor-grid select {
        width: 100%;
        min-width: 0;
        height: 42px;
        font-size: 14px;
      ...
      .semi-edit-card-grid {
        display: grid;
        grid-template-columns: minmax(180px, 2fr) minmax(90px, .8fr) minmax(80px, .7fr) minmax(110px, 1fr) minmax(90px, .8fr);
        gap: 10px;
        align-items: end;
      }
      .semi-edit-card-grid label {
        display: flex;
        flex-direction: column;
        gap: 6px;
        min-width: 0;
      }
      .semi-edit-card-grid input,
      .semi-edit-card-grid select {
        width: 100%;
        min-width: 0;
        height: 42px;
        padding: 8px 10px;
      ...
      .semi-edit-card-grid input[readonly] {
        background: #f3f4f6;
        color: #64748b;
      }
      ...
      /* v19: wide inline editor for semi-finished ingredients */
      .semi-editor-full-width {
      ...
      .semi-edit-cards-wide {
        display: grid !important;
        grid-template-columns: 1fr !important;
        gap: 12px !important;
      }
      .semi-editor-full-width .semi-edit-card {
        display: grid !important;
      ...
      .semi-editor-full-width .semi-edit-card.active {
        border-color: #94a3b8 !important;
        box-shadow: 0 0 0 2px rgba(148, 163, 184, 0.18);
      }
      .semi-editor-full-width .semi-edit-card-title {
        margin: 0 !important;
        align-self: center;
        white-space: normal;
        line-height: 1.25;
      }
      .semi-editor-full-width .semi-edit-card-grid {
        display: grid !important;
        grid-template-columns: minmax(260px, 2.4fr) minmax(110px, .8fr) minmax(105px, .7fr) minmax(150px, 1fr) minmax(110px, .8fr) !important;
        gap: 12px !important;
        align-items: end !important;
      }
      .semi-editor-full-width .semi-edit-card-grid label {
        min-width: 0 !important;
      }
      .semi-editor-full-width .semi-edit-card-grid input,
      .semi-editor-full-width .semi-edit-card-grid select {
        width: 100% !important;
        min-width: 0 !important;
