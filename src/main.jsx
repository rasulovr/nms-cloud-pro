...
function App() {
  ...
  useEffect(() => {
    async function loadProfile() {
      ...
      try {
        const { data: rpcAccess } = await supabase.rpc('rms_current_user_access')
        if (rpcAccess?.profile) {
          prof = rpcAccess.profile
          perms = rpcAccess.permissions || []
        } else {
          const { data: directProf } = await supabase.from('user_profiles').select('*').eq('id', session.user.id).maybeSingle()
          const { data: directPerms } = await supabase.from('user_permissions').select('*').eq('user_id', session.user.id)
          prof = directProf || null
          perms = directPerms || []
        }
      } catch (_e) {
        const { data: directProf } = await supabase.from('user_profiles').select('*').eq('id', session.user.id).maybeSingle()
  ...
  const visibleSections = isAdmin ? SECTIONS : SECTIONS.filter(s => canReadAccess(sectionAccess(s.id)))
  ...
  }, [permissions, profile, section, accessLoading])

  if (loading || accessLoading) return <div className="login-screen"><div className="login-card">{t('loading')}</div></div>
  if (!session) return <Login lang={lang} setLang={setLang} t={t} />
  if (!visibleSections.length && !isAdmin) return <div className={`app ${theme === 'modern' ? 'theme-modern' : 'theme-classic'}`}><aside className="sidebar"><div className="brand"><ProductLogo compact /><div><h1>{t('system_short_title')}</h1><p>{t('brand_subtitle')}</p></div></div><div className="userbar"><span>{profile?.full_name || profile?.login_name || session.user.email}</span><button className="ghost small" onClick={() => supabase.auth.signOut()}>{t('logout')}</button></div></aside><main className="main"><div className="card"><h3>Нет доступных разделов</h3><p className="hint">Для этого пользователя не выданы права доступа. Назначьте доступ в разделе Настройки → Пользователи.</p></div></main></div>
...
function ProductLogo({ compact = false, login = false }) {
  return <div className={`rms-brandmark ${compact ? 'compact' : ''} ${login ? 'login' : ''}`}>
    <img className="rms-logo-image" src={RMS_LOGO_DATA_URL} alt="RMS logo" />
  </div>
}
...
function Revenue({ t }) {
  ...
  function addAiPercent(rows, { branchName, indicator, amount, revenue, limit, recommendation }) {
    const actual = revenue > 0 ? amount / revenue * 100 : 0
    const deviation = actual - limit
    if (actual > limit) {
      rows.push({
        branchName,
        indicator,
        fact: `${pct(actual)} · ${fmt(amount)} AZN`,
        norm: `≤ ${fmt(limit)}%`,
...
function ThemeStyles() {
  return <style>{`
    ...
    .theme-modern .rms-logo-image { filter: drop-shadow(0 12px 28px rgba(45,212,191,.26)); }
  ...
  async function load() {
    ...
    if (branchId === ALL_BRANCHES) {
      ...
      for (const branch of branches) {
        ...
        branchAiRows.push(...buildAiForBranch({
          branch,
          branchStats,
      ...
      setAiRows(branchAiRows)
    } else {
      setAiRows(buildAiForBranch({
          ...
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                ...
                {!aiRows.length && <tr><td colSpan="6" className="hint">Недостаточно данных для аналитики.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
  ...
  <div class="page">
    ...
    <div class="sheet">
      ...
      <div class="footer">
        ...
        <span>Подпись ответственного: ____________________</span>
      </div>
      <div class="help">Чтобы сохранить техкарту в PDF, нажмите «Печать / PDF» и выберите в диалоге печати пункт «Сохранить как PDF».</div>
    </div>
  </div>
</body>
</html>`
