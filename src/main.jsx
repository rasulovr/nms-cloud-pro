...
function Salaries({ t }) {
  ...
  async function load() {
    ...
    if (allSalError || allAdvError || allPaysError) {
      setMessage(`Список сотрудников загружен. История выплат временно недоступна: ${allSalError?.message || allAdvError?.message || allPaysError?.message}`)
    }
    ...
    ;((allPaysError ? [] : allPays) || []).forEach(p => {
    ...
    setSalaryPayments(allPaysError ? [...(pays || []), ...(previousDisplayPays || [])] : (allPays || [...(pays || []), ...(previousDisplayPays || [])]))
    setDsmfRows(dsmf || [])
