...
function Finance({ t, lang }) {
  ...
  async function load() {
    ...
    for (const r of rawExpenseRows) {
      ...
      if (group === 'food_market' || group === 'packaging' || group === 'household') continue
    ...
    const currentForFinance = (extraDsmf > 0 || salaryOverride > 0 || allocatedSupplierExpenseTotal > 0)
      ? {
        ...current,
        salary: Math.max(parseNum(current.salary), directSalaryTotal),
        ...
        forecastProfit: parseNum(current.forecastProfit) - allocatedSupplierExpenseTotal
      }
      : current
    ...
    if (branchId === ALL_BRANCHES) {
      ...
      for (const branch of branches) {
        ...
        const branchStatsForAi = (!branchDsmfAlready && branchDsmfAmount > 0) || branchSupplierExpenseTotal > 0
          ? {
            ...branchStats,
            expenses: parseNum(branchStats.expenses) + (!branchDsmfAlready ? branchDsmfAmount : 0) + branchSupplierExpenseTotal,
            net: parseNum(branchStats.net) - (!branchDsmfAlready ? branchDsmfAmount : 0) - branchSupplierExpenseTotal
          }
          : branchStats
