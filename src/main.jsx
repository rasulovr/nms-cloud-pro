...
async function rmsCalculateNetworkForecastForMonth(y, m) {
  const TAX_RATE = 8
  ...
  const [
    ...
    supabase.from('monthly_branch_expenses').select('branch_id,total_expenses').eq('month', monthDate),
  ...
  ;(revRows || []).forEach(r => revenueShareMap.set(r.branch_id, revenue > 0 ? parseNum(r.total_revenue) / revenue : 0))
  ...
  const baseForecastProfit = forecastRevenue - (forecastRevenue * costRate) - (forecastRevenue * TAX_RATE / 100)

  const allocatedSupplierTotals = rmsFinanceAllocatedSupplierTotals(purchaseRows || [], 1)
  ...
  const payrollDetails = rmsFinancePayrollDetailsForScope(employeeRows || [], 'all', revenueShareMap)
  const directSalaryTotal = parseNum(payrollDetails.totalSalary) || parseNum(salary)
  const salaryOverride = directSalaryTotal - parseNum(salary)
  const extraDsmf = parseNum(payrollDetails.totalDsmf)

  const forecastProfit = baseForecastProfit - allocatedSupplierExpenseTotal - Math.max(0, salaryOverride) - extraDsmf
  return { forecastRevenue, forecastProfit, avg }
}
