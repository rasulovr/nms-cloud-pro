...
function DashboardStyles() {
  return <style>{`
    ...
    .dash-bar-row { display: grid; grid-template-columns: 120px minmax(180px, 1fr) 150px; gap: 10px; align-items: center; }
    ...
    .dash-bar-value { text-align: right; font-weight: 800; font-size: 12px; display:flex; justify-content:flex-end; gap:8px; align-items:center; white-space:nowrap; }
    .dash-bar-value em { font-style: normal; color: var(--muted); font-weight: 800; }
...
function MiniBarChart({ rows, valueKey = 'revenue', labelKey = 'name', title, subtitle, showShare = false }) {
  ...
  const total = rows.reduce((s, r) => s + Math.max(0, parseNum(r[valueKey])), 0)
    ...
    <div className="dash-bars">
      {rows.map(r => {
        ...
        return <div className="dash-bar-row" key={r.id || r[labelKey]}>
          ...
          <div className="dash-bar-track" title={`${fmt(val)}${showShare ? ` · ${pct(share)} от общей выручки` : ''}`}><div className={`dash-bar ${val < 0 ? 'negative' : ''}`} style={{width: `${width}%`}} /></div>
          <div className={`dash-bar-value ${val < 0 ? 'bad' : ''}`}>{fmt(val)}{showShare && <em>{pct(share)}</em>}</div>
        </div>
      })}
