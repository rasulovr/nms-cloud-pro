              ...
              {groupRows.map(r => {
                ...
                return <React.Fragment key={r.id || r.employee_id}>
                  <tr style={underworked ? { background: 'rgba(155,45,45,.08)' } : undefined}>
                    ...
                    <td>{fmt(r.salary_gross)}</td>
                    <td style={{textAlign:'center'}}><strong className={parseNum(r.final_balance) >= 0 ? 'good' : 'bad'}>{fmt(r.final_balance)}</strong></td>
                    <td>{fmt(r.advance_amount)}</td>
                    <td>{fmt(r.deduction_amount)}</td>
                    <td>{fmt(r.salary_net)}</td>
                    <td>{fmt(r.payroll_payments)}</td>
                    ...
                    <td>{fmt(r.employees?.monthly_salary)}</td>
                    <td>{fmt(calcDailyRate(r.employees))}</td>
  ...
  const productPriceRows = useMemo(() => {
    ...
    filteredPurchases.forEach(p => {
      ;(p.supplier_purchase_items || []).forEach(i => {
        ...
        const price = parseNum(i.price_per_base_unit || i.unit_price)
        if (!id || !price) return
        if (!rowsByProduct.has(id)) rowsByProduct.set(id, {
          id,
          name: product?.name || '—',
          category: product?.category || '—',
          unit: i.base_unit || product?.base_unit || i.unit || '',
          purchases: []
        })
        rowsByProduct.get(id).purchases.push({
          ...
          price,
          unitPrice: parseNum(i.unit_price),
          unit: i.unit,
          baseUnit: i.base_unit || product?.base_unit || '',
    ...
    const result = Array.from(rowsByProduct.values()).map(row => {
      const sorted = row.purchases.sort((a,b) => new Date(b.date) - new Date(a.date))
      const latest = sorted[0]
      const previous = sorted[1]
      const changePct = previous?.price ? ((latest.price - previous.price) / previous.price) * 100 : null
      return { ...row, latest, previous, changePct }
