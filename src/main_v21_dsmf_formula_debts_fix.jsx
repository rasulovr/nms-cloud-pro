...
function DebtsPayments({ t }) {
  ...
  const ledgerBaseRows = [
    ...purchases.map(p => ({
      ...
      comment: p.invoice_number ? `Фактура ${p.invoice_number}` : 'Поступление',
    ...
    ...payments.map(p => ({
      ...
      comment: p.comment || 'Оплата',
      debit: 0,
      credit: parseNum(p.amount)
    }))
  ].filter(r => ledgerSupplierId === 'all' || r.supplier_id === ledgerSupplierId).sort((a, b) => String(a.transaction_date || '').localeCompare(String(b.transaction_date || '')))
  ...
  function printSupplierLedger() {
    ...
    const rowsHtml = supplierLedgerRows.map(r => `<tr><td>${r.transaction_date || ''}</td><td>${r.suppliers?.name || ''}</td><td>${r.comment || ''}</td><td>${fmt(r.debit)}</td><td>${fmt(r.credit)}</td><td>${fmt(r.balance)}</td></tr>`).join('')
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Supplier Ledger</title><style>body{font-family:Arial;padding:24px;color:#17251d}table{width:100%;border-collapse:collapse}th,td{border-bottom:1px solid #ddd;padding:8px;text-align:left}th{background:#f4eddf}</style></head><body><h2>Баланс поставщика: ${supplierName}</h2><table><thead><tr><th>Дата</th><th>Поставщик</th><th>Операция</th><th>Приход / долг</th><th>Оплата</th><th>Остаток</th></tr></thead><tbody>${rowsHtml}</tbody></table></body></html>`
