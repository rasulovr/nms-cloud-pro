...
export default function QRMenu() {
  ...
  const loyaltyQrPayload = loyaltyToken?.token
    ? JSON.stringify({ type: 'rms_loyalty', token: loyaltyToken.token, client_id: loyaltyClient?.id, phone: loyaltyClient?.phone })
    : ''
  const loyaltyQrUrl = loyaltyQrPayload
    ? `https://api.qrserver.com/v1/create-qr-code/?size=260x260&margin=12&data=${encodeURIComponent(loyaltyQrPayload)}`
    : ''
    ...
    {screen === 'loyalty' && <section className="qr-panel qr-loyalty-panel">
      ...
      {loyaltyClient && loyaltyStep === 'verified' ? <>
        ...
        <div className="qr-loyalty-token">
          ...
          <p>Официант сканирует этот QR-код в POS, чтобы привязать чек к вашему профилю и начислить cashback при оплате наличными или картой.</p>
          <button onClick={generateNewLoyaltyToken}>Обновить QR-код</button>
        </div>

        <div className="qr-loyalty-bill-preview">
          <div className="qr-section-head">
            <div>
              <h2>Ваш счёт</h2>
              <p>Состав текущего заказа/счёта для начисления cashback.</p>
            </div>
            <button className="qr-refresh" onClick={loadBill}>Обновить</button>
          </div>
          {!visibleBillItems.length ? <div className="qr-empty"><h3>Заказ пока пустой</h3><p>Добавьте позиции в корзину или попросите официанта открыть счёт.</p></div> : <>
            <div className="qr-loyalty-bill-table">
              <div className="qr-loyalty-bill-head"><span>Наименование</span><span>Кол-во</span><span>Цена</span><span>Сумма</span></div>
              {visibleBillItems.map((i) => <div className="qr-loyalty-bill-row" key={i.id || i.product_id}>
                <span><b>{i.product_name}</b><small>{i.category || ''}</small></span>
                <span>{fmt(i.qty)}</span>
                <span>{fmt(i.price)} AZN</span>
                <span>{fmt(i.total)} AZN</span>
              </div>)}
            </div>
            <div className="qr-loyalty-bill-total"><span>Total</span><b>{fmt(visibleBillTotal)} AZN</b></div>
          </>}
        </div>
