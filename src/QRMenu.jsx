...
export default function QRMenu() {
  ...
  async function findLoyaltyClient(phoneValue = loyaltyPhone, showMessages = true) {
    ...
    if (showMessages) setLoyaltyMessage('Баланс Loyalty загружен.')
  }
  ...
  async function registerLoyaltyClient() {
    ...
    localStorage.setItem('qr_loyalty_phone', phone)
    setLoyaltyClient(data)
    setLoyaltyMessage('Регистрация выполнена. Теперь бонусы будут привязаны к этому номеру.')
  }
  ...
  const loyaltyMaxRedeem = Math.min(loyaltyBalance, Number(((billTotal || cartTotal || 0) * 0.30).toFixed(2)))
  const loyaltyEarnPreview = Number((((billTotal || cartTotal || 0) * 0.05)).toFixed(2))
  ...
  return <div className="qr-page theme-mediterranean">
    ...
    {screen === 'loyalty' && <section className="qr-panel qr-loyalty-panel">
      <div className="qr-section-head">
        <div>
          <h1>Loyalty</h1>
          <p>Введите номер телефона, чтобы увидеть бонусный баланс и уровень клиента.</p>
        </div>
      </div>
      ...
      <div className="qr-loyalty-login">
        ...
        />
        <button onClick={() => findLoyaltyClient()}>Проверить баланс</button>
        <button className="secondary" onClick={registerLoyaltyClient}>Зарегистрироваться</button>
      </div>

      {loyaltyMessage ? <div className="qr-loyalty-message">{loyaltyMessage}</div> : null}

      {loyaltyClient ? <>
        <div className={`qr-loyalty-card ${loyaltyClient.level || 'new'}`}>
          ...
          <div className="qr-loyalty-balance">
            <div><small>Баланс бонусов</small><b>{fmt(loyaltyBalance)} AZN</b></div>
        ...
        <div className="qr-loyalty-rules">
          <div><span>Начисление</span><b>5%</b><small>от оплаченного чека</small></div>
          <div><span>Можно списать</span><b>{fmt(loyaltyMaxRedeem)} AZN</b><small>максимум 30% от текущего чека</small></div>
