...
export default function QRMenu() {
  ...
  const [theme, setTheme] = useState(localStorage.getItem('qr_theme') || 'mediterranean')
  ...
  return (
    <div className={`qr-page ${THEMES[theme]?.className || THEMES.mediterranean.className}`}>
      <Header
        branchId={branchId}
      ...
      {showAd && ad?.is_active ? (
        <AdPopup ad={ad} onClose={() => setShowAd(false)} />
      ) : null}
    </div>
  )
}

function Header({ branchId, tableNumber, screen, setScreen, theme, setTheme }) {
  return (
    <header className="qr-header">
      ...
      <select value={theme} onChange={(e) => setTheme(e.target.value)} aria-label="QR Menu theme">
...
function MenuScreen({ categories, category, setCategory, loadingMenu, filteredProducts, ratings, rateDish, setSelectedPhoto }) {
  return (
    <>
      ...
      {loadingBill ? (
        ...
        <>
          <div className="qr-bill-meta">
            ...
            {billCategories.map((cat) => <span key={cat}>Категория: {cat}</span>)}
          </div>
          ...
          <div className="qr-bill-list">
            {billCategories.map((cat) => (
              <div className="qr-bill-category" key={cat}>
                ...
                {billItems
                  .filter((item) => (item.category || item.category_name || 'Без категории') === cat)
                  .map((item) => (
...
function AdPopup({ ad, onClose }) {
  return (
    <div className="qr-ad-popup">
      ...
      <div>
        <b>{ad.title || 'Акция'}</b>
        <p>{ad.text || ad.description || 'Специальное предложение'}</p>
      </div>
    </div>
  )
}
