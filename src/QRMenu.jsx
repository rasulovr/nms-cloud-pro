...
export default function QRMenu() {
  ...
  async function simulateQrPayment(useBonus = false) {
    ...
    if (!total) return setLoyaltyMessage('Нет суммы заказа/счёта для оплаты.')
    ...
    if (redeemAmount > 0) {
      ...
      if (redeemError) return setLoyaltyMessage(redeemError.message)
    }
    ...
    setLoyaltyMessage(`Оплата засчитана. Списано бонусов: ${fmt(redeemAmount)} AZN. К оплате: ${fmt(netPaidAmount)} AZN. Cashback: ${fmt(cashback)} AZN.`)
  }
    ...
    {screen === 'bill' && <section className="qr-panel">
      ...
      {!visibleBillItems.length ? <div className="qr-empty"><h3>Открытого счёта пока нет</h3><p>Если заказ собран в корзине, он появится здесь после добавления позиций.</p></div> : <>
        ...
        <div className="qr-payment-box">{!payChoice ? <button onClick={() => setPayChoice(true)}>Оплатить</button> : <>
          ...
          {loyaltyClient ? <div className="qr-bonus-pay-box">
            ...
            <button onClick={() => simulateQrPayment(true)} disabled={qrRedeemMax <= 0}>Оплатить с бонусами</button>
          </div> : <div className="qr-bonus-pay-box muted">Войдите в Loyalty, чтобы оплатить бонусами.</div>}
          <button onClick={() => simulateQrPayment(false)}>QR online payment · начислить cashback</button>
