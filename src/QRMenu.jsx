    ...
    {screen === 'loyalty' && <section className="qr-panel qr-loyalty-panel">
      ...
      {loyaltyClient && loyaltyStep === 'verified' ? <>
        ...
        <div className="qr-loyalty-token">
          ...
          <div className="qr-token-box">{loyaltyToken?.token ? loyaltyToken.token.slice(0, 8).toUpperCase() : 'TOKEN'}</div>
          <p>Официант сканирует этот код в POS, чтобы привязать чек и начислить cashback при оплате наличными или картой.</p>
          <button onClick={generateNewLoyaltyToken}>Обновить код</button>
        </div>
