...
<style>
  body { font-family: Arial, sans-serif; color: #17251d; margin: 32px; }
  .header { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #17251d; padding-bottom: 14px; margin-bottom: 20px; }
  ...
  .kpi { border: 1px solid #d9dfd8; border-radius: 14px; padding: 14px; background: #faf8f2; }
  .kpi span { display: block; color: #667085; font-size: 12px; margin-bottom: 8px; }
  .kpi strong { font-size: 24px; }
  table { width: 100%; border-collapse: collapse; margin-top: 18px; }
  th { text-align: left; color: #667085; font-size: 12px; border-bottom: 1px solid #d9dfd8; padding: 10px; }
  td { border-bottom: 1px solid #edf0ec; padding: 10px; font-size: 13px; }
  .num { text-align: right; }
  .strong { font-weight: 800; }
  .footer { margin-top: 24px; color: #667085; font-size: 12px; display: flex; justify-content: space-between; gap: 24px; }
...
<body>
  <div class="header">
    <div>
      <div class="brand">NMS · TECH CARD</div>
      <h1>${escapeHtml(selectedMenu?.name || 'Блюдо не выбрано')}</h1>
      <div class="muted">Категория: ${escapeHtml(selectedMenu?.category || '—')} · Сформировано: ${escapeHtml(generatedAt)}</div>
    </div>
  ...
  function exportTechCardHtml() {
    const blob = new Blob([buildTechCardHtml()], { type: 'text/html;charset=utf-8' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    const safeName = String(selectedMenu?.name || 'techcard').replace(/[^a-zA-Z0-9а-яА-ЯёЁ._-]+/g, '_')
    a.href = url
    a.download = `techcard-${safeName}.html`
        ...
        <div className="card recipe-sheet" style={{ gridColumn: '1 / -1' }}>
          <div className="card-head recipe-sheet-head">
            ...
            <div className="row-actions">
              ...
              {!recipeEditMode && recipeItems.length > 0 && <button className="ghost small" onClick={exportTechCardHtml}>Экспорт HTML</button>}
