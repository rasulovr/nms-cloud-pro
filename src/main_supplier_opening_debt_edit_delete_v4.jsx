...
function DebtsPayments({ t }) {
  ...
  function balanceForSupplier(id) {
    const purchaseTotal = purchases.filter(p => p.supplier_id === id).reduce((sum, p) => sum + parseNum(p.total_amount), 0)
  ...
  function balanceForSupplierLegal(supplierId, legalEntityId) {
    const purchaseTotal = purchases.filter(p => p.supplier_id === supplierId && p.legal_entity_id === legalEntityId).reduce((sum, p) => sum + parseNum(p.total_amount), 0)
  ...
  function supplierAlert(supplier, legalEntityId = '') {
    ...
    const overLimit = limit > 0 && balance > limit ? balance - limit : 0
    const now = new Date()
    const overdue = termDays > 0 ? purchases.filter(p => p.supplier_id === supplier.id && (!legalEntityId || p.legal_entity_id === legalEntityId) && ((now - new Date(p.purchase_date)) / 86400000) > termDays) : []
    return { overLimit, overdueCount: overdue.length, overdueInvoices: overdue.map(p => p.invoice_number || p.purchase_date).slice(0, 5) }
  }
  function suppliersForLegalEntity(entityId) {
    const ids = new Set([
