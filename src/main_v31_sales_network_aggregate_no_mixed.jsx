...
function aggregateSalesRows(reports, options = {}) {
  ...
  ;(reports || []).forEach(report => {
    ;(report.rows || []).forEach(row => {
      ...
      const key = `${normalizeSalesKey(row.name)}|${branchKey}|${dept}`
...
function Reports({ t }) {
  ...
  const rows = useMemo(() => aggregateSalesRows(filteredReports, { includeBranch: !isNetworkSalesView }), [filteredReports, isNetworkSalesView])
  ...
  const aiSearchResult = useMemo(() => {
    ...
    let resultRows = aggregateSalesRows(baseReports, { includeBranch: branchFilter !== 'all' })
  ...
  const SalesTableBlock = <div className="card span-2">
    <div className="card-head"><div><h3>Общая таблица продаж</h3><p className="hint">В общем режиме продажи агрегируются по всей сети без разбивки по филиалам. Разрез по филиалу включается только при выборе конкретного филиала.</p></div></div>
    <div className="form-grid compact">
