...
function App() {
  ...
  useEffect(() => {
    async function loadProfile() {
      ...
      try {
        ...
        const profileId = prof?.id || session.user.id
  ...
  const hasLoadedAccess = Boolean(profile) || permissions.length > 0
  ...
  const visibleSections = isAdmin
    ? SECTIONS
    : hasLoadedAccess
      ? SECTIONS.filter(s => canReadAccess(sectionAccess(s.id)))
      : []
  ...
  if (accessLoading) return <div className="login-screen"><div className="login-card">{t('loading')}</div></div>
  if (!visibleSections.length && !isAdmin) {
    return <div className={`app ${theme === 'modern' ? 'theme-modern' : 'theme-classic'}`}>
      ...
      <main className="main">
        ...
        <div className="card">
          ...
          <p className="hint">Для этого пользователя не выданы права доступа или они не загрузились. Проверьте Настройки → Пользователи под admin.</p>
          <button className="ghost small" onClick={() => supabase.auth.signOut()}>Выйти</button>
        </div>
      </main>
    </div>
  }
  ...
  return (
    <div className={`app ${theme === 'modern' ? 'theme-modern' : 'theme-classic'}`}>
      ...
      <main className={`main ${currentAccess === 'read' ? 'readonly-mode' : ''}`}>
        ...
        {visibleSections.some(s => s.id === section) ? <>
        ...
        </> : <div className="card"><h3>Раздел недоступен</h3><p className="hint">Этот раздел скрыт для текущего пользователя.</p></div>}
      </main>
    </div>
  )
}
