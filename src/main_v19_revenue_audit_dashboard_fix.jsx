...
function Revenue({ t }) {
  ...
  const [revenueRow, setRevenueRow] = useState(null)
  const [expenses, setExpenses] = useState([])
  const [categories, setCategories] = useState([])
  const [monthStats, setMonthStats] = useState({ cash: 0, bank: 0, wolt: 0, revenue: 0 })
  ...
  async function loadLogs(activeBranchId = branchId, activeDate = date) {
    ...
    const { data } = await supabase
      .from('finance_operation_log')
      .select('*')
      .eq('branch_id', activeBranchId)
      .eq('operation_date', activeDate)
  ...
  async function loadMonthStats(activeBranchId = branchId, activeDate = date) {
    ...
    setMonthStats({ cash, bank, wolt, revenue: cash + bank + wolt })
  }
  ...
  async function saveRevenue() {
    ...
    const old = revenueRow || {}
    const { data, error } = await supabase.from('daily_revenue').upsert(payload, { onConflict: 'branch_id,revenue_date' }).select('*').single()
    if (error) return setMessage(error.message)

    const action = revenueRow ? 'update' : 'create'
    await writeLog({ entity_type: 'revenue', record_id: data.id, action, field_name: 'daily_revenue', old_value: revenueRow ? fmt(parseNum(old.cash_amount)+parseNum(old.bank_amount)+parseNum(old.wolt_amount)) : null, new_value: fmt(parseNum(data.cash_amount)+parseNum(data.bank_amount)+parseNum(data.wolt_amount)) })

    for (const f of ['cash_amount', 'bank_amount', 'wolt_amount', 'comment']) {
      const before = old[f] ?? ''
      const after = data[f] ?? ''
      if (revenueRow && String(before) !== String(after)) await writeLog({ entity_type: 'revenue', record_id: data.id, action: 'field_update', field_name: f, old_value: before, new_value: after })
    }

    setRevenueRow(data)
    setForm({ cash_amount: data.cash_amount ?? '', bank_amount: data.bank_amount ?? '', wolt_amount: data.wolt_amount ?? '', comment: data.comment ?? '' })
    await Promise.all([loadMonthStats(branchId, date), loadLogs(branchId, date)])
    setMessage('Выручка сохранена и зафиксирована в журнале')
  }

  async function cancelRevenue() {
    if (!revenueRow?.id || revenueRow.deleted_at) return
    const user = await currentUserMeta()
    const payload = { deleted_at: new Date().toISOString(), deleted_by: user.user_id, updated_by: user.user_id }
    const { error } = await supabase.from('daily_revenue').update(payload).eq('id', revenueRow.id)
    if (error) return setMessage(error.message)
    await writeLog({ entity_type: 'revenue', record_id: revenueRow.id, action: 'cancel', field_name: 'daily_revenue', old_value: fmt(dailyRevenueTotal), new_value: '0' })
    await load()
  }
  ...
  async function restoreRevenue() {
    if (!revenueRow?.id || !revenueRow.deleted_at) return
    const user = await currentUserMeta()
    const payload = { deleted_at: null, deleted_by: null, updated_by: user.user_id }
    const { error } = await supabase.from('daily_revenue').update(payload).eq('id', revenueRow.id)
    if (error) return setMessage(error.message)
    await writeLog({ entity_type: 'revenue', record_id: revenueRow.id, action: 'restore', field_name: 'daily_revenue', old_value: 'cancelled', new_value: fmt(dailyRevenueTotal) })
    await load()
  }
  ...
  async function updateExpense(id, patch) {
    ...
    const user = await currentUserMeta()
    const payload = { ...patch, updated_by: user.user_id }
  ...
  const revenueCancelled = Boolean(revenueRow?.deleted_at)
  const dailyExpenseTotal = expenses.filter(e => !e.deleted_at).reduce((s, e) => s + parseNum(e.amount), 0)
  const dailyRevenueTotal = parseNum(form.cash_amount) + parseNum(form.bank_amount) + parseNum(form.wolt_amount)
  const activeDailyRevenueTotal = revenueCancelled ? 0 : dailyRevenueTotal
  ...
  return (
    <section id="revenuePage">
      ...
      <section className="grid">
        ...
        <div className={`card span-2 ${revenueCancelled ? 'cancelled-row' : ''}`}><div className="card-head"><div><h3>{t('daily_revenue_title')}</h3><p className="hint">Новые суммы и изменения фиксируются в журнале операций ниже.</p></div><div className="row-actions">{revenueCancelled ? <button className="small" onClick={restoreRevenue}>Восстановить</button> : revenueRow?.id ? <button className="remove" onClick={cancelRevenue}>Отменить</button> : null}<button className="small primary" onClick={saveRevenue}>Сохранить</button></div></div><div className="form-grid">
          <MoneyInput label={t('cash')} value={form.cash_amount} disabled={revenueCancelled} onChange={v => setForm(f => ({ ...f, cash_amount: v }))} />
          <MoneyInput label={t('bank')} value={form.bank_amount} disabled={revenueCancelled} onChange={v => setForm(f => ({ ...f, bank_amount: v }))} />
          <MoneyInput label={t('wolt')} value={form.wolt_amount} disabled={revenueCancelled} onChange={v => setForm(f => ({ ...f, wolt_amount: v }))} />
          <label><span>{t('comment')}</span><input value={form.comment} disabled={revenueCancelled} onChange={e => setForm(f => ({ ...f, comment: e.target.value }))} /></label>
        </div><div className="form-grid compact" style={{marginTop:12}}><label><span>Итого за дату</span><input value={fmt(activeDailyRevenueTotal)} readOnly /></label></div>{message && <p className="hint">{message}</p>}</div>
        ...
        <div className="card span-2"><h3>Журнал подтверждённых операций</h3><p className="hint">Каждое создание, изменение и отмена по выбранной дате фиксируется с временем и пользователем.</p><div className="table-wrap"><table><thead><tr><th>Время</th><th>Пользователь</th><th>Раздел</th><th>Действие</th><th>Поле</th><th>Было</th><th>Стало</th></tr></thead><tbody>
          {logs.map(l => <tr key={l.id} className={l.action === 'cancel' ? 'cancelled-row' : ''}><td>{formatDT(l.created_at)}</td><td>{l.user_email || l.user_id || '—'}</td><td>{l.entity_type === 'revenue' ? 'Выручка' : 'Расход'}</td><td>{operationLabel(l.action)}</td><td>{fieldLabel(l.field_name)}</td><td>{l.old_value || '—'}</td><td>{l.new_value || '—'}</td></tr>)}
...
function fieldLabel(field) {
  const labels = { daily_revenue: 'Выручка за дату', expense: 'Расход', cash_amount: 'Наличные', bank_amount: 'Банк', wolt_amount: 'Wolt', comment: 'Комментарий', amount: 'Сумма', category_id: 'Статья', custom_category: 'Своя статья' }
...
function Dashboard({ t }) {
  ...
  async function calcMonth(y, m) {
    ...
    const [{ data: revRows }, { data: expRows }, { data: salRows }, supplierResult] = await Promise.all([
