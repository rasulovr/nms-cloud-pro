...
function Login({ lang, setLang, t, onInternalLogin }) {
  ...
  async function signIn() {
    ...
    if (!rawLogin || !rawPassword) return setError(t('login_error'))
    ...
    if (internalUser) {
      if (internalUser.is_active === false) return setError('Пользователь отключён')
      if (String(internalUser.password || '') !== rawPassword) return setError(t('login_error'))
    ...
    const isRmsInternalEmail = /@(rms|nms)\.local\.az$/i.test(rawLogin) || /@rms\.internal$/i.test(rawLogin)
    if (isRmsInternalEmail || !rawLogin.includes('@')) {
      return setError('Пользователь не найден или пароль неверный. Создайте пользователя заново в Настройки → Пользователи.')
    }
...
function Salaries({ t, view = 'employees' }) {
  ...
  async function load() {
    ...
    let advRows = adv || []
    let payRows = pays || []
    let previousDisplayPayRows = previousDisplayPays || []
    let dsmfExpenseRows = dsmf || []
    let prevSalRows = prevSal || []
    let prevPayRows = prevPays || []
