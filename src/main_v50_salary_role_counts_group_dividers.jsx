...
function Salaries({ t, view = 'employees', isAdmin = false }) {
  ...
  function salaryRoleKey(position) {
    ...
    if (p.includes('стюард')) return 'steward'
    return 'other'
  }
  ...
  function salaryGroupMetaText(rows) {
    ...
    return `сотрудников: ${rows.length} (повара: ${stats.cook} · бар: ${stats.bar} · стюард: ${stats.steward}${stats.other ? ` · прочее: ${stats.other}` : ''})`
  }
