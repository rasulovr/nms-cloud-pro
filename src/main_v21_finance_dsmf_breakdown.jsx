...
function Finance({ t, lang }) {
  ...
  function financeDsmfForSalaryRows(salaryRows = []) {
    ...
    ;(salaryRows || []).forEach(r => {
      ...
      const branchIdValue = r.branch_id || employee.branch_id || null
      const monthly = parseNum(employee.monthly_salary) || parseNum(r.salary_gross)
      if (!monthly) return
      const days = parseNum(officialDays[r.employee_id]) || defaultDays
  ...
  async function load() {
    ...
    let salaryDsmfQuery = supabase.from('salary_periods').select('employee_id, branch_id, salary_gross, employees(monthly_salary, branch_id)').eq('salary_month', monthDate)
    ...
    if (branchId !== ALL_BRANCHES) {
      ...
      salaryDsmfQuery = salaryDsmfQuery.eq('branch_id', branchId)
    }

    const [{ data: rows }, { data: purchaseRows }, { data: employeeRows }, { data: salaryDsmfRows }] = await Promise.all([expQuery, purQuery, empQuery, salaryDsmfQuery])
    ...
    const dsmfAlreadyInExpenses = [...map.keys()].some(name => String(name || '').toLowerCase().includes('dsmf'))
    const calculatedDsmfByBranch = financeDsmfForSalaryRows(salaryDsmfRows || [])
    ...
    const currentForFinance = !dsmfAlreadyInExpenses && calculatedDsmfTotal > 0
      ? { ...current, expenses: parseNum(current.expenses) + calculatedDsmfTotal, net: parseNum(current.net) - calculatedDsmfTotal }
      : current
