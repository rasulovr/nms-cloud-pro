...
function App() {
  ...
  useEffect(() => {
    async function loadProfile() {
      ...
      const permissionUserId = prof?.id || session.user.id
  ...
  useEffect(() => {
    const reloadUserAccess = async () => {
      ...
      const loginName = String(email).split('@')[0] || ''
      let prof = null
      const { data: profById } = await supabase.from('user_profiles').select('*').eq('id', session.user.id).maybeSingle()
      prof = profById || null
      if (!prof && (email || loginName)) {
        const filters = []
        if (email) filters.push(`email.eq.${email}`)
        if (loginName) filters.push(`login_name.eq.${loginName}`)
        const { data: profByLogin } = await supabase.from('user_profiles').select('*').or(filters.join(',')).maybeSingle()
        prof = profByLogin || null
      }
      const permissionUserId = prof?.id || session.user.id
      const { data: perms } = await supabase.from('user_permissions').select('*').eq('user_id', permissionUserId)
      setProfile(prof)
      setPermissions(perms || [])
    }
    window.addEventListener('rms-user-settings-updated', reloadUserAccess)
    return () => window.removeEventListener('rms-user-settings-updated', reloadUserAccess)
  }, [session])

  const currentEmail = session?.user?.email || ''
  const isInternalLoginUser = /@(rms|nms)\.local\.az$/i.test(currentEmail)
  const isAdmin = profile?.role === 'admin' || (!profile && !isInternalLoginUser)
  const sectionAccess = (sectionId) => {
    if (isAdmin) return 'admin'
    if (profile?.is_active === false) return 'none'
  ...
  if (profile?.is_active === false) {
    return <div className="login-screen"><div className="login-card"><ProductLogo login /><h1 className="login-title">{t('system_title')}</h1><p className="bad">Пользователь отключён администратором.</p><button className="primary" onClick={() => supabase.auth.signOut()}>{t('logout')}</button></div></div>
  }
  if (!isAdmin && !visibleSections.length) {
    return <div className="login-screen"><div className="login-card"><ProductLogo login /><h1 className="login-title">{t('system_title')}</h1><p className="bad">Нет доступа ни к одному разделу.</p><button className="primary" onClick={() => supabase.auth.signOut()}>{t('logout')}</button></div></div>
  }
  ...
  return (
    <div className={`app theme-${theme || 'classic'}`}>
      ...
      <main className={`main ${currentAccess === 'read' ? 'readonly-mode' : ''}`}>
        ...
        {canReadAccess(currentAccess) && section === 'salaries' && <SalaryWorkspace t={t} />}
...
function Attendance({ t, mode = 'attendance' }) {
  ...
  async function loadSalaryPrivacyProfile() {
    ...
    if (!rpcError && rpcData) {
      setSalaryPrivacyProfile(Array.isArray(rpcData) ? rpcData[0] : rpcData)
      return
    }
  ...
  async function load() {
    ...
    const shouldHideManagers = Boolean(salaryPrivacyProfile?.hide_manager_salary)
    const visibleEmployees = (emp || [])
...
function Salaries({ t, view = 'employees' }) {
  ...
  async function loadSalaryPrivacyProfile() {
    ...
    if (email) filters.push(`email.eq.${email}`)
    if (loginName) filters.push(`login_name.eq.${loginName}`)
    if (!filters.length) return setSalaryPrivacyProfile(null)
  ...
  useEffect(() => {
    loadSalaryPrivacyProfile()
    const refreshPrivacy = () => loadSalaryPrivacyProfile()
    window.addEventListener('rms-user-settings-updated', refreshPrivacy)
    return () => window.removeEventListener('rms-user-settings-updated', refreshPrivacy)
  }, [])
  useEffect(() => { loadSalaryPrivacyProfile() }, [year, month, branchId, positionFilter])
  ...
  const operationalRows = rows.filter(r => canSeeSalaryValue(r))
  ...
  const dsmfEmployeeRows = operationalRows.map(dsmfLineForRow).filter(r => r.base > 0)
  ...
  const paymentRows = operationalRows.filter(r => paymentBranchId === 'all' || employeeGroupId(r.employees) === paymentBranchId)
  const selectedPaymentRow = operationalRows.find(r => r.employee_id === paymentForm.employee_id)
  ...
  const salaryReportRowsForSelectedDate = operationalRows
    .map(r => {
  ...
  const moneyVisibleRows = operationalRows
  ...
  const branchTotals = staffGroupOptions(branches).map(b => {
    const branchRows = operationalRows.filter(r => employeeGroupId(r.employees) === b.id)
    const visibleBranchRows = branchRows
    const masked = false
    return {
      id: b.id,
      name: b.name,
  ...
  const salaryRows = operationalRows.filter(r => r.employees?.employment_status !== 'terminated' && r.employees?.is_active !== false)
  const archiveRows = operationalRows.filter(r => r.employees?.employment_status === 'terminated' || r.employees?.is_active === false)
    ...
    <section className="grid">
      <div className="card span-2">
        ...
        {shouldHideManagerSalaries
          ? <p className="hint">Зарплаты менеджерского состава полностью скрыты для текущего пользователя.</p>
      ...
      <div className="card span-2">
        ...
        <div className="form-grid compact">
          <label><span>Филиал / группа</span><select value={paymentBranchId} onChange={e => { setPaymentBranchId(e.target.value); const first = operationalRows.find(r => e.target.value === 'all' || employeeGroupId(r.employees) === e.target.value); setPaymentForm(f => ({ ...f, employee_id: first?.employee_id || '' })) }}><option value="all">Все филиалы и менеджеры</option>{staffGroupOptions(branches).map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select></label><label><span>Сотрудник</span><select value={paymentForm.employee_id} onChange={e => setPaymentForm({ ...paymentForm, employee_id: e.target.value })}>{paymentRows.map(r => <option key={r.employee_id} value={r.employee_id}>{employeeGroupName(r.employees)} · {r.employees?.full_name}</option>)}</select></label>
      ...
      <div className="card span-2">
        ...
        <div className="table-wrap"><table>
          ...
          <tbody>{salaryPayments.filter(p => operationalRows.some(r => r.employee_id === p.employee_id)).map(p => {
            const row = operationalRows.find(r => r.employee_id === p.employee_id)
...
function Advances({ t }) {
  ...
  async function loadSalaryPrivacyProfile() {
    ...
    const loginName = String(email).split('@')[0]
    if (!userId) return setSalaryPrivacyProfile(null)
    const { data: profById } = await supabase.from('user_profiles').select('id, email, login_name, role, hide_manager_salary').eq('id', userId).maybeSingle()
    if (profById) return setSalaryPrivacyProfile(profById)
    const filters = []
    if (email) filters.push(`email.eq.${email}`)
    if (loginName) filters.push(`login_name.eq.${loginName}`)
    if (!filters.length) return setSalaryPrivacyProfile(null)
    const { data: profByLogin } = await supabase.from('user_profiles').select('id, email, login_name, role, hide_manager_salary').or(filters.join(',')).maybeSingle()
    setSalaryPrivacyProfile(profByLogin || null)
  }
  ...
  async function load() {
    ...
    const shouldHideManagers = Boolean(salaryPrivacyProfile?.hide_manager_salary)
    const visibleEmployees = (emp || []).filter(e => !(shouldHideManagers && isManagerStaff(e)))
    const visibleEmployeeIds = new Set(visibleEmployees.map(e => e.id))
    setEmployees(visibleEmployees)
    setAdvances((adv || []).filter(a => visibleEmployeeIds.has(a.employee_id)))
    setProfiles(prof || [])
  }
...
function Settings({ session, t, theme, setTheme }) {
  ...
  async function updatePermission(userId, section, access) {
    const normalizedAccess = access === 'none' ? 'none' : access
    let error = null
    if (normalizedAccess === 'none') {
      const res = await supabase.from('user_permissions').delete().eq('user_id', userId).eq('section', section)
      error = res.error
    } else {
      const res = await supabase.from('user_permissions').upsert({ user_id: userId, section, access: normalizedAccess }, { onConflict: 'user_id,section' })
      error = res.error
    }
    if (error) setMsg(error.message); else {
      setPermissions(prev => {
        const rest = prev.filter(p => !(p.user_id === userId && p.section === section))
        return normalizedAccess === 'none' ? rest : [...rest, { user_id: userId, section, access: normalizedAccess }]
      })
      try { window.dispatchEvent(new Event('rms-user-settings-updated')) } catch (_e) {}
      setMsg(t('saved'))
      load()
