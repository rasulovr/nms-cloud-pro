...
function Revenue({ t }) {
  ...
  const [monthStats, setMonthStats] = useState({ cash: 0, bank: 0, wolt: 0, revenue: 0, expenses: 0, inflows: 0 })
  ...
  async function loadMonthStats(activeBranchId = branchId, activeDate = date) {
    ...
    const [{ data: monthRows }, { data: expRows }, { data: inflowRows }] = await Promise.all([
    ...
    setMonthStats({ cash, bank, wolt, revenue: cash + bank + wolt, expenses, inflows })
  }
    ...
    <section id="revenuePage">
      ...
      <section className="grid">
        ...
        <div className="card span-2"><h3>{t('revenue_summary')}</h3>
          ...
          <Metric label="Приходы за дату" value={fmt(dailyInflowTotal)} />
          ...
          <Metric label="Приходы месяца" value={fmt(monthStats.inflows)} />
...
function entityLabel(entity) {
  const labels = { revenue: 'Выручка', expense: 'Расход', inflow: 'Приход', cash: 'Касса' }
...
function fieldLabel(field) {
  const labels = { daily_revenue: 'Выручка за дату', daily_revenue_entry: 'Строка выручки', cash_register: 'Касса за день', cash_inflow: 'Приход', expense: 'Расход', source: 'Источник', cash_amount: 'Наличные', bank_amount: 'Банк', wolt_amount: 'Wolt', comment: 'Комментарий', amount: 'Сумма', category_id: 'Статья', custom_category: 'Своя статья' }
