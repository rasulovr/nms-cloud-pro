...
function Reports({ t }) {
  ...
  const previousRowsByName = useMemo(() => {
    const map = new Map()
    previousMonthRows.forEach(r => map.set(`${normalizeSalesKey(r.name)}|${r.department || ''}|${isNetworkSalesView ? 'all' : (r.branch_id || r.branch_name || '')}`, r))
    return map
