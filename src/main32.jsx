...
function Revenue({ t }) {
  ...
  async function loadMonthStats(activeBranchId = branchId, activeDate = date) {
    ...
    const [{ data: monthRows }, { data: expRows }, { data: inflowRows }, { data: svcRows }] = await Promise.all([
    ...
    const expenses = (expRows || []).reduce((s, r) => s + parseNum(r.amount), 0)
  ...
  const dailyExpenseTotal = expenses.filter(e => !e.deleted_at).reduce((s, e) => s + parseNum(e.amount), 0)
    ...
    <section id="revenuePage">
      ...
      <section className="grid">
        ...
        <div className="card span-2"><div className="card-head"><div><h3>{t('daily_revenue_title')}</h3><p className="hint">Кнопка “Добавить” создаёт строку выручки ниже. Любые изменения фиксируются в журнале операций.</p></div><button className="small primary" onClick={addRevenueEntry}>Добавить</button></div><div className="form-grid">
          ...
          <div className="table-wrap" style={{marginTop:14}}><table><thead><tr><th>{t('cash')}</th><th>{t('bank')}</th><th>{t('wolt')}</th><th>{t('comment')}</th><th>Итого</th><th>Статус</th><th></th></tr></thead><tbody>
            {revenueEntries.map(r => <RevenueEntryRow key={r.id} row={r} onSave={patch => updateRevenueEntry(r.id, patch)} onCancel={() => cancelRevenueEntry(r.id)} />)}
            {!revenueEntries.length && <tr><td colSpan="7" className="hint">Пока нет добавлений</td></tr>}
          </tbody></table></div>
        </div>
        ...
        <div className="card span-2">
          ...
          <div className="table-wrap"><table><thead><tr><th>{t('expense_item')}</th><th>{t('amount')}</th><th>{t('comment')}</th><th>Статус</th><th></th></tr></thead><tbody>
            ...
            {!expenses.length && <tr><td colSpan="5" className="hint">—</td></tr>}
          </tbody></table></div>
        </div>
        ...
        <div className="card span-2">
          <div className="card-head"><div><h3>Настройка service charge филиала</h3><p className="hint">Указывается один раз для ресторана. При вводе выручки расчёт появляется рядом с суммами; отдельная ежедневная таблица не используется.</p></div><button className="small primary" onClick={saveServiceCharge}>Сохранить настройку филиала</button></div>
          <div className="form-grid">
            <label className="checkbox-row"><input type="checkbox" checked={serviceForm.enabled} onChange={e => setServiceForm(f => ({ ...f, enabled: e.target.checked }))} /> В этом филиале есть service charge</label>
            <MoneyInput label="Service charge % в счёте" value={serviceForm.service_percent} onChange={v => setServiceForm(f => ({ ...f, service_percent: v }))} />
            <MoneyInput label="% затрат персоналу от базы" value={serviceForm.staff_cost_percent} onChange={v => setServiceForm(f => ({ ...f, staff_cost_percent: v }))} />
            <label><span>Service charge внутри выручки за дату</span><input value={fmt(dailyServiceChargeAmount)} readOnly /></label>
            <label><span>Затраты персоналу за дату</span><input value={fmt(dailyServiceStaffCost)} readOnly /></label>
          </div>
        </div>
...
function RevenueEntryRow({ row, onSave, onCancel }) {
      ...
      <section className="grid">
        ...
        <div className="card span-2">
          ...
          <div className="form-grid compact">
            ...
            <label><span>Позиция</span><input value={transferForm.position} onChange={e => setTransferForm({...transferForm, position: e.target.value})} /></label>
...
function Suppliers({ t }) {
  ...
  function updateLine(index, patch) {
    setLineRows(rows => rows.map((row, i) => {
      ...
      if (Object.prototype.hasOwnProperty.call(patch, 'category') && !Object.prototype.hasOwnProperty.call(patch, 'product_id')) next.product_id = ''
      return next
    }))
  }
  function productLabel(product) { return product ? `${product.category || 'Прочее'} · ${product.name}` : '' }
  function productOptionsForRow(row) {
    const sameCategory = products.filter(p => (p.category || 'Прочее') === row.category)
    const otherCategories = products.filter(p => (p.category || 'Прочее') !== row.category)
    return [...sameCategory, ...otherCategories]
  }
  function selectProductForLine(index, productId) {
    const selected = products.find(p => p.id === productId)
    setLineRows(rows => rows.map((row, i) => i === index ? {
      ...row,
      product_id: selected?.id || '',
      category: selected?.category || row.category || PRODUCT_CATEGORIES[0]
    } : row))
  }
  ...
  async function softDeletePurchase(id) {
    if (!window.confirm('Отменить поступление? Оно будет скрыто из списка, но история сохранится.')) return
  ...
  return <section>
    ...
    <section className="grid">
      <div className="card span-2">
        ...
        <div className="table-wrap"><table><thead><tr><th>Категория</th><th>Товар</th><th>Кол-во закупа</th><th>Ед. закупа</th><th>Цена за ед.</th><th>Сумма</th><th></th></tr></thead><tbody>{lineRows.map((row, idx) => <tr key={idx}>
      ...
      <div className="card span-2">
        ...
        <div className="form-grid compact"><label><span>Товар</span><input value={productForm.name} onChange={e => setProductForm({...productForm, name: e.target.value})} /></label><label><span>Категория</span><select value={productForm.category} onChange={e => setProductForm({...productForm, category: e.target.value})}>{PRODUCT_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select></label><label><span>Базовая ед. для техкарты</span><select value={productForm.base_unit} onChange={e => setProductForm({...productForm, base_unit: e.target.value})}>{BASE_UNITS.map(u => <option key={u.value} value={u.value}>{u.label}</option>)}</select></label></div><button className="small" onClick={addProductFromForm}>+ Добавить товар</button>
      </div>

     <div className="card span-2"><div className="card-head"><div><h3>Редактирование последних поступлений</h3><p className="hint">Список последних поступлений. Оплаты и долги вынесены в раздел “Долги и оплаты”.</p></div></div><div className="table-wrap"><table><thead><tr><th>Дата</th><th>Фактура</th><th>Поставщик</th><th>Наш VOEN</th><th>Филиал</th><th>Товары</th><th>Сумма</th><th>История</th><th></th></tr></thead><tbody>{purchases.slice(0, 50).map(p => <tr key={p.id}><td>{editingPurchaseId === p.id ? <input type="date" defaultValue={p.purchase_date} onBlur={e => updatePurchase(p.id, { purchase_date: e.target.value })} /> : p.purchase_date}</td><td>{editingPurchaseId === p.id ? <input defaultValue={p.invoice_number || ''} onBlur={e => updatePurchase(p.id, { invoice_number: e.target.value.trim() || null })} /> : (p.invoice_number || '—')}</td><td>{p.suppliers?.name}</td><td>{p.legal_entities?.voen}</td><td>{editingPurchaseId === p.id ? <select defaultValue={p.branch_id || ''} onBlur={e => updatePurchase(p.id, { branch_id: e.target.value || null })}><option value="">—</option>{branches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select> : (p.branches?.name || '—')}</td><td>{(p.supplier_purchase_items || []).map(i => <div key={i.id} style={{display:'grid', gridTemplateColumns:'1.2fr .7fr .8fr .8fr', gap:8, marginBottom:6}}>{editingPurchaseId === p.id ? <select defaultValue={i.product_id} onBlur={e => updatePurchaseItem(p.id, i, { product_id: e.target.value })}>{products.map(prod => <option key={prod.id} value={prod.id}>{prod.category} · {prod.name}</option>)}</select> : <span>{i.supplier_products?.name}</span>}{editingPurchaseId === p.id ? <input inputMode="decimal" defaultValue={i.quantity} onBlur={e => updatePurchaseItem(p.id, i, { quantity: parseNum(e.target.value) })} /> : <span>{fmt(i.quantity)}</span>}{editingPurchaseId === p.id ? <select defaultValue={i.unit} onBlur={e => updatePurchaseItem(p.id, i, { unit: e.target.value })}>{PURCHASE_UNITS.map(u => <option key={u.value} value={u.value}>{u.value}</option>)}</select> : <span>{i.unit}</span>}{editingPurchaseId === p.id ? <input inputMode="decimal" defaultValue={i.unit_price} onBlur={e => updatePurchaseItem(p.id, i, { unit_price: parseNum(e.target.value) })} /> : <span>{fmt(i.unit_price)} AZN</span>}</div>)}</td><td><strong>{fmt(p.total_amount)}</strong></td><td className="hint"><div>Создано: {p.created_at ? new Date(p.created_at).toLocaleString() : '—'} · {userName(p.created_by)}</div><div>Изменено: {p.updated_at ? new Date(p.updated_at).toLocaleString() : '—'} · {userName(p.updated_by)}</div>{p.deleted_at && <div>Удалено: {new Date(p.deleted_at).toLocaleString()} · {userName(p.deleted_by)}</div>}</td><td><button className="ghost small" onClick={() => setEditingPurchaseId(editingPurchaseId === p.id ? '' : p.id)}>{editingPurchaseId === p.id ? 'Готово' : 'Ред.'}</button><button className="remove" onClick={() => softDeletePurchase(p.id)}>×</button></td></tr>)}{!purchases.length && <tr><td colSpan="9" className="hint">—</td></tr>}</tbody></table></div></div>
    </section>
  </section>
}
...
function Settings({ session, t, theme, setTheme }) {
  ...
  async function load() {
    const [{ data: u }, { data: le }, { data: perms }] = await Promise.all([
  ...
  async function createProfileForCurrentUser() {
    await supabase.from('user_profiles').upsert({ id: session.user.id, email: session.user.email, full_name: fullName || session.user.email, role: 'admin', ui_theme: theme })
    setMsg(t('saved'))
    load()
  }
  ...
  async function addUser() {
    setMsg('')
    if (!newUser.email.trim()) return setMsg('Введите email пользователя')
    ...
    if (!userId) return setMsg('Пользователь создан в Auth, но ID не вернулся. Проверь Supabase Auth.')
    const { error: profileError } = await supabase.from('user_profiles').upsert({ id: userId, email: newUser.email.trim(), full_name: newUser.full_name || newUser.email.trim(), role: newUser.role, is_active: true, ui_theme: theme })
    if (profileError) return setMsg(profileError.message)
    const defaultAccess = editableSections.map(s => ({ user_id: userId, section: s.id, access: newUser.role === 'admin' ? 'admin' : 'read' }))
    ...
    setMsg('Пользователь добавлен. Если вход не работает, проверь email confirmation в Supabase Auth.')
    load()
  }
  ...
  return (
    <section><section className="topbar"><div><h2>{t('settings_tab')}</h2><p>{t('settings_subtitle')}</p></div></section>
      <section className="grid">
        ...
        <div className="card span-2"><div className="card-head"><h3>Добавить пользователя</h3></div><p className="hint">Создаёт пользователя через Supabase Auth и профиль в системе.</p><div className="form-grid compact"><label><span>Email</span><input value={newUser.email} onChange={e => setNewUser({...newUser, email: e.target.value})} placeholder="user@example.com" /></label><label><span>Временный пароль</span><input type="password" value={newUser.password} onChange={e => setNewUser({...newUser, password: e.target.value})} /></label><label><span>Имя</span><input value={newUser.full_name} onChange={e => setNewUser({...newUser, full_name: e.target.value})} /></label><label><span>Роль</span><select value={newUser.role} onChange={e => setNewUser({...newUser, role: e.target.value})}><option value="readonly">Readonly</option><option value="manager">Manager</option><option value="accountant">Accountant</option><option value="admin">Admin</option></select></label></div><button className="small" onClick={addUser}>+ Добавить пользователя</button></div>

        <div className="card span-2"><div className="card-head"><h3>Права доступа</h3></div><p className="hint">Нет доступа / Просмотр / Редактирование / Админ по каждому разделу.</p><div className="table-wrap"><table><thead><tr><th>Пользователь</th><th>Роль</th><th>Активен</th><th>Разделы</th></tr></thead><tbody>{users.map(u => <tr key={u.id}><td><b>{u.full_name || u.email || u.id}</b><br /><span className="hint">{u.email || u.id}</span></td><td><select value={u.role || 'readonly'} onChange={e => updateUser(u.id, { role: e.target.value })}><option value="readonly">Readonly</option><option value="cashier">Cashier</option><option value="manager">Manager</option><option value="accountant">Accountant</option><option value="chef">Chef</option><option value="admin">Admin</option></select></td><td><select value={String(u.is_active !== false)} onChange={e => updateUser(u.id, { is_active: e.target.value === 'true' })}><option value="true">Да</option><option value="false">Нет</option></select></td><td><div className="permission-grid">{editableSections.map(sec => <React.Fragment key={`${u.id}-${sec.id}`}><b>{t(sec.key)}</b><select value={u.role === 'admin' ? 'admin' : getPermission(u.id, sec.id)} disabled={u.role === 'admin'} onChange={e => updatePermission(u.id, sec.id, e.target.value)}><option value="none">Нет доступа</option><option value="read">Просмотр</option><option value="edit">Редактирование</option><option value="admin">Админ</option></select></React.Fragment>)}</div></td></tr>)}</tbody></table></div></div>
